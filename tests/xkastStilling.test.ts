...
function participant(
  ...
  return { kasterid, navn, omganger: omganger.map(([poeng, antall_ringer]) => ({ poeng, antall_ringer })) }
}
...
describe('buildXkastStanding', () => {
  it('ranks by total poeng first', () => {
    const rows = buildXkastStanding([
      participant(1, 'Lav', [[5, 1], [3, 0]]),
      participant(2, 'Hog', [[12, 2], [10, 2]]),
    ])
  ...
  it('breaks poeng ties on total ringere', () => {
    const rows = buildXkastStanding([
      participant(1, 'Faa ringar', [[12, 0], [8, 1]]),   // 20 poeng, 1 ringer
      participant(2, 'Mange ringar', [[10, 2], [10, 2]]), // 20 poeng, 4 ringere
    ])
    expect(rows.map(r => r.kasterid)).toEqual([2, 1])
  ...
  it('breaks poeng+ringer ties on best single omgang', () => {
    const rows = buildXkastStanding([
      participant(1, 'Jamn', [[10, 2], [10, 2]]),   // best omgang 10
      participant(2, 'Topp', [[15, 3], [5, 1]]),    // best omgang 15
    ])
    expect(rows.map(r => r.kasterid)).toEqual([2, 1])
  })
  ...
  it('gives fully tied players the same placement and skips ahead', () => {
    const rows = buildXkastStanding([
      participant(1, 'A', [[12, 2], [5, 1]]),
      participant(2, 'B', [[12, 2], [5, 1]]),
      participant(3, 'C', [[5, 1], [5, 1]]),
    ])
    expect(rows.map(r => r.plassering)).toEqual([1, 1, 3])
  })

  it('counts null ringere as zero without affecting poeng', () => {
    const rows = buildXkastStanding([participant(1, 'A', [[12, null], [8, 1]])])
