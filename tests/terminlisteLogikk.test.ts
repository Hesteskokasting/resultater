...
describe('groupSchedule', () => {
  ...
  it('groups multiple rows into the same month and preserves input order within it', () => {
    ...
    expect(groups.upcoming).toHaveLength(1)
    expect(groups.upcoming[0]!.rows.map(r => r.dato)).toEqual(['2026-08-01', '2026-08-20', '2026-08-10'])
  })
...
describe('sortSchedule', () => {
  it('sorts by dato ascending', () => {
    const rows = [sortRow({ dato: '2026-08-10' }), sortRow({ dato: '2026-07-01' }), sortRow({ dato: '2026-09-05' })]
    const sort: ScheduleSort = { column: 'dato', direction: 'asc' }
    expect(sortSchedule(rows, sort).map(r => r.dato)).toEqual(['2026-07-01', '2026-08-10', '2026-09-05'])
  })

  it('sorts by dato descending', () => {
    const rows = [sortRow({ dato: '2026-08-10' }), sortRow({ dato: '2026-07-01' }), sortRow({ dato: '2026-09-05' })]
    const sort: ScheduleSort = { column: 'dato', direction: 'desc' }
    expect(sortSchedule(rows, sort).map(r => r.dato)).toEqual(['2026-09-05', '2026-08-10', '2026-07-01'])
  })

  it('sorts by navn using Norwegian locale compare', () => {
    const rows = [sortRow({ navn: 'Øystre Cup' }), sortRow({ navn: 'Aker Cup' }), sortRow({ navn: 'Bergen Cup' })]
    const sort: ScheduleSort = { column: 'navn', direction: 'asc' }
    expect(sortSchedule(rows, sort).map(r => r.navn)).toEqual(['Aker Cup', 'Bergen Cup', 'Øystre Cup'])
  })
  ...
  it('sorts by metode joining innledende and avsluttende', () => {
    ...
    expect(sortSchedule(rows, sort).map(r => `${r.innledende?.navn ?? ''} ${r.avsluttende?.navn ?? ''}`.trim()))
