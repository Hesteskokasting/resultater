import { buildMatchPlayerUpdates, type RoundScoreRow, type MatchSideConfirm } from '@/services/kampService'

const P1_ID = 101
const P2_ID = 202

function row(spelarId: number, score: number | null, rings: number | null): RoundScoreRow {
...
describe('buildMatchPlayerUpdates — Singel', () => {
  describe('omgang row accumulation', () => {
    it('sums score and antall_ringer across all rows for each player', () => {
      ...
      const res = buildMatchPlayerUpdates({ roundData, side1, side2, ...NO_HCP, erWalkover: false })
  ...
  describe('HCP', () => {
    it('HCP adds to score_poeng but not antall_ringer', () => {
      ...
      const res = buildMatchPlayerUpdates({ roundData, side1, side2, hcp1: 3, hcp2: 5, erWalkover: false })
    ...
    it('HCP-boosted score determines kamp_poeng — lower raw score can still win', () => {
      ...
      const res = buildMatchPlayerUpdates({ roundData, side1, side2, hcp1: 5, hcp2: 0, erWalkover: false })
  ...
  describe('null side', () => {
    it('produces no entries for the null side, entries for the other', () => {
      const res = buildMatchPlayerUpdates({ roundData: [], side1, side2: null, ...NO_HCP, erWalkover: false })
...
describe('buildMatchPlayerUpdates — Par/Mix', () => {
  // posisjon 1 throws odd omgangar, posisjon 2 even
  const A1 = 101, A2 = 102  // pair A: posisjon 1, posisjon 2
  const B1 = 201, B2 = 202  // pair B
  const parA: MatchSideConfirm = { playerIds: [A1, A2], baseScore: 0 }
  const parB: MatchSideConfirm = { playerIds: [B1, B2], baseScore: 0 }
  ...
  it('each player gets score_poeng and antall_ringer from their OWN omgangar only', () => {
    const res = buildMatchPlayerUpdates({ roundData, side1: parA, side2: parB, ...NO_HCP, erWalkover: false })
  ...
  it('side HCP lands on the rep only, so the side sum counts it exactly once', () => {
    const res = buildMatchPlayerUpdates({ roundData, side1: parA, side2: parB, hcp1: 3, hcp2: 0, erWalkover: false })
  ...
  it('walkover: rep gets 21, partner 0, both get kamp_poeng=2', () => {
    const res = buildMatchPlayerUpdates({ roundData: [], side1: parA, side2: null, ...NO_HCP, erWalkover: true })
  ...
  it('a Par side draws against a Singel side on equal side totals', () => {
    ...
    const res = buildMatchPlayerUpdates({ roundData: data, side1: parA, side2: { playerIds: [S1], baseScore: 0 }, ...NO_HCP, erWalkover: false })
