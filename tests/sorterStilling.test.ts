...
function confirmedMatch(spelarar: { kasterid: number; kamp_poeng: number; score_poeng?: number }[]): MatchForSorting {
  return { er_bekreftet: true, spelarar }
}

function unconfirmedMatch(spelarar: { kasterid: number; kamp_poeng: number; score_poeng?: number }[]): MatchForSorting {
