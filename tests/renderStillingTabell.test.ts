...
function player(kasterid: number, naam: string | null = null, overrides: Partial<StandingRow> = {}): StandingRow {
...
describe('renderStandingTable', () => {
  describe('wrapper and heading', () => {
    ...
    it('heading shows "N spelarar" when hasMatchCount is true', () => {
      const el = parse(renderStandingTable([player(1), player(2)], NO_KAMPER, NO_STARTNR, { hasMatchCount: true }))
  ...
  describe('table identity', () => {
    ...
    it('uses the tableId option when provided', () => {
      const el = parse(renderStandingTable([player(1)], NO_KAMPER, NO_STARTNR, { tableId: 'my-tabell' }))
  ...
  describe('player rows', () => {
    it('renders one player row per stilling entry', () => {
      const el = parse(renderStandingTable([player(1), player(2), player(3)], NO_KAMPER, NO_STARTNR))
    ...
    it('sets the correct data-kasterid on each player row', () => {
      ...
      const ids = [...el.querySelectorAll('tr.standing-player-row')].map(tr => tr.getAttribute('data-kasterid'))
    ...
    it('# column shows sequential position within the group', () => {
      const el = parse(renderStandingTable([player(1), player(2), player(3)], NO_KAMPER, NO_STARTNR))
      const positions = [...el.querySelectorAll('tr.standing-player-row')].map(tr => tr.querySelector('td')?.textContent)
  ...
  describe('column headers', () => {
    ...
    it('K header present when hasMatchCount is true', () => {
      const el = parse(renderStandingTable([player(1)], NO_KAMPER, NO_STARTNR, { hasMatchCount: true }))
      const headers = [...el.querySelectorAll('thead th')].map(th => th.textContent)
  ...
  describe('hasElimination', () => {
    it('adds final-elim-position class to the # cell of eliminated players', () => {
      ...
      const el = parse(renderStandingTable(stilling, NO_KAMPER, NO_STARTNR, { hasElimination: true }))
    ...
    it('final-elim-position is absent when hasElimination is false even for eliminated players', () => {
      ...
      const el = parse(renderStandingTable(stilling, NO_KAMPER, NO_STARTNR, { hasElimination: false }))
      expect(el.querySelector('tr.standing-player-row td')?.classList.contains('final-elim-position')).toBe(false)
    })
  })
