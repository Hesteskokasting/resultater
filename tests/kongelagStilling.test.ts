...
function xkastRow(kasterid: number, poeng: number): KongelagSeedingRow {
  return { kasterid, poeng_xkast: poeng, antall_ring_xkast: 0, kamp_poeng_innl: null, score_poeng_innl: null }
}

function kampRow(kasterid: number, kampPoeng: number): KongelagSeedingRow {
  return { kasterid, poeng_xkast: null, antall_ring_xkast: null, kamp_poeng_innl: kampPoeng, score_poeng_innl: 50 }
}

function standingRow(kasterid: number, poeng: number, ringer: number, omganger: number[]): XkastStandingRow {
  return {
    kasterid,
...
describe('calcCarryOverByKasterid', () => {
  it('rounds the normalized X-kast total', () => {
    const carry = calcCarryOverByKasterid(
      [xkastRow(1, 300), xkastRow(2, 151)],
      { isXkast: true, antallOmganger: 15 },
    )
  ...
  it('passes kamp_poeng_innl through unrounded (fractional draw points)', () => {
    const carry = calcCarryOverByKasterid(
      [kampRow(1, 1.5), kampRow(2, 4)],
      { isXkast: false, antallOmganger: null },
    )
  ...
  it('treats missing innledende values as 0', () => {
    const carry = calcCarryOverByKasterid(
      [{ kasterid: 1, poeng_xkast: null, antall_ring_xkast: null, kamp_poeng_innl: null, score_poeng_innl: null }],
