...
describe("buildCupMatchRows", () => {
  it("assigns sequential lanes from laneStart for non-walkover matches", () => {
    ...
    expect(
      built.every(
        (b) =>
          b.match.stevneid === 10 && b.match.fase === "avsluttende" && b.match.runde_nummer === 1,
      ),
    ).toBe(true);
  });
  ...
  it("expands a representative kasterid to all side members (pairs/mix)", () => {
    ...
    const pairings: CupPairing[] = [{ players: [1, 3], isWalkover: false, isThreePlayers: false }];
  ...
  it("falls back to the kasterid itself when it has no side mapping (singel)", () => {
    const pairings: CupPairing[] = [{ players: [7, 8], isWalkover: false, isThreePlayers: false }];
