  ...
  `}function G(e,t,n=0,r=0){let i=e.slice(0,t),a=e.slice(t),o=e.some(e=>e.poeng_xkast!=null);function s(e,t=0){return e.map((e,n)=>{let r=n<t,i=o?e.antall_ring_xkast??0:e.kamp_poeng??0,a=o?e.poeng_xkast??0:e.score_poeng??0;return`
    ...
    <div class="card">
      ...
      <span class="small text-muted text-nowrap">${p(e)}</span>
        ...
        </div>`:``,ee=r===!0&&g>0?`<div class="d-flex gap-3 flex-wrap mb-3">
      ...
      <div class="card final-dialog-card-wide">
        <div class="final-dialog-drag-handle">
          ...
          <h5 class="mb-1">Gruppe ${d(n)} — Runde ${a}</h5>
    ...
    `,s.querySelector(`#avbryt-tre-btn`).addEventListener(`click`,()=>s.remove()),s.querySelectorAll(`[data-kasterid]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=Number(e.dataset.kasterid),n=o.indexOf(t);n===-1?o.length<2&&o.push(t):o.splice(n,1),c()})}),s.querySelector(`#bekreft-tre-btn`)?.addEventListener(`click`,async()=>{if(o.length!==2)return;let a=t.find(e=>!o.includes(e.rep.kasterid))??null,c=o.map(e=>t.find(t=>t.rep.kasterid===e)).filter(e=>e!=null).map(e=>e.members.map(e=>e.kasterid)),l=t.flatMap(e=>e.members.map(e=>e.kasterid));s.remove();let{error:u}=await S({kampId:e.id,stevneId:n,roundNumber:e.runde_nummer,roundName:e.runde_navn,allThrowerIds:l,eliminatedIds:a?.members.map(e=>e.kasterid)??[],advancingSides:c});if(u){i(`DB-feil ved bekreft`,`error`);return}await r()})}c()}function Se(e){return typeof e!=`object`||!e||Array.isArray(e)?null:e}function Ce(e){return e.map(e=>({er_bekreftet:e.er_bekreftet,er_walkover:e.er_walkover,runde_nummer:e.runde_nummer,bane_nummer:e.bane_nummer,spelarar:we(e.spelarar)}))}function we(e){return e.map(e=>({kasterid:e.kasterid??0,kamp_poeng:e.kamp_poeng??0,score_poeng:e.score_poeng??0,antall_ringer:e.antall_ringer,omgangar:e.omgangar,kaster:e.kaster}))}function Te(e){let n=null,l=null,u=!1,d=new Set;async function f(e,{id:t,isAdmin:r=!1},i=null){l=i,u=r,n&&=(await T(n),null),e.replaceChildren(m(`Laster…`)),await h(e,t)}async function h(n,f){try{let[{data:t},{data:m},{data:_},{data:v},{count:b}]=await Promise.all([c(f),y(f),re(f),ae([`A`,`B`]),s(f)]);if(!t){n.replaceChildren(a(`Stevne ikkje funne.`));return}let x=_.filter(e=>e.kasterid!=null),S=m.filter(e=>e.fase===`innledende`),C=m.filter(e=>e.fase===`avsluttende`),w={},T={},k=new Map;for(let e of x)e.startnummer!=null&&(w[e.kasterid]=e.startnummer,k.set(e.startnummer,(k.get(e.startnummer)??0)+1)),e.posisjon!=null&&(T[e.kasterid]=e.posisjon);let j=[...k.values()].some(e=>e>1),M={};for(let e of m)for(let t of e.spelarar)t.kasterid&&t.kaster&&!M[t.kasterid]&&(M[t.kasterid]=`${t.kaster.fornavn} ${t.kaster.etternavn}`);for(let e of x)e.kaster&&!M[e.kasterid]&&(M[e.kasterid]=`${e.kaster.fornavn} ${e.kaster.etternavn}`);let N=Ce(S),P=oe(N,x,M,w,T),F=S.length>0&&S.every(e=>e.er_bekreftet),I=C.length>0,se=(S.length>0||I)&&S.every(e=>e.er_bekreftet)&&C.every(e=>e.er_bekreftet),L=x.some(e=>e.gruppe!=null),ce=Object.fromEntries(v.map(e=>[e.navn,e.id])),R=Se(t.runde1_format),z=b??0;if(t.kategori?.erlagbasert){let{data:e}=await o(f);z=e.length}let B={container:n,stevneid:f,stevne:t,standings:P,startNumberMap:w,positionMap:T,isTeam:j,nameMap:M,initialMatches:S,finalMatches:C,results:x,isAdmin:u,hasGroupAssignment:L,allInitialConfirmed:F,hasFinalMatches:I,round1Format:R,unitCount:z,groupNameMap:ce,reload:()=>h(n,f)};u&&l&&(l.innerHTML=E(t,{allMatchesConfirmed:se,hasFinalMatches:I,hasGroupAssignment:L,hasPreconfiguredFormat:R!=null&&t.stevne_fase!==`avsluttende`}));let V=ee(n);if(L){let t=O(P,N,w,{tableId:`standing-final`,hasGroups:!0,hasElimination:!0,positionMap:T,unitLabel:j?`par`:`spelarar`});n.innerHTML=A(e.renderMatchesHtml(B),t),ne(n,`standing-final`,d),te(n),V===`standing`&&D(n,`standing`),e.bindMatchEvents(n,B),g(n,f)}else n.innerHTML=e.renderSetupHtml(B);l?.querySelector(`#complete-tournament-btn`)?.addEventListener(`click`,async()=>{if(!await r({title:`Fullfør turnering`,message:`Vil du fullføre turneringa? Dette kan ikkje angrast.`,danger:!0}))return;let{error:e}=await ie(f,[...P.filter(e=>e.gruppe?.navn===`A`),...P.filter(e=>e.gruppe?.navn===`B`),...P.filter(e=>e.gruppe?.navn!==`A`&&e.gruppe?.navn!==`B`)]);if(e){i(`Feil ved lagring av plasseringar`,`error`);return}let{error:t}=await p(f);if(t){i(`Feil ved fullføring av turnering`,`error`);return}await h(n,f)}),e.bindHeaderEvents(l,B)}catch(e){t(`avsluttendeBase.loadAndRender`,e),n.replaceChildren(a(`Kunne ikkje laste avsluttande fase.`))}}function g(t,r){if(n)return;let i=F(r,[`avsluttende`],t,h,()=>{n&&=(T(n),null)});n=w(r,e.channelName(r),i)}return f}function Ee(e,t,n){return _(e.spelarar.filter(e=>e.kasterid!=null),t,n)}function Q(e){return e?.members.reduce((e,t)=>e+g(t),0)??0}var De=Te({channelName:e=>`stevne-avsl-cup-${e}`,renderMatchesHtml:e=>{let{finalMatches:t,standings:n,startNumberMap:r,positionMap:i,isTeam:a,isAdmin:o}=e;return`<div class="d-flex gap-3 flex-wrap">${[...new Set(n.map(e=>e.gruppe?.navn).filter(e=>e!=null))].sort().map(e=>{let s=t.filter(t=>t.gruppe_navn===e),c=n.filter(t=>t.gruppe?.navn===e),l=c.filter(e=>e.runde_eliminert==null).length,u=c.length,d=s.length?Math.max(...s.map(e=>e.runde_nummer)):0,f=s.filter(e=>e.runde_nummer===d),p=f.length>0&&f.every(e=>e.er_bekreftet||e.er_walkover),m=s.some(e=>e.runde_navn===`Semifinale`),h=o&&(s.length===0||p)&&l>1&&!m,g=f.length>0&&f.every(e=>e.runde_navn===`Semifinale`),_=s.some(e=>e.runde_navn===`Finale`);return Oe(e,s,l,u,d,h,o&&g&&p&&!_,r,i,a?`par`:`spelarar`,o)}).join(``)}</div>`},bindMatchEvents:(e,t)=>{!t.isAdmin&&t.finalMatches.length===0||Pe(e,t.stevneid,t.finalMatches,t.isAdmin,t.reload,t.startNumberMap,t.positionMap)},renderSetupHtml:e=>{let{stevne:t,isAdmin:n,round1Format:r,unitCount:i,standings:a}=e,o=r?.nA??null;if(t.stevne_fase===`avsluttende`)return n?W(a,{showPlayerList:!0,initNa:o,initFormat:r}):`<p class="text-muted fst-italic">Gruppefordeling er ikkje klar enno.</p>`;if(!n)return``;if(i<2)return`<p class="text-muted fst-italic">${t.kategori?.erlagbasert?`Minst 2 par må vere oppretta før gruppefordelinga kan setjast.`:`Minst 2 spelarar må vere påmelde før gruppefordelinga kan setjast.`}</p>`;let s=a.length>0;return W(s?a:i,{showPlayerList:s,initNa:o,initFormat:r})},bindHeaderEvents:(e,n)=>{let{container:a,stevneid:o,stevne:s,standings:c,results:d,round1Format:p,allInitialConfirmed:m,hasGroupAssignment:h,groupNameMap:g,reload:_}=n;if(e?.querySelector(`#start-final-btn`)?.addEventListener(`click`,async()=>{if(!m)return;let{error:e}=await f(o,`avsluttende`);if(e){i(`Feil ved oppstart av avsluttande fase`,`error`);return}if(p?.nA!=null){let e=p.nA,{error:t}=await P(o,$(c,d,e,g.A??null,g.B??null));if(t){i(`Feil ved lagring av gruppefordeling: `+u(t),`error`);return}}await _()}),!h){let e=parseInt(a.querySelector(`#group-assignment-wrapper`)?.dataset.n??`0`)||c.length;function t(e,t){let n=a.querySelector(`input[name="${e}"]:checked`);if(n?.dataset.oppsett)try{return JSON.parse(n.dataset.oppsett)}catch{}return V(t)[0]??null}function n(e,t,n){let r=a.querySelector(`#group-preview`);r&&(r.innerHTML=G(c.map((e,t)=>({...e,cupPlassering:t+1})),e,t?.walkovers??0,n?.walkovers??0))}let r=a.querySelector(`#group-panels`);r&&r.addEventListener(`change`,r=>{let i=r.target;if(!i.matches(`input[name^="round1-format"]`))return;let o=parseInt(a.querySelector(`input[name="group-split"]:checked`)?.value??String(e)),s=e-o,c=t(`round1-format-a`,o),l=t(`round1-format-b`,s);if(i.name===`round1-format-a`){let e=a.querySelector(`#structure-a`);e&&(e.outerHTML=K(o,c,`a`))}else{let e=a.querySelector(`#structure-b`);e&&(e.outerHTML=K(s,l,`b`))}n(o,c,l)}),a.querySelectorAll(`input[name="group-split"]`).forEach(t=>{t.addEventListener(`change`,()=>{let i=parseInt(t.value),a=e-i,o=V(i)[0]??null,s=a>=2?V(a)[0]??null:null;r&&(r.innerHTML=`<div id="group-panel-a" class="final-group-col">
                ${q(`Gruppe A`,i,`round1-format-a`,o)}
              </div>`+(a>=2?`<div id="group-panel-b" class="final-group-col">
                ${q(`Gruppe B`,a,`round1-format-b`,s)}
              </div>`:``)),n(i,o,s)})}),a.querySelector(`#confirm-group-btn`)?.addEventListener(`click`,async()=>{let n=a.querySelector(`input[name="group-split"]:checked`);if(!n)return;let r=parseInt(n.value),p=e-r,{error:m}=await l(o,{A:t(`round1-format-a`,r),B:p>=2?t(`round1-format-b`,p):null,nA:r});if(m){i(`Feil ved lagring av format: `+u(m),`error`);return}let h=s.stevne_fase;if(h!==`avsluttende`&&se(s.kastemetodeInnl?.navn??``)){let{data:e}=await I(o);if(!e){i(`Fullfør den innleiande fasen før cupen kan startast`,`error`);return}let{error:t}=await f(o,`avsluttende`);if(t){i(`Feil ved oppstart av avsluttande fase: `+u(t),`error`);return}h=`avsluttende`}if(h===`avsluttende`){let{error:e}=await P(o,$(c,d,r,g.A??null,g.B??null));if(e){i(`Feil ved lagring av gruppefordeling: `+u(e),`error`);return}}i(`Gruppefordeling lagra`,`success`),await _()})}e?.querySelector(`#edit-group-assignment-btn`)?.addEventListener(`click`,async()=>{await r({title:`Tilbakestill gruppeinndeling`,message:`Gruppefordeling og format vert fjerna.`,danger:!0})&&(await Promise.all([N(o),l(o,null)]),await _())}),h&&(a.querySelectorAll(`[data-generate-group]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.generateGroup??``,n=parseInt(e.dataset.runde??`1`);be(o,t,c.filter(e=>e.gruppe?.navn===t),n,p,_)})}),a.querySelectorAll(`[data-generate-finale-group]`).forEach(e=>{e.addEventListener(`click`,async()=>{let n=e.dataset.generateFinaleGroup??``;e.disabled=!0,e.textContent=`Genererer…`;try{await ye(o,n),await _()}catch(n){t(`cup:genererFinale`,n),i(`Feil ved generering av finale`,`error`),e.disabled=!1,e.textContent=`Generer finale`}})}))}});function $(e,t,n,r,i){return e.flatMap((e,a)=>{let o=a<n?r:i??r,s=e.startnummer==null?[]:t.filter(t=>t.startnummer===e.startnummer).map(e=>e.kasterid);return(s.length?s:[e.kasterid]).map(e=>({kasterid:e,gruppeid:o}))})}function Oe(e,t,n,r,i,a,o,s,c,l,u=!0){let f=new Map;for(let e of t)f.has(e.runde_nummer)||f.set(e.runde_nummer,[]),f.get(e.runde_nummer).push(e);let p=[...f.entries()].reverse().map(([e,t])=>{let n=t[0]?.runde_navn??`Runde ${e}`,r=t.filter(e=>!e.er_walkover);return r.length?`
      <h6 class="fw-bold text-center mb-1">${d(n)}</h6>
      <div class="d-flex flex-wrap gap-2 mb-2">
        ${r.map(e=>Ne(e,s,c,u)).join(``)}
    ...
    <td class="${u}"${d}>${a}</td>
        ...
        <td colspan="2">${k(t[0]??null,!1)} <span class="badge bg-secondary">Walkover</span></td>
          ...
          </tr>`}function Ne(e,t,r,i=!0){let a=Ee(e,t,r),o=e.er_bekreftet||e.er_walkover,s=e.spelarar.some(e=>(e.omgangar?.length??0)>0),c={isConfirmed:o,hasRounds:s,canEditScore:i&&e.er_bekreftet&&!e.er_tre_spelarar&&!s,isThreeSides:e.er_tre_spelarar},l=i?Me(e,o,je(e,a,s,o)):``;return`
    ...
    </div>`}function Pe(e,n,r,a,o,s,c){M(e);for(let l of r){let r=Ee(l,s,c),u=r[0]??null,d=r[1]??null,f=u?.rep??null,p=d?.rep??null,m=k(u,!1),g=k(d,!1),_=r.flatMap(e=>e.members.map(e=>e.id)),y=async(e,n)=>{let r=[];f?.id&&r.push(h(f.id,e)),p?.id&&r.push(h(p.id,n));for(let e of[u,d])for(let t of e?.members.slice(1)??[])r.push(h(t.id,0));try{return(await Promise.all(r)).find(e=>e.error)??null}catch(e){return t(`cup:writeSideScore`,e),{error:e}}};if(e.querySelector(`#plus-${l.id}`)?.addEventListener(`click`,()=>{ce({side1Name:m,side2Name:g,currentS1:Q(u),currentS2:Q(d),playerIds:_,hasRounds:l.spelarar.some(e=>(e.omgangar?.length??0)>0),logPrefix:`cup`,onSave:y,onSaved:o})}),e.querySelector(`#bekrft-${l.id}`)?.addEventListener(`click`,async e=>{if(l.er_tre_spelarar)xe(l,r,n,o);else{let t=e.currentTarget;t.disabled=!0,t.textContent=`Lagrer…`;try{await Fe(n,l,r,o)||(t.disabled=!1,t.textContent=`Bekreft`)}catch{t.disabled=!1,t.textContent=`Bekreft`}}}),a&&l.er_bekreftet&&!l.er_tre_spelarar){let t=r.flatMap(e=>e.members.map(e=>e.kasterid)),a=()=>{L([{name:m,score:Q(u)},{name:g,score:Q(d)}],async([e=0,r=0])=>{if(_.length){let{error:e}=await x(_);if(e){i(`DB-feil ved sletting av omgangar`,`error`);return}}if(await y(e,r)){i(`DB-feil ved oppdatering av score`,`error`);return}let a=e>=r?u:d,s=e>=r?d:u,c=a?.members.map(e=>e.kasterid)??[],f=s?.members.map(e=>e.kasterid)??[],p=[...c.map(e=>({kasterid:e,plassering:1})),...f.map(e=>({kasterid:e,plassering:2}))],{error:m}=await C(l.id,p);if(m){i(`DB-feil ved oppdatering av plassering`,`error`);return}await v({stevneId:n,roundNumber:l.runde_nummer,roundName:l.runde_navn,allThrowerIds:t,newWinnerIds:c,newLoserIds:f}),await o()})};e.querySelectorAll(`[data-endre-score="${l.id}"]`).forEach(e=>e.addEventListener(`click`,a))}}}async function Fe(e,t,n,a){let o=n[0]??null,s=n[1]??null,{data:c}=await b(t.id),l=e=>e?.members.reduce((e,t)=>e+g(c.find(e=>e.id===t.id)??t),0)??0,u=l(o),d=l(s);if(u===0&&d===0&&!await r({title:`Ingen score registrert`,message:`Vil du bekrefte kampen likevel?`}))return!1;let f=u>=d?o:s,p=u>=d?s:o,m=n.flatMap(e=>e.members.map(e=>e.kasterid)),{error:h}=await S({kampId:t.id,stevneId:e,roundNumber:t.runde_nummer,roundName:t.runde_navn,allThrowerIds:m,eliminatedIds:p?.members.map(e=>e.kasterid)??[],advancingSides:f?[f.members.map(e=>e.kasterid)]:[]});return h?(i(`DB-feil ved bekreft`,`error`),!1):(await a(),!0)}export{De as render};