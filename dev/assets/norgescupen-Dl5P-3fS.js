import{t as e}from"./logError-Cjb5zwtM.js";import{$ as t,ct as n,dt as r,ht as i,lt as a,ot as o,st as s,tt as c,ut as l}from"./index-D3K0Az14.js";import{t as u}from"./LoadingState-DU0ZcPlb.js";import{t as d}from"./EmptyState-DXltqcjg.js";import{t as f}from"./Table-DKAMtDIe.js";import{t as p}from"./expandableRows-BRVmd-Rj.js";var m=2007,h=2024,g={year:new Date().getFullYear(),cupType:`NC`,classNum:1,view:`singel`},_={year:null,rules:null,tournaments:[],results:[]};async function v(t){if(_.year===t)return!0;try{let[{data:e,error:r},{stevner:i,resultater:a,error:o}]=await Promise.all([s(t),n(t)]);return r||o?!1:(_.year=t,_.rules=e,_.tournaments=i,_.results=a,!0)}catch(t){return e(`fetchAndBufferData`,t),!1}}function y(e,t){return t===`SNC`?`Dei ${e.max_snc} beste SNC-stevna er teljande`:t===`DNC`?`Dei ${e.max_dnc} beste DNC-stevna er teljande`:`Dei ${e.maxtotal} beste stevna, herav maks ${e.max_nc_total} NC-stevner og ${e.max_snc_total} SNC-stevner er teljande`}function b(e){return`
    ...
    <div class="content-page">
      ...
      <div id="nc-content"></div>
    </div>`}async function E(e){if(g.year=new Date().getFullYear(),g.cupType=`NC`,g.classNum=1,g.view=`singel`,_={year:null,rules:null,tournaments:[],results:[]},e.replaceChildren(u(`Laster Norgescupen...`)),!await v(g.year)){e.replaceChildren(t(`Kunne ikkje laste data for Norgescupen.`));return}e.innerHTML=T(g.year,g.cupType);function n(){let{year:t,cupType:r,classNum:i,view:o}=g,{rules:s}=_,c=e.querySelector(`#nc-content`);if(e.querySelector(`.nc-main-title`).textContent=`Norgescupen ${t}`,e.querySelector(`#nc-cuptype`).classList.toggle(`d-none`,t<h),e.querySelector(`#nc-view-tabs-container`).innerHTML=r===`NC`?b(o):``,o===`lag`&&r===`NC`){c.innerHTML=`
        <section>
        ...
        </section>`;let e=c.querySelector(`#nc-team-table-container`);if(!s)e.replaceChildren(d(`Ingen data.`));else{let n=l(_.results,_.tournaments,s,t<2026);e.replaceChildren(w(n)),p(e,{triggerSel:`.nc-team-points-cell`,idAttr:`team-idx`,detailSel:`.nc-team-detail-row`,lookupRoot:c})}}else{c.innerHTML=`
        <section id="nc-single-section">
          <h2 class="nc-section-title">${r} Singel ${t}${t<=2025?` - Klasse ${i}`:``}</h2>
          <p class="nc-description">${s?y(s,r):`Ingen telleregel funnet for ${t}`}</p>
          <div id="nc-class-tabs-container">${x(i,t)}</div>
          <div id="nc-single-table-container"></div>
        </section>`;let e=c.querySelector(`#nc-single-table-container`);if(!s)e.replaceChildren(d(`Ingen data.`));else{let n=a(_.results,_.tournaments,s,r,i,t<2026);e.replaceChildren(C(n)),p(e,{triggerSel:`.nc-points-cell`,idAttr:`idx`,detailSel:`.nc-detail-row`,lookupRoot:c})}c.querySelector(`#nc-single-section`).addEventListener(`click`,e=>{let t=e.target.closest(`[data-class]`);t&&(g.classNum=Number(t.dataset.class),n())})}}n(),e.querySelector(`#nc-year`).addEventListener(`change`,async r=>{if(g.year=Number(r.target.value),g.classNum=1,g.year<h&&(g.cupType=`NC`,g.view=`singel`,e.querySelector(`#nc-cuptype`).value=`NC`),e.querySelector(`#nc-content`).replaceChildren(u()),!await v(g.year)){e.querySelector(`#nc-content`).replaceChildren(t(`Feil ved henting av data.`));return}n()}),e.querySelector(`#nc-cuptype`).addEventListener(`change`,e=>{g.cupType=e.target.value,g.classNum=1,g.cupType!==`NC`&&(g.view=`singel`),n()}),e.querySelector(`#nc-view-tabs-container`).addEventListener(`click`,e=>{let t=e.target.closest(`[data-view]`);t&&(g.view=t.dataset.view,n())})}export{E as render};