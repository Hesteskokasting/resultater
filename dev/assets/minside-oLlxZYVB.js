import{n as e,t}from"./logError-ByTg738k.js";import{Bt as n,C as r,D as i,E as a,F as o,L as s,Nt as c,Qn as l,S as u,Wn as d,X as f,Zn as p,b as m,dr as h,f as g,g as _,h as v,lr as y,mr as b,p as x,pn as S,r as C,s as w,t as T,ur as E,z as D}from"./index-BdFDMJa0.js";import{d as O,r as k}from"./kasterService-BN8H2rLx.js";import{t as A}from"./SearchSelect-DyA33beI.js";import{n as j,r as M,t as N}from"./accountService-DNYDQOti.js";import{c as P,d as ee}from"./kampService-CBwYcJbw.js";import{n as F,t as I}from"./groupBy-Bg_SEHjk.js";import{t as L}from"./navigationService-CIqZsl-n.js";import{n as R}from"./ScoreboardButton-cO_q_Bk1.js";function z(e,t,n){let r=(e?.spelarar??[]).filter(e=>e.kasterid!=null),i={};for(let t of r){let r=e?.stevneid==null?void 0:n[`${e.stevneid}:${t.kasterid}`];r!=null&&(i[t.kasterid]=r)}let a=d(r,i),o=a.findIndex(e=>e.members.some(e=>e.kasterid===t));return{mine:o===-1?[]:a[o]?.members??[],others:a.filter((e,t)=>t!==o).map(e=>e.members)}}function te(e){return!e?.length||e.every(e=>e.kaster==null)}function B(e,t){return p({rep:e[0],members:e},t)}function ne(e,t,n){let r=e?.er_bekreftet??!1,{mine:i,others:a}=z(e,t,n);if(e?.er_walkover)return{kind:`walkover`};if(r&&(e?.er_tre_spelarar||a.length>1)){let e=i.find(e=>e.kasterid===t)?.kamp_plassering;return e==null?{kind:`unknown`}:{kind:`placement`,placement:e}}let o=a[0];return!i.length||!o?.length?{kind:`unknown`}:{kind:`score`,me:B(i,r),them:B(o,r),confirmed:r}}function re(e){return`
    ...
    </div>`}function ae(e,t){let n=e.querySelector(`#pending-name`);!n||t==null||(async()=>{let{data:e}=await O(t);e&&(n.textContent=` for ${u(e)}`)})()}function V(e,t){let n=e.querySelector(`#thrower-error`),a=[],o=null;o=A({slot:e.querySelector(`#thrower-search-slot`),loadItems:async()=>{let{data:e}=await k();return a=e.map(e=>({id:e.id,label:r(e),sublabel:e.klubb?.navn??null})),a},placeholder:`Søk på navn…`,onSelect:e=>{e!=null&&(async()=>{n.classList.add(`d-none`);let r=a.find(t=>t.id===e),s=r?r.label+(r.sublabel?` (${r.sublabel})`:``):``;if(!await _({title:`Er dette deg?`,message:`Send koblingforespørsel for ${s}?`,confirmText:`Send forespørsel`})){o?.setValue(null);return}let{error:c}=await f(t,e);if(c){n.textContent=`Kunne ikkje sende forespørsel.`,n.classList.remove(`d-none`);return}i(),await C()})()}})}function H(e,t){return t.status===`godkjent`&&t.profil?.kasterid!=null?t.profil.kasterid:t.status===`venter`?(e.innerHTML=ie(),ae(e,t.profil?.kobling_kasterid??null),null):(e.innerHTML=re(t.status),V(e,t.user.id),null)}function U(e,t,n){let r=H(e,t);return r==null?null:(e.innerHTML=`
    <div class="card mb-4">
      <div class="card-body">
    ...
    </div>`).join(``)}</div>`}var oe={win:`Vunne`,loss:`Tapt`,draw:`Uavgjort`,neutral:``};function Y(e,t,n=oe[t]){return`<span class="result-badge result-badge--${t}"${n?` title="${g(n)}"`:``}>${e}</span>`}function se(e){return e==null?``:`<span class="match-grid__rings" title="Ringar">${e}</span>`}function ce(e){let t=e.map(e=>e.map(e=>g(u(e.kaster))).filter(Boolean).map(e=>`<span class="match-grid__name-line">${e}</span>`).join(``)).filter(Boolean);return t.length?t.map(e=>`<span class="match-grid__side">${e}</span>`).join(``):`–`}async function le(e){let[{data:t,error:r},i]=await Promise.all([P(e),n(e)]);if(r){let e=document.createElement(`p`);return e.className=`text-muted`,e.textContent=`Kunne ikkje laste kampar.`,e}let a=t,o=[...new Set(a.map(e=>e.kamp?.stevneid).filter(e=>e!=null))],s=await ee(o),c=a.filter(e=>e.kamp?.stevne?.erfullfort===!1).sort((e,t)=>K(e)-K(t)||(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),l=a.filter(e=>e.kamp?.stevne?.erfullfort===!0).sort((e,t)=>(t.kamp?.stevne?.dato??``).localeCompare(e.kamp?.stevne?.dato??``)||K(e)-K(t)||(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),d=e=>(e.kamp?.spelarar??[]).some(e=>(e.omgangar?.length??0)>0),f=t=>{let n=ne(t.kamp,e,s);switch(n.kind){case`walkover`:return Y(`21 – 0`,`win`,`Vunne på walkover`);case`placement`:return Y(`${n.placement}. plass`,n.placement>=3?`loss`:`win`,`Plassering i kampen: ${n.placement}`);case`score`:{let{me:e,them:t}=n;return n.confirmed?Y(`${e} – ${t}`,e>t?`win`:e<t?`loss`:`draw`):Y(`${e} – ${t}`,`neutral`,`Ikkje stadfesta`)}default:return Y(`–`,`neutral`)}},p=e=>{let t=e.kamp;return t?.er_walkover?``:t?.er_bekreftet??!1?d(e)?R(t?.id??``,L(),`scoreboard-btn--stats`):``:R(t?.id??``,L(),`scoreboard-btn--touch`)},m=t=>{let n=t.kamp,{others:r}=z(n,e,s),i=n?.er_walkover&&te(r[0]);return{slot:`<span class="match-grid__round">R${n?.runde_nummer??``} /</span> B${n?.bane_nummer??``}`,name:i?`<span class="match-grid__bye">Walkover</span>`:ce(r),result:f(t),stats:p(t)}},h=e=>J([`<span class="visually-hidden">Runde og bane</span>`,`Motstandar`,`Resultat`,``],e.map(e=>m(e))),_=e=>{let t=G.map(({key:t,label:n})=>({label:n,matches:e.filter(e=>e.kamp?.fase===t)})).filter(e=>e.matches.length),n=e.filter(e=>!G.some(t=>t.key===e.kamp?.fase));return[...t.map(({label:e,matches:t})=>`
      <p class="match-grid__phase">${g(q(t[0],e))}</p>
      ${h(t)}`),...n.length?[h(n)]:[]].join(``)},v=e=>e.length?[...I(e,e=>e.kamp?.stevneid??`unknown`).values()].map(e=>`
      <p class="match-grid__stevne">${g(e[0]?.kamp?.stevne?.navn??``)}</p>
      ${_(e)}`).join(``):null,y=v(c),b=v(l),x=i.data.filter(e=>e.fase===`innledende`),S=x.filter(e=>e.stevne?.erfullfort===!1).sort((e,t)=>(e.bane_nummer??0)-(t.bane_nummer??0)),C=x.filter(e=>e.stevne?.erfullfort===!0).sort((e,t)=>(t.stevne?.dato??``).localeCompare(e.stevne?.dato??``)||(e.bane_nummer??0)-(t.bane_nummer??0)),w=e=>`#/stevne/${e.stevneid}/innledende`,T=t=>{let n=t.deltakarar.find(t=>t.kasterid===e),r=t.deltakarar.filter(t=>t.kasterid!==e).map(e=>g(u(e.kaster))),i=t.er_bekreftet??!1;return{slot:`B${t.bane_nummer??``}`,name:r.length?r.join(` / `):`–`,result:i?Y(n?.poeng==null?`–`:String(n.poeng),`neutral`):`<a href="${w(t)}" class="btn btn-sm btn-primary">Opne bane</a>`,stats:i?se(n?.antall_ringer):``}},E=e=>e.length?[...I(e,e=>e.stevneid??`unknown`).values()].map(e=>`
      <p class="match-grid__stevne">${g(`${e[0]?.stevne?.navn??``} – X-kast`)}</p>
      ${J([`<span class="visually-hidden">Bane</span>`,`Medspelarar`,`Poeng`,`<span class="visually-hidden">Ringar</span>`],e.map(e=>T(e)))}`).join(``):null,D=E(S),O=E(C),k=[y,D].filter(Boolean).join(``),A=[b,O].filter(Boolean).join(``);return F({tabs:[{id:`active`,label:`Aktive (${c.length+S.length})`,panel:W(k||`<p class="text-muted">Ingen aktive kampar.</p>`)},{id:`completed`,label:`Ferdige (${l.length+C.length})`,panel:W(A||`<p class="text-muted">Ingen ferdige kampar enno.</p>`)}]})}async function ue(e,t){let n=U(e,t,`Kampar / X-kast`);if(!n)return;let r=await le(n.throwerId);n.slot.replaceChildren(r)}async function de(e){let{data:t,error:n}=await S(e),r=new Map;if(n){let e=document.createElement(`p`);return e.className=`text-muted`,e.textContent=`Kunne ikkje laste påmeldingar.`,{node:e,registeredMap:r}}let i=t.filter(e=>e.stevne?.erfullfort!==!0);if(!i.length)return{node:y(`Ingen påmeldingar enno.`),registeredMap:r};let a=[...i].sort((e,t)=>(e.stevne?.dato??``).localeCompare(t.stevne?.dato??``)),o=document.createElement(`div`);o.className=`stevne-card-list`;for(let e of a){let t=e.stevne,n=t?.id;n!=null&&r.set(n,e.id),t&&o.appendChild(w(t,{href:n==null?`#`:`#/stevne/${n}/info`,registrationSlotId:n??void 0}))}return{node:o,registeredMap:r}}async function fe(e,t){let n=U(e,t,`Påmeldingar`);if(!n)return;let{node:r,registeredMap:i}=await de(n.throwerId);n.slot.replaceChildren(r),x(e,n.throwerId,i)}async function pe(n){let{data:r,error:i}=await e.from(`bruker_profil`).select(`varsle_stevne_start, varsle_kamp_opprettet`).eq(`id`,n).maybeSingle();return i&&t(`getNotificationPreferences`,i),{data:r,error:i}}async function me(n,r,i){let a=r===`varsle_stevne_start`?{varsle_stevne_start:i}:{varsle_kamp_opprettet:i},{error:o}=await e.from(`bruker_profil`).update(a).eq(`id`,n);return o&&t(`updateNotificationPreference`,o),{error:o}}function he(e){return`
    ...
    <a href="#/kastere/${m(t)}" class="btn btn-sm btn-outline-primary mt-1">Vis profil</a>`}function ye(e){let t=e?`Bytt passord`:`Opprett passord`;return`
    ...
    <table class="table table-sm align-middle linked-accounts-table">
      ...
      <td class="linked-accounts-table__date">${g(l(e.opprettet_at))}</td>
      <td class="text-end linked-accounts-table__actions">${n}</td>
    </tr>`}).join(``)}</tbody>
    </table>`}function xe(e){e.querySelector(`#password-form`).addEventListener(`submit`,async n=>{n.preventDefault();let r=n.target,i=e.querySelector(`#ko-password-error`);i.classList.add(`d-none`);let a=e.querySelector(`#ko-password`).value;if(a!==e.querySelector(`#ko-password2`).value){i.textContent=`Passorda er ikkje like.`,i.classList.remove(`d-none`);return}let o=r.querySelector(`[type=submit]`);o.disabled=!0;let{error:l}=await s(a);if(o.disabled=!1,l){t(`minsideKonto.updatePassword`,l),i.textContent=`Kunne ikkje endre passord: ${c(l)}`,i.classList.remove(`d-none`);return}v(`Passordet er endra.`,`success`),r.reset()})}function Se(e){e.querySelector(`[data-slot="accounts"]`).addEventListener(`click`,async e=>{let t=e.target.closest(`[data-login-email]`);t&&(await o(),location.hash=`#/logginn?email=${encodeURIComponent(t.dataset.loginEmail)}`)})}function Z(e,t){e.querySelector(`#delete-own-account`).addEventListener(`click`,async()=>{if(!await _({title:`Slette kontoen?`,message:`Innloggingskontoen ${t.user.email??``} vert sletta permanent. ${X}`,confirmText:`Slett konto`,danger:!0}))return;let{error:e}=await N(t.user.id);if(e){v(`Kunne ikkje slette kontoen: ${c(e)}`,`error`);return}try{await o()}catch{}location.hash=`#/logginn`})}async function Ce(e,n){let r=n.status===`godkjent`?n.profil?.kasterid??null:null,i=r!=null,a=n.user.identities?.some(e=>e.provider===`email`)??!0;if(e.innerHTML=`
    ...
    <div class="card mb-4"><div class="card-body">
      ...
      <li class="nav-item">
        <a class="nav-link${e===t?` active`:``}"
           href="#/minside/${t}">${n}</a>
      </li>`).join(``)}</ul>`}async function $(e,n){T(()=>$(e,n));let r=String(n.tab??`kampar`);e.replaceChildren(h(`Laster min side…`));try{let t=await a();if(!t){location.hash=`#/logginn`;return}let{profil:n,user:i}=t,o=n?.kobling_status??`ingen`,s=r===`varslingar`?`innstillingar`:r,c=we.has(s)?s:`kampar`;e.innerHTML=`
      ...
      </div>`,e.querySelector(`[data-slot="logout"]`).appendChild(M());let l=e.querySelector(`#minside-subpage`);await Te[c](l,{user:i,profil:n,status:o})}catch(n){t(`minside.render`,n),e.replaceChildren(E(`Kunne ikkje laste min side.`))}}export{$ as render};