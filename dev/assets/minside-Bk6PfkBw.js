import{n as e,t}from"./logError-ByTg738k.js";import{$n as n,C as r,D as i,E as a,Ft as o,Ht as s,Kn as c,L as l,Q as u,S as d,V as f,b as p,dr as m,er as h,f as g,fr as _,g as v,gr as y,h as b,hn as x,p as S,pr as C,r as w,s as T,t as E,z as D}from"./index-CkbUM2Oe.js";import{d as O,r as k}from"./kasterService-BN8H2rLx.js";import{t as A}from"./SearchSelect-Ytjszrcf.js";import{n as j,r as M,t as N}from"./accountService-DX3JYBsX.js";import{c as P,d as ee}from"./kampService-C7a37Acj.js";import{n as F,t as I}from"./groupBy-Bg_SEHjk.js";import{t as L}from"./navigationService-B2zOONcH.js";import{n as R}from"./ScoreboardButton-cO_q_Bk1.js";function z(e,t,n){let r=(e?.spelarar??[]).filter(e=>e.kasterid!=null),i={};for(let t of r){let r=e?.stevneid==null?void 0:n[`${e.stevneid}:${t.kasterid}`];r!=null&&(i[t.kasterid]=r)}let a=c(r,i),o=a.findIndex(e=>e.members.some(e=>e.kasterid===t));return{mine:o===-1?[]:a[o]?.members??[],others:a.filter((e,t)=>t!==o).map(e=>e.members)}}function te(e){return!e?.length||e.every(e=>e.kaster==null)}function B(e,t){return n({rep:e[0],members:e},t)}function ne(e,t,n){let r=e?.er_bekreftet??!1,{mine:i,others:a}=z(e,t,n);if(e?.er_walkover)return{kind:`walkover`};if(r&&(e?.er_tre_spelarar||a.length>1)){let e=i.find(e=>e.kasterid===t)?.kamp_plassering;return e==null?{kind:`unknown`}:{kind:`placement`,placement:e}}let o=a[0];return!i.length||!o?.length?{kind:`unknown`}:{kind:`score`,me:B(i,r),them:B(o,r),confirmed:r}}function re(e){return`
    ...
    </div>`).join(``)}</div>`}var oe={win:`Vunne`,loss:`Tapt`,draw:`Uavgjort`,neutral:``};function Y(e,t,n=oe[t]){return`<span class="result-badge result-badge--${t}"${n?` title="${g(n)}"`:``}>${e}</span>`}function se(e){return e==null?``:`<span class="match-grid__rings" title="Ringar">${e}</span>`}function ce(e){let t=e.map(e=>e.map(e=>g(d(e.kaster))).filter(Boolean).map(e=>`<span class="match-grid__name-line">${e}</span>`).join(``)).filter(Boolean);return t.length?t.map(e=>`<span class="match-grid__side">${e}</span>`).join(``):`–`}async function le(e){let[{data:t,error:n},r]=await Promise.all([P(e),s(e)]);if(n){let e=document.createElement(`p`);return e.className=`text-muted`,e.textContent=`Kunne ikkje laste kampar.`,e}let i=t,a=[...new Set(i.map(e=>e.kamp?.stevneid).filter(e=>e!=null))],o=await ee(a),c=i.filter(e=>e.kamp?.stevne?.erfullfort===!1).sort((e,t)=>K(e)-K(t)||(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),l=i.filter(e=>e.kamp?.stevne?.erfullfort===!0).sort((e,t)=>(t.kamp?.stevne?.dato??``).localeCompare(e.kamp?.stevne?.dato??``)||K(e)-K(t)||(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),u=e=>(e.kamp?.spelarar??[]).some(e=>(e.omgangar?.length??0)>0),f=t=>{let n=ne(t.kamp,e,o);switch(n.kind){case`walkover`:return Y(`21 – 0`,`win`,`Vunne på walkover`);case`placement`:return Y(`${n.placement}. plass`,n.placement>=3?`loss`:`win`,`Plassering i kampen: ${n.placement}`);case`score`:{let{me:e,them:t}=n;return n.confirmed?Y(`${e} – ${t}`,e>t?`win`:e<t?`loss`:`draw`):Y(`${e} – ${t}`,`neutral`,`Ikkje stadfesta`)}default:return Y(`–`,`neutral`)}},p=e=>{let t=e.kamp;return t?.er_walkover?``:t?.er_bekreftet??!1?u(e)?R(t?.id??``,L(),`scoreboard-btn--stats`):``:R(t?.id??``,L(),`scoreboard-btn--touch`)},m=t=>{let n=t.kamp,{others:r}=z(n,e,o),i=n?.er_walkover&&te(r[0]);return{slot:`<span class="match-grid__round">R${n?.runde_nummer??``} /</span> B${n?.bane_nummer??``}`,name:i?`<span class="match-grid__bye">Walkover</span>`:ce(r),result:f(t),stats:p(t)}},h=e=>J([`<span class="visually-hidden">Runde og bane</span>`,`Motstandar`,`Resultat`,``],e.map(e=>m(e))),_=e=>{let t=G.map(({key:t,label:n})=>({label:n,matches:e.filter(e=>e.kamp?.fase===t)})).filter(e=>e.matches.length),n=e.filter(e=>!G.some(t=>t.key===e.kamp?.fase));return[...t.map(({label:e,matches:t})=>`
    ...
    <p class="mb-1"><strong>${g(d(t))}</strong> · ${g(t.klubb?.navn??``)}</p>
    <a href="#/kastere/${p(t)}" class="btn btn-sm btn-outline-primary mt-1">Vis profil</a>`}function ye(e){let t=e?`Bytt passord`:`Opprett passord`;return`
    ...
    <table class="table table-sm align-middle linked-accounts-table">
      ...
      <td class="linked-accounts-table__date">${g(h(e.opprettet_at))}</td>
      <td class="text-end linked-accounts-table__actions">${n}</td>
    </tr>`}).join(``)}</tbody>
    </table>`}function xe(e){e.querySelector(`#password-form`).addEventListener(`submit`,async n=>{n.preventDefault();let r=n.target,i=e.querySelector(`#ko-password-error`);i.classList.add(`d-none`);let a=e.querySelector(`#ko-password`).value;if(a!==e.querySelector(`#ko-password2`).value){i.textContent=`Passorda er ikkje like.`,i.classList.remove(`d-none`);return}let s=r.querySelector(`[type=submit]`);s.disabled=!0;let{error:c}=await D(a);if(s.disabled=!1,c){t(`minsideKonto.updatePassword`,c),i.textContent=`Kunne ikkje endre passord: ${o(c)}`,i.classList.remove(`d-none`);return}b(`Passordet er endra.`,`success`),r.reset()})}function Se(e){e.querySelector(`[data-slot="accounts"]`).addEventListener(`click`,async e=>{let t=e.target.closest(`[data-login-email]`);t&&(await l(),location.hash=`#/logginn?email=${encodeURIComponent(t.dataset.loginEmail)}`)})}function Z(e,t){e.querySelector(`#delete-own-account`).addEventListener(`click`,async()=>{if(!await v({title:`Slette kontoen?`,message:`Innloggingskontoen ${t.user.email??``} vert sletta permanent. ${X}`,confirmText:`Slett konto`,danger:!0}))return;let{error:e}=await N(t.user.id);if(e){b(`Kunne ikkje slette kontoen: ${o(e)}`,`error`);return}try{await l()}catch{}location.hash=`#/logginn`})}async function Ce(e,n){let r=n.status===`godkjent`?n.profil?.kasterid??null:null,i=r!=null,a=n.user.identities?.some(e=>e.provider===`email`)??!0;if(e.innerHTML=`
    ...
    <div class="card mb-4"><div class="card-body">
      ...
      <li class="nav-item">
        <a class="nav-link${e===t?` active`:``}"
           href="#/minside/${t}">${n}</a>
      </li>`).join(``)}</ul>`}async function $(e,n){E(()=>$(e,n));let r=String(n.tab??`kampar`);e.replaceChildren(C(`Laster min side…`));try{let t=await a();if(!t){location.hash=`#/logginn`;return}let{profil:n,user:i}=t,o=n?.kobling_status??`ingen`,s=r===`varslingar`?`innstillingar`:r,c=we.has(s)?s:`kampar`;e.innerHTML=`
      ...
      </div>`,e.querySelector(`[data-slot="logout"]`).appendChild(M());let l=e.querySelector(`#minside-subpage`);await Te[c](l,{user:i,profil:n,status:o})}catch(n){t(`minside.render`,n),e.replaceChildren(_(`Kunne ikkje laste min side.`))}}export{$ as render};