...
import{t as e}from"./logError-CB4-2Lin.js";import{Jt as t,Lt as n,Wt as r,c as i,i as a,zt as o}from"./index-fkSDkKQN.js";import{t as s}from"./LoadingState-C6NB62Ct.js";import{i as c,l,r as u}from"./kasterService-D9jqvobU.js";import{t as d}from"./EmptyState-CCNgsnix.js";import{n as f,r as p}from"./kaster-2cwCS5i9.js";import{t as m}from"./SearchInput-BwD50MFz.js";import{t as h}from"./AdminLinkBar-C4OSvRLS.js";var g=24,_=`https://placehold.co/200x200/444/888?text=?`,v={showAll:!1,searchText:``,page:1};function y(e){let t=p(e);return`
    ...
    </div>`}function R(e,t,n){return t==null?``:`<span class="kd-round-chip">${e} ${n==null?`${t}`:`${t} (${n})`}</span>`}function z(e,t){return`<span class="kd-sort-icon${e?` kd-sort-icon--active`:``}" aria-hidden="true">${e?t===`asc`?`↑`:`↓`:`↕`}</span>`}function B(e,t,n){let r=n.column===e,i=n.direction===`asc`?`stigande`:`synkande`,a=r?`Sortert etter ${t}, ${i}. Vel for å snu.`:`Sorter etter ${t}`;return`<button type="button" class="kd-sort-btn${r?` kd-sort-btn--active`:``}" data-sort="${e}" aria-pressed="${r}" aria-label="${a}">${t}${z(r,n.direction)}</button>`}function V(e,t,n,r){let a=e;t!==`alle`&&(a=a.filter(e=>String(T(e.stevne?.dato))===t)),n!==`alle`&&(a=a.filter(e=>String(e.stevne?.stevnetype?.id)===n));let s=a.length,c=`
    ...
    <div class="kd-res-head">
      <div class="kd-res-head__date">Sortér: ${B(`dato`,`Dato`,r)}</div>
      ...
      <div class="kd-res-head__pl">${B(`plassering`,`Pl.`,r)}</div>
    </div>`,u=D(a,r).map(e=>{let t=e.stevne,n=t?.id?`<a href="#/stevne/${t.id}/resultat" class="kd-res-row__name">${i(t.navn??``)}</a>`:`<span class="kd-res-row__name">${i(t?.navn??`–`)}</span>`,r=R(`X-kast`,e.poeng_xkast,e.antall_ring_xkast)+R(`Kongelag`,e.poeng_kongelag,e.antall_ring_kongelag),a=e.plassering==null?`<span class="kd-res-row__pl kd-res-row__pl--empty">–</span>`:`<span class="kd-res-row__pl">${e.plassering}</span>`;return`
      <div class="kd-res-row">
        ${n}
        <div class="kd-res-row__meta">
          <span class="kd-res-row__date">${o(t?.dato)}</span>
          <span class="kd-res-row__type">${i(t?.stevnetype?.navn??`–`)}</span>
          <span class="kd-res-row__klubb">${i(e.klubb?.navn??`–`)}</span>
        </div>
    ...
    </div>`}async function U(e,n){I();let{labels:r,stevneNamn:i,verdiar:a}=M(n,P.chartMetric,P.chartMethod,P.chartFrom?Number(P.chartFrom):null,P.chartTo?Number(P.chartTo):null);if(!a.length){let t=e.parentElement;if(t){let e=d(`Ingen data for valt filter.`);e.classList.add(`pt-3`),t.replaceChildren(e)}return}let{Chart:o,registerables:s}=await t(async()=>{let{Chart:e,registerables:t}=await import(`./charts-qQqpzAio.js`).then(e=>e.t);return{Chart:e,registerables:t}},__vite__mapDeps([0,1]));N||=(o.register(...s),!0);let c=P.chartMetric===`plassering`,l=c?`Plassering`:`% Ring`;F=new o(e,{type:`line`,data:{labels:r,datasets:[{label:l,data:a,borderColor:`#4e8fc7`,backgroundColor:`rgba(78,143,199,0.15)`,pointBackgroundColor:`#4e8fc7`,pointRadius:4,tension:.1,fill:!1}]},options:{responsive:!0,maintainAspectRatio:!1,scales:{x:{ticks:{maxTicksLimit:14,maxRotation:45,color:`#ccc`},grid:{color:`rgba(255,255,255,0.08)`}},y:{reverse:c,ticks:{color:`#ccc`},grid:{color:`rgba(255,255,255,0.08)`},title:{display:!0,text:l,color:`#ccc`}}},plugins:{legend:{display:!1},tooltip:{callbacks:{title:e=>{let t=e[0]?.dataIndex;return t==null?``:i[t]??r[t]??``},label:e=>`${l}: ${String(e.raw)}`}}}}})}async function W(t,r){P.active=`resultater`,P.year=`alle`,P.tournamentType=`alle`,P.resultSort={column:`dato`,direction:`desc`},P.chartMetric=`plassering`,P.chartMethod=`kongelag`,P.chartFrom=null,P.chartTo=null,I(),t.replaceChildren(s(`Laster utøvar...`));try{let{kaster:e,resultater:i,error:o}=await l(r);if(o||!e){t.replaceChildren(n(`Kunne ikkje laste utøvar.`));return}let s=e;a(p(s)),t.innerHTML=L(s,i);let c=t.querySelector(`#kd-year`),u=t.querySelector(`#kd-type`),d=t.querySelector(`#kd-chart-method`),f=t.querySelector(`#kd-result-table`);function m(){f.innerHTML=V(i,P.year,P.tournamentType,P.resultSort)}f.addEventListener(`click`,e=>{let t=e.target.closest(`[data-sort]`);if(!t)return;let n=t.dataset.sort;P.resultSort=P.resultSort.column===n?{column:n,direction:P.resultSort.direction===`asc`?`desc`:`asc`}:{column:n,direction:n===`plassering`?`asc`:`desc`},m()});function g(){t.querySelector(`#kd-statistics-content`).innerHTML=H(i,s)}function _(){let e=t.querySelector(`#kd-chart-canvas`);e&&U(e,i)}function v(e){P.active=e,t.querySelectorAll(`.thrower-tab-button`).forEach(t=>{t.classList.toggle(`active`,t.dataset.tab===e)}),t.querySelectorAll(`.kd-tab`).forEach(t=>{t.classList.toggle(`kd-hidden`,t.id!==`kd-tab-${e}`)}),e===`statistikk`&&g(),e===`graf`&&_()}m(),c.addEventListener(`change`,()=>{P.year=c.value,m()}),u.addEventListener(`change`,()=>{P.tournamentType=u.value,m()}),t.querySelectorAll(`.thrower-tab-button`).forEach(e=>{e.addEventListener(`click`,()=>v(e.dataset.tab??``))});let y=t.querySelector(`#kd-chart-metric`);y.addEventListener(`change`,()=>{P.chartMetric=y.value,d.classList.toggle(`d-none`,y.value!==`prosent`),_()}),d.addEventListener(`change`,()=>{P.chartMethod=d.value,_()});let b=t.querySelector(`#kd-chart-from`),x=t.querySelector(`#kd-chart-to`);b.addEventListener(`change`,()=>{P.chartFrom=b.value||null,_()}),x.addEventListener(`change`,()=>{P.chartTo=x.value||null,_()}),h(t,{href:`#/kaster/${r}/admin`,label:`Rediger utøvar`,variant:`warning`,canShow:e=>e.profil?.role===`admin`||e.profil?.role===`klubbadmin`&&e.clubs.includes(s.klubbid??-1)})}catch(r){e(`renderDetail`,r),t.replaceChildren(n(`Kunne ikkje laste utøvar.`))}}var G=async(e,t)=>{I(),t.id?await W(e,Number(t.id)):await S(e)};export{G as render};