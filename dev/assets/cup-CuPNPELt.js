  ...
  `}function W(e,t,n=0,i=0){let a=e.slice(0,t),o=e.slice(t);function s(e,t=0){return e.map((e,n)=>{let i=n<t;return`
    ...
    <div class="card">
      ...
      <span class="small text-muted text-nowrap">${e.kamp_poeng??0}p (${e.score_poeng??0})</span>
        ...
        </div>`:``,ee=i===!0&&m>0?`<div class="d-flex gap-3 flex-wrap mb-3">
    ...
    <td class="${u}"${d}>${a}</td>
        ...
        <td colspan="2">${E(t[0]??null,!1)} <span class="badge bg-secondary">Walkover</span></td>
          ...
          </tr>`}function Oe(e,t,n,r=!0){let a=Q(e,t,n),o=e.er_bekreftet||e.er_walkover,s=e.spelarar.some(e=>(e.omgangar?.length??0)>0),c={isConfirmed:o,hasRounds:s,canEditScore:r&&e.er_bekreftet&&!e.er_tre_spelarar&&!s,isThreeSides:e.er_tre_spelarar},l=r?De(e,o,Ee(e,a,s,o)):``;return`
    ...
    </div>`}function ke(e,n,r,i,a,o,s){N(e);for(let c of r){let r=Q(c,o,s),l=r[0]??null,u=r[1]??null,d=l?.rep??null,p=u?.rep??null,m=E(l,!1),g=E(u,!1),_=r.flatMap(e=>e.members.map(e=>e.id)),y=async(e,n)=>{let r=[];d?.id&&r.push(h(d.id,e)),p?.id&&r.push(h(p.id,n));for(let e of[l,u])for(let t of e?.members.slice(1)??[])r.push(h(t.id,0));try{return(await Promise.all(r)).find(e=>e.error)??null}catch(e){return t(`cup:writeSideScore`,e),{error:e}}};if(e.querySelector(`#plus-${c.id}`)?.addEventListener(`click`,()=>{F({side1Name:m,side2Name:g,currentS1:$(l),currentS2:$(u),playerIds:_,hasRounds:c.spelarar.some(e=>(e.omgangar?.length??0)>0),logPrefix:`cup`,onSave:y,onSaved:a})}),e.querySelector(`#bekrft-${c.id}`)?.addEventListener(`click`,async e=>{if(c.er_tre_spelarar)_e(c,r,n,a);else{let t=e.currentTarget;t.disabled=!0,t.textContent=`Lagrer…`;try{await Ae(n,c,r,a)||(t.disabled=!1,t.textContent=`Bekreft`)}catch{t.disabled=!1,t.textContent=`Bekreft`}}}),i&&c.er_bekreftet&&!c.er_tre_spelarar){let t=r.flatMap(e=>e.members.map(e=>e.kasterid)),i=()=>{P([{name:m,score:$(l)},{name:g,score:$(u)}],async([e=0,r=0])=>{if(_.length){let{error:e}=await x(_);if(e){f(`DB-feil ved sletting av omgangar`,`error`);return}}if(await y(e,r)){f(`DB-feil ved oppdatering av score`,`error`);return}let i=e>=r?l:u,o=e>=r?u:l,s=i?.members.map(e=>e.kasterid)??[],d=o?.members.map(e=>e.kasterid)??[],p=[...s.map(e=>({kasterid:e,plassering:1})),...d.map(e=>({kasterid:e,plassering:2}))],{error:m}=await C(c.id,p);if(m){f(`DB-feil ved oppdatering av plassering`,`error`);return}await v({stevneId:n,roundNumber:c.runde_nummer,roundName:c.runde_navn,allThrowerIds:t,newWinnerIds:s,newLoserIds:d}),await a()})};e.querySelectorAll(`[data-endre-score="${c.id}"]`).forEach(e=>e.addEventListener(`click`,i))}}}async function Ae(e,t,n,r){let i=n[0]??null,o=n[1]??null,{data:s}=await b(t.id),c=e=>e?.members.reduce((e,t)=>e+g(s.find(e=>e.id===t.id)??t),0)??0,l=c(i),u=c(o);if(l===0&&u===0&&!await a({title:`Ingen score registrert`,message:`Vil du bekrefte kampen likevel?`}))return!1;let d=l>=u?i:o,p=l>=u?o:i,m=n.flatMap(e=>e.members.map(e=>e.kasterid)),{error:h}=await S({kampId:t.id,stevneId:e,roundNumber:t.runde_nummer,roundName:t.runde_navn,allThrowerIds:m,eliminatedIds:p?.members.map(e=>e.kasterid)??[],advancingSides:d?[d.members.map(e=>e.kasterid)]:[]});return h?(f(`DB-feil ved bekreft`,`error`),!1):(await r(),!0)}export{xe as render};