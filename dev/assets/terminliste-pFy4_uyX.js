import{t as e}from"./logError-Cjb5zwtM.js";import{r as t}from"./authService-9qLZxJYd.js";import{$ as n,D as r,I as i,Q as a,R as o,et as s,nt as c,ot as l,r as u,t as d}from"./index-0-lbj3eN.js";import{t as f}from"./LoadingState-DU0ZcPlb.js";import{t as p}from"./buildDropdownOptions-lgetK572.js";var m={column:`dato`,direction:`asc`};function h(e,t){switch(t){case`navn`:return e.navn??``;case`dato`:return e.dato??``;case`sted`:return e.sted??``;case`metode`:return[e.innledende?.navn,e.avsluttende?.navn].filter(e=>!!e).join(` `);case`organizer`:return e.klubb?.navn??``;case`type`:return e.stevnetype?.navn??``;case`klassifisering`:return e.kategori?.navn??``;default:return``}}function g(e){return[...e].sort((e,t)=>{let n=h(e,m.column),r=h(t,m.column),i=n.localeCompare(r,`nb`);return m.direction===`asc`?i:-i})}var _={year:new Date().getFullYear(),text:``,tournamentTypeId:``,throwingMethodId:``,clubId:``,categoryId:``},v=[],y=null,b=new Map;function x(e){return e.filter(e=>{if(_.text){let t=_.text.toLowerCase();if(![e.navn,e.sted,e.klubb?.navn,e.stevnetype?.navn,e.kategori?.navn,e.innledende?.navn,e.avsluttende?.navn].some(e=>e?.toLowerCase().includes(t)))return!1}if(_.tournamentTypeId&&String(e.stevnetype?.id)!==_.tournamentTypeId)return!1;if(_.throwingMethodId){let t=_.throwingMethodId;if(!(String(e.innledende?.id)===t||String(e.avsluttende?.id)===t))return!1}return!(_.clubId&&String(e.klubb?.id)!==_.clubId||_.categoryId&&String(e.kategori?.id)!==_.categoryId)})}async function S(e){await s(e.map(e=>({Dato:e.dato?new Date(e.dato).toLocaleDateString(`nb-NO`):``,Navn:e.navn??``,Sted:e.sted??``,Arrangør:e.klubb?.navn??``,Stevnetype:e.stevnetype?.navn??``,"Kastemetode (innledende)":e.innledende?.navn??``,"Kastemetode (avsluttende)":e.avsluttende?.navn??``,Kategori:e.kategori?.navn??``,NM:e.ernm?`Ja`:`Nei`,InnbydelseUrl:e.innbydelseurl??``})),`terminliste-${_.year}.xlsx`,`Terminliste`)}var C=[{id:`navn`,label:`Stevne`},{id:`dato`,label:`Dato`},{id:`sted`,label:`Sted`},{id:`metode`,label:`Metode`},{id:`organizer`,label:`Arrangør`},{id:`type`,label:`Type`},{id:`klassifisering`,label:`Klassifisering`}];function w(e){return m.column===e?m.direction===`asc`?`<span class="tl-sort-icon active">↑</span>`:`<span class="tl-sort-icon active">↓</span>`:`<span class="tl-sort-icon">↕</span>`}function T(e){let t=e.dato?new Date(e.dato+`T12:00:00`).toLocaleDateString(`nb-NO`):``,n=[e.innledende?.navn,e.avsluttende?.navn].filter(e=>!!e).join(` \\ `),r=e.ernm?`<span class="tl-nm-merke">NM</span> `:``,i=e.innbydelseurl?`<a href="${a(e.innbydelseurl)}" target="_blank" rel="noopener" class="tl-invitation-icon" title="Innbydelse">📄</a>`:``;return`<tr class="tl-tr">
  ...
  </tr></thead>`}${`<tbody>${g(e).map(T).join(``)}</tbody>`}</table>`}function D(e){return window.innerWidth>600?E(e):k(e)}function O(e){let t=c(e.dato),n=e.sted?`<p class="tl-detail">Sted: ${a(e.sted)}</p>`:``,r=e.klubb?`<p class="tl-detail">Arrangør: ${a(e.klubb.navn??``)}</p>`:``,i=e.stevnetype?`<p class="tl-detail">Type: ${a(e.stevnetype.navn??``)}</p>`:``,o=e.ernm?`<span class="tl-nm-merke">NM</span>`:``,s=e.innbydelseurl?`<a class="tl-invitation-link" href="${a(e.innbydelseurl)}" target="_blank" rel="noopener">Innbydelse 📄</a>`:``,l=e.resultaturl?`<a class="tournament-link" href="#/stevne/${e.id}/resultat">Vis resultat</a>`:``,u=e.dato&&new Date(e.dato+`T12:00:00`)>new Date,d=e.stevne_fase===null||e.stevne_fase===`ikke_startet`,f=y?.profil?.kobling_status===`godkjent`&&u&&d&&!e.erfullfort?`<span data-registration-slot="${e.id}"></span>`:``;return`
  ...
  `}function k(e){return e.length===0?`<p class="empty-state">Ingen stevner funnet med valgte filtre.</p>`:`<div class="tournament-list">${e.map(O).join(``)}</div>`}async function A(s){d(()=>A(s)),s.replaceChildren(f(`Laster terminliste…`));try{let[{data:c,error:d},{data:h},g]=await Promise.all([o(_.year),r(),t()]);if(y=g,b=g?.profil?.kasterid==null?new Map:await i(g.profil.kasterid),d){e(`terminliste.render`,d),s.replaceChildren(n(`Kunne ikkje laste terminliste.`));return}v=c??[];function C(e){return{year:`<select class="tl-select" id="tl-year${e}">${l(_.year,1983,new Date().getFullYear()+1)}</select>`,tournamentType:`<select class="tl-select" id="tl-tournamenttype${e}">${p(h.stevnetyper,_.tournamentTypeId,`Alle typer`)}</select>`,throwingMethod:`<select class="tl-select" id="tl-throwingmethod${e}">${p(h.kastemetoder,_.throwingMethodId,`Alle metoder`)}</select>`,organizer:`<select class="tl-select" id="tl-organizer${e}">${p(h.klubber,_.clubId,`Alle arrangører`)}</select>`,category:`<select class="tl-select" id="tl-category${e}">${p(h.kategorier,_.categoryId,`Alle kategorier`)}</select>`}}let w=C(``),T=C(`-mobil`);s.innerHTML=`
    ...
    `;function E(){let e=x(v),t=s.querySelector(`.tl-list-container`);if(!t)return e;t.innerHTML=D(e);let n=s.querySelector(`.tl-count`);n&&(n.textContent=`${e.length} stevner`);let r=y?.profil?.kasterid,i=y?.user.id;return r!=null&&i&&u(t,r,i,b),e}if(E(),g?.profil&&(g.profil.role===`admin`||g.profil.role===`klubbadmin`)){let e=document.createElement(`div`);e.className=`mb-3 px-2 d-flex gap-2`,e.innerHTML=`<a href="#/stevne/ny" class="btn btn-sm btn-success">+ Nytt stevne</a>`,s.querySelector(`.terminliste`)?.prepend(e)}let O=s.querySelector(`.tl-list-container`),k=s.querySelector(`#tl-year`),A=s.querySelector(`#tl-text`),j=s.querySelector(`#tl-text-mobile`),M=s.querySelector(`#tl-tournamenttype`),N=s.querySelector(`#tl-throwingmethod`),P=s.querySelector(`#tl-organizer`),F=s.querySelector(`#tl-category`),I=s.querySelector(`#tl-excel-desktop`),L=s.querySelector(`#tl-excel-mobile`),R=s.querySelector(`#tl-filter-open`),z=s.querySelector(`#tl-sheet`),B=s.querySelector(`#tl-backdrop`),V=s.querySelector(`#tl-reset`),H=s.querySelector(`#tl-apply`),U=s.querySelector(`#tl-year-mobil`),W=s.querySelector(`#tl-tournamenttype-mobil`),G=s.querySelector(`#tl-throwingmethod-mobil`),K=s.querySelector(`#tl-organizer-mobil`),q=s.querySelector(`#tl-category-mobil`);O.addEventListener(`click`,e=>{let t=e.target.closest(`[data-column]`);if(!t)return;let n=t.dataset.column;m.column===n?m.direction=m.direction===`asc`?`desc`:`asc`:(m.column=n,m.direction=`asc`),E()});let J=null;function Y(){if(!s.querySelector(`.tl-list-container`)){window.removeEventListener(`resize`,Y);return}J!==null&&clearTimeout(J),J=setTimeout(E,200)}window.addEventListener(`resize`,Y);async function X(t){s.querySelector(`.tl-title`).textContent=`Terminliste ${_.year}`,s.querySelector(`.tl-list-container`).replaceChildren(f(`Laster...`));let{data:r,error:i}=await o(_.year);return i?(e(t,i),s.querySelector(`.tl-list-container`).replaceChildren(n(`Feil ved henting.`)),!1):(v=r??[],!0)}k.addEventListener(`change`,async()=>{_.year=Number(k.value),await X(`terminliste.yearChange`)&&E()}),A.addEventListener(`input`,()=>{_.text=A.value,E()}),j.addEventListener(`input`,()=>{_.text=j.value,A.value=j.value,E()}),M.addEventListener(`change`,()=>{_.tournamentTypeId=M.value,E()}),N.addEventListener(`change`,()=>{_.throwingMethodId=N.value,E()}),P.addEventListener(`change`,()=>{_.clubId=P.value,E()}),F.addEventListener(`change`,()=>{_.categoryId=F.value,E()});let Z=()=>S(x(v));I.addEventListener(`click`,Z),L.addEventListener(`click`,Z);function Q(){z.classList.add(`active`),B.classList.add(`active`)}function $(){z.classList.remove(`active`),B.classList.remove(`active`)}R.addEventListener(`click`,Q),B.addEventListener(`click`,$),V.addEventListener(`click`,()=>{_.text=``,_.tournamentTypeId=``,_.throwingMethodId=``,_.clubId=``,_.categoryId=``,W.value=``,G.value=``,K.value=``,q.value=``,j.value=``,A.value=``,E()}),H.addEventListener(`click`,async()=>{let e=Number(U.value),t=e!==_.year;_.year=e,_.tournamentTypeId=W.value,_.throwingMethodId=G.value,_.clubId=K.value,_.categoryId=q.value,$(),!(t&&!await X(`terminliste.applyFilter`))&&E()})}catch(t){e(`terminliste.render`,t),s.replaceChildren(n(`Kunne ikkje laste terminliste.`))}}export{A as render};