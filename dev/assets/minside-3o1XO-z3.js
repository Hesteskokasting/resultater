import{n as e,t}from"./logError-D5z16FyH.js";import{C as n,D as r,E as i,F as a,G as o,Lt as s,M as c,P as l,Rt as u,S as d,St as f,Vt as p,d as m,i as h,n as g,t as _,w as v,wt as y,xt as b}from"./index-CH5yb5iS.js";import{r as x,u as ee}from"./kasterService-B3gLOC11.js";import{t as te}from"./LoadingState-BWi0wPLz.js";import{t as S}from"./EmptyState-B1E_7OzB.js";import{d as C,m as w}from"./kampService-D8HXqauM.js";import{t as T}from"./Tabs-B8ddqDpf.js";function E(e){return`
    ...
    <div class="card mb-4">
      ...
      <table class="table table-sm mb-3"><thead><tr><th>Runde/Bane</th><th>Motstandar</th><th></th></tr></thead><tbody>
        ${n.map(e=>s(e,t(e))).join(``)}
      </tbody></table>`).join(``)},l=c(a,t=>{if(!t.kamp?.er_bekreftet)return`<a href="#/kamp/${t.kamp?.id??``}" class="btn btn-sm btn-primary" target="_blank" rel="noopener">Scoreboard</a>`;let n=t.kamp.stevneid,r=t.kamp.spelarar?.find(t=>t.kasterid===e)?.score_poeng,a=j(t.kamp.spelarar??[],e,n,i)[0]?.score_poeng;return r==null||a==null?`–`:`<span class="fw-semibold">${r} – ${a}</span>`}),d=c(o,e=>`<a href="#/kamp/${e.kamp?.id??``}" class="btn btn-sm btn-outline-secondary" target="_blank" rel="noopener">Sjå kamp</a>`);return T({tabs:[{id:`active`,label:`Aktive (${a.length})`,panel:A(l??`<p class="text-muted">Ingen aktive kampar.</p>`)},{id:`completed`,label:`Ferdige (${o.length})`,panel:A(d??`<p class="text-muted">Ingen ferdige kampar enno.</p>`)}]})}async function N(e,t){let n=k(e,t);if(n==null)return;e.innerHTML=`
    ...
    </div>`;let r=e.querySelector(`[data-slot="content"]`),i=await M(n);r.replaceWith(i)}async function P(e){let{data:t,error:n}=await m(e),r=new Map;if(n)return{html:`<p class="text-muted">Kunne ikkje laste påmeldingar.</p>`,registeredMap:r};let i=t.filter(e=>e.stevne?.erfullfort!==!0);return i.length?{html:`
    ...
    </div>`}function R(e,t){for(let[r,i]of[[`varsle-stevne-start`,`varsle_stevne_start`],[`varsle-kamp-opprettet`,`varsle_kamp_opprettet`]]){let o=e.querySelector(`#${r}`);o&&o.addEventListener(`change`,async()=>{let e=o.checked;o.disabled=!0,e&&await a();let{error:r}=await L(t,i,e);o.disabled=!1,r&&(o.checked=!e,n(`Kunne ikkje lagre varslingsinnstilling: ${d(r)}`,`error`))})}}async function z(e,t){if(!p.isNativePlatform()){e.innerHTML=`<p class="text-muted">Varslingar er berre tilgjengelege i appen.</p>`;return}e.innerHTML=`
    <div class="card mb-4">
      <div class="card-body">
        <h5 class="card-title">Varslingar</h5>
        <div data-slot="content"><div class="skeleton-block skeleton-block--card"></div></div>
      </div>
    </div>`;let n=e.querySelector(`[data-slot="content"]`),{data:r}=await I(t.user.id);if(!r){n.innerHTML=`<p class="text-muted">Kunne ikkje laste varslingsinnstillingar.</p>`;return}n.innerHTML=ne(r),R(e,t.user.id)}async function B(e,t){e.innerHTML=`
    <div class="card mb-4">
      <div class="card-body">
        <h5 class="card-title">Innstillingar</h5>
        <p class="text-muted mb-0">Fleire innstillingar kjem her seinare.</p>
      </div>
    </div>`}async function V(){let{data:n,error:r}=await e.rpc(`hent_kobla_kontoar`);return r&&t(`getLinkedAccounts`,r),{data:n??[],error:r}}async function H(n){let{error:r}=await e.rpc(`slett_brukarkonto`,{target_id:n});return r&&t(`deleteUserAccount`,r),{error:r}}var U=`Resultater og statistikk for koblet utøver blir ikkje slettet.`;async function W(e){let{data:t,error:n}=await ee(e);return n||!t?``:`
    ...
    </div></div>`,q(e),X(e,n),r==null)return;Y(e,n);let o=e.querySelector(`[data-slot="thrower"]`),s=e.querySelector(`[data-slot="accounts"]`);try{let[e,t]=await Promise.all([W(r),V()]);o.innerHTML=e,s.innerHTML=t.error?`<p class="text-muted">Kunne ikkje laste kontoar.</p>`:K(t.data,n.user.id)}catch(e){t(`minsideKonto.render`,e),s.innerHTML=`<p class="text-muted">Kunne ikkje laste kontoar.</p>`}}var Q=[{key:`kampar`,label:`Kampar`,nativeOnly:!1},{key:`pameldingar`,label:`Påmeldingar`,nativeOnly:!1},{key:`varslingar`,label:`Varslingar`,nativeOnly:!0},{key:`innstillingar`,label:`Innstillingar`,nativeOnly:!1},{key:`konto`,label:`Konto`,nativeOnly:!1}],re=new Set(Q.map(e=>e.key)),ie=new Set(Q.filter(e=>e.nativeOnly).map(e=>e.key)),ae={kampar:N,pameldingar:F,varslingar:z,innstillingar:B,konto:Z};function oe(e,t){return`<ul class="nav nav-underline mypage-nav mb-3">${Q.filter(e=>t||!e.nativeOnly).map(({key:t,label:n})=>`
      <li class="nav-item">
        <a class="nav-link${e===t?` active`:``}"
           href="#/minside/${t}">${n}</a>
      </li>`).join(``)}</ul>`}async function $(e,n){_(()=>$(e,n));let r=String(n.tab??`kampar`);e.replaceChildren(te(`Laster min side…`));try{let t=await i();if(!t){location.hash=`#/logginn`;return}let{profil:n,user:a}=t,o=n?.kobling_status??`ingen`,s=p.isNativePlatform(),c=!re.has(r)||!s&&ie.has(r)?`kampar`:r;e.innerHTML=`
