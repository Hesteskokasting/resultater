import{W as e,c as t,d as n,f as r,h as i,l as a,m as o,nt as s,p as c,s as l,u}from"./index-BogKy9jL.js";import{t as d}from"./adminForms-DPsl7IED.js";import{r as f}from"./klubbService-D7G0Rm2V.js";import{i as p}from"./kasterService-g2LXrNMc.js";import{t as m}from"./LoadingState-RVZNML7E.js";import{t as h}from"./EmptyState-a5aDhc-8.js";var g=[`kobling`,`brukarar`,`klubbadmin`],_={kobling:`Koblingforespørslar`,brukarar:`Brukarar`,klubbadmin:`Klubbadmin-tilgang`};async function v(e){e.innerHTML=`
    ...
    </table>`,r.forEach(e=>{let t=n.querySelector(`tr[data-id="${e.id}"]`);t&&(t.querySelector(`.rolle-vel`).value=e.rolle)}),n.querySelectorAll(`.lagre-rolle`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.closest(`tr`),r=t.querySelector(`.rolle-vel`).value,i=n.querySelector(`#brukar-feil`);i.classList.add(`d-none`);let{error:a}=await o(t.dataset.id,r);a?(i.textContent=d(a),i.classList.remove(`d-none`)):(e.textContent=`✓`,setTimeout(()=>{e.textContent=`Lagre`},2e3))})})}async function S(t){let r,i,a;try{let e=await Promise.all([u(),f(),n()]);r=e[0].data,i=e[1].data,a=e[2].data}catch(e){s(`admin._visKlubbadmin`,e),t.innerHTML=`<div class="alert alert-danger">Kunne ikkje laste data.</div>`;return}if(!r.length){t.replaceChildren(h(`Ingen brukarar med rolle "klubbadmin".`));return}let o=await b(r.map(e=>e.id)),p={};a.forEach(e=>{(p[e.bruker_id]??=new Set).add(e.klubbid)});let m=i.map(t=>`<option value="${t.id}">${e(t.navn)}</option>`).join(``);t.innerHTML=`
    <div id="ka-feil" class="alert alert-danger d-none"></div>
    ${r.map(t=>{let n=[...p[t.id]??[]].map(t=>{let n=i.find(e=>e.id===t);return n?`<span class="badge bg-primary me-1" data-kid="${t}">${e(n.navn)} <button class="btn-close btn-close-white btn-close-xs fjern-klubb"></button></span>`:``}).join(``);return`<div class="card mb-3" data-bruker="${t.id}">
        <div class="card-body">
          <h6 class="card-title mb-2">${e(o[t.id]??t.id)}</h6>
