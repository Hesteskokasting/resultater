import{n as e,t}from"./logError-ByTg738k.js";import{S as n,dr as r,f as i,fr as a,ur as o}from"./index-BzkBRoPl.js";import{t as s}from"./SearchSelect-AmUwjtH-.js";import{t as c}from"./navigation-DSwGXsno.js";var l={tournamentId:0,playerId:null};async function u(){let{data:t,error:n}=await e.from(`stevne`).select(`id, navn, dato, kamp!inner(id, kamp_spelar!inner(id, kamp_omgang!inner(id)))`).limit(1,{referencedTable:`kamp`}).limit(1,{referencedTable:`kamp.kamp_spelar`}).limit(1,{referencedTable:`kamp.kamp_spelar.kamp_omgang`}).order(`dato`,{ascending:!1});if(n)throw n;return(t??[]).map(({id:e,navn:t,dato:n})=>({id:e,navn:t,dato:n}))}async function d(t){let{data:r,error:i}=await e.from(`kamp`).select(`
    ...
    </div>`}function m(e,t){return t==null?e:e.filter(e=>e.spelarar.some(e=>e.kasterid===t))}function h(e){let t=new Map;for(let n of e)for(let e of n.spelarar)t.set(e.kasterid,e.navn);return[...t].map(([e,t])=>({id:e,label:t})).sort((e,t)=>e.label.localeCompare(t.label,`nb`))}async function g(e){e.replaceChildren(a(`Laster stevne…`));let n;try{n=await u()}catch(n){t(`kampstatistikk.fetchTournaments`,n),e.replaceChildren(r(`Kunne ikkje laste stevne.`));return}if(!n.length){e.replaceChildren(o(`Ingen stevne funne.`));return}let f=Number(c(`stevneid`));l.tournamentId=n.some(e=>e.id===f)?f:n[0].id,l.playerId=Number(c(`kasterid`))||null,e.innerHTML=`
