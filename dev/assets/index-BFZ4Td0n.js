const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/gloppen-KDgweHCD.js","assets/innledendeBase-d5HBmIly.js","assets/ScoreNumberpad-C-b3sWkY.js","assets/nordhordland-BRh5CA_F.js","assets/cup-C6fcTVop.js"])))=>i.map(i=>d[i]);
...
`),y.from(`kamp_spelar`).select(`id, kasterid, score_poeng, antall_ringer, omgangar:kamp_omgang(score, antall_ringer)`);async function Yr(e){let{data:t,error:n}=await y.from(`kamp`).select(`
    ...
    </div>`}function fi(){return`<div class="alert alert-info mb-4">Koblingforespørselen din ventar på godkjenning frå ein administrator.</div>`}async function pi(e){let{data:t,error:n}=await on(e);return n||!t?``:`
    <div class="card mb-4">
      <div class="card-body">
    ...
    </div>`}async function mi(e){let{data:t,error:n}=await wr(e);return n?`<p class="text-muted">Kunne ikkje laste påmeldingar.</p>`:t.length?`
    <div class="card mb-4">
      <div class="card-body">
    ...
    </div>`:`<p class="empty-state">Ingen påmeldingar enno.</p>`}async function hi(e){let{data:t,error:n}=await Lr(e);if(n){let e=document.createElement(`p`);return e.className=`text-muted`,e.textContent=`Kunne ikkje laste kampar.`,e}let r=t.filter(e=>!e.kamp?.er_walkover),i=r.filter(e=>e.kamp?.stevne?.erfullfort===!1&&!e.kamp?.er_bekreftet).sort((e,t)=>(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),a=r.filter(e=>e.kamp?.er_bekreftet).sort((e,t)=>(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),o=(t,n)=>{let r=t.kamp,i=(r?.spelarar??[]).find(t=>t.kasterid!==e),a=i?.kaster?j(`${i.kaster.fornavn} ${i.kaster.etternavn}`):`–`;return`<tr>
      ...
      </tbody></table>`).join(``)},c=s(i,e=>`<a href="#/kamp/${e.kamp?.id??``}" class="btn btn-sm btn-primary">Scoreboard</a>`),l=s(a,e=>`<a href="#/kamp/${e.kamp?.id??``}" class="btn btn-sm btn-outline-secondary">Sjå kamp</a>`),u=document.createElement(`div`);u.className=`card mb-4`,u.id=`mine-kampar-seksjon`;let d=document.createElement(`div`);d.className=`card-body`;let f=document.createElement(`h5`);return f.className=`card-title`,f.textContent=`Mine kampar`,d.appendChild(f),d.appendChild(xr({tabs:[{id:`kommande`,label:`Kommande (${i.length})`,panel:li(c??`<p class="text-muted">Ingen kommande kampar.</p>`)},{id:`ferdige`,label:`Ferdige (${a.length})`,panel:li(l??`<p class="text-muted">Ingen ferdige kampar enno.</p>`)}]})),u.appendChild(d),u}function gi(e,t){let n=null,r=null,i=e.querySelector(`#kaster-sok`),o=e.querySelector(`#kaster-treff`),s=e.querySelector(`#kasting-feil`);i.addEventListener(`input`,()=>{n!==null&&clearTimeout(n);let e=i.value.trim().toLowerCase();if(e.length<2){o.innerHTML=``;return}n=setTimeout(async()=>{if(!r){let{data:e}=await tn();r=e}let t=r.filter(t=>t.fornavn.toLowerCase().includes(e)||t.etternavn.toLowerCase().includes(e)).slice(0,8);if(!t.length){let e=L(`Ingen treff.`);e.classList.add(`small`),o.replaceChildren(e);return}o.innerHTML=t.map(e=>`<button class="list-group-item list-group-item-action" data-id="${e.id}">
          ${j(a(e))} <span class="text-muted small">· ${j(e.klubb?.navn??``)}</span>
        </button>`).join(``)},300)}),o.addEventListener(`click`,async e=>{let n=e.target.closest(`[data-id]`);if(!n)return;s.classList.add(`d-none`);let{error:r}=await He(t,Number(n.dataset.id));if(r){s.textContent=`Kunne ikkje sende forespørsel.`,s.classList.remove(`d-none`);return}location.reload()})}async function _i(e){e.replaceChildren(A(`Laster min side…`));try{let t=await M();if(!t){location.hash=`#/logginn`;return}let{profil:n,user:r}=t,i=n?.kobling_status??`ingen`,a=n?ui[n.rolle]:`Ukjent`,o=`
      <div class="minside-container">
        <h2 class="mb-1">Min side</h2>
        <p class="text-muted mb-4">${j(r.email??``)} · <span class="badge bg-secondary">${j(a)}</span></p>`;if(i===`ingen`||i===`avvist`)o+=di(i);else if(i===`venter`)o+=fi();else if(i===`godkjent`&&n?.kasterid){let t=n.kasterid,[i,a,s]=await Promise.all([pi(t),mi(r.id),hi(t)]);o+=i+a,o+=`</div>`,e.innerHTML=o,e.querySelector(`.minside-container`).appendChild(s);return}o+=`</div>`,e.innerHTML=o,(i===`ingen`||i===`avvist`)&&gi(e,r.id)}catch(t){b(`minside.render`,t),e.replaceChildren(k(`Kunne ikkje laste min side.`))}}function q(e){return e&&typeof e==`object`&&`message`in e?String(e.message):`Ukjend feil`}function J(e,t){return`<div class="mb-3"><label class="form-label fw-semibold">${j(e)}</label>${t}</div>`}function vi(e,t){let n=e.querySelector(`.admin-feil`);n||(n=document.createElement(`div`),n.className=`alert alert-danger admin-feil mt-3 d-none`,e.querySelector(`form`)?.append(n)),n.textContent=t,n.classList.remove(`d-none`),n.scrollIntoView({behavior:`smooth`,block:`nearest`})}function yi(e,t){let n=e.querySelector(`.admin-suksess`);n||(n=document.createElement(`div`),n.className=`alert alert-success admin-suksess mt-3 d-none`,e.querySelector(`form`)?.append(n)),n.textContent=t,n.classList.remove(`d-none`);let r=n;setTimeout(()=>{r.classList.add(`d-none`)},4e3)}var bi=[`kobling`,`brukarar`,`klubbadmin`],xi={kobling:`Koblingforespørslar`,brukarar:`Brukarar`,klubbadmin:`Klubbadmin-tilgang`};async function Si(e){e.innerHTML=`
    <div class="container py-4 admin-skjema-xl">
      <h2 class="mb-3">Administrasjon</h2>
      <ul class="nav nav-tabs mb-4" id="admin-faner">
        ${bi.map((e,t)=>`<li class="nav-item">
          <button class="nav-link${t===0?` active`:``}" data-fane="${e}">${xi[e]}</button>
        </li>`).join(``)}
      </ul>
      <div id="admin-innhald"></div>
    </div>`;let t=e.querySelector(`#admin-innhald`);async function n(n){e.querySelectorAll(`[data-fane]`).forEach(e=>{e.classList.toggle(`active`,e.dataset.fane===n)}),A(`Laster...`),n===`kobling`&&await Ci(t),n===`brukarar`&&await wi(t),n===`klubbadmin`&&await Ti(t)}e.querySelector(`#admin-faner`).addEventListener(`click`,e=>{let t=e.target.closest(`[data-fane]`);t?.dataset.fane&&n(t.dataset.fane)}),n(`kobling`)}async function Ci(e){let{data:t,error:n}=await Ue();if(n){e.innerHTML=`<div class="alert alert-danger">${j(q(n))}</div>`;return}if(!t.length){e.replaceChildren(L(`Ingen ventande forespørslar.`));return}let r=t.map(e=>e.id),i=t.map(e=>e.kobling_kasterid).filter(e=>e!==null),[{data:a},{data:o}]=await Promise.all([We(r),ln(i)]),s=Object.fromEntries((a??[]).map(e=>[e.id,e.epost])),c=new Map((o??[]).map(e=>[e.id,e]));e.innerHTML=`<table class="table table-hover">
  ...
  `,document.body.appendChild(Y),Y.querySelector(`#cd-cancel`).addEventListener(`click`,()=>{Mi(!1)}),Y.querySelector(`#cd-confirm`).addEventListener(`click`,()=>{Mi(!0)}),Y)}function Ai(e){Ei=document.createElement(`div`),Ei.className=`modal-backdrop show`,document.body.appendChild(Ei),document.body.classList.add(`modal-open`),e.style.display=`block`,e.classList.add(`show`),e.querySelector(`#cd-confirm`)?.focus(),Oi=e=>{e.key===`Escape`&&(e.preventDefault(),Mi(!1))},document.addEventListener(`keydown`,Oi)}function ji(e){e.classList.remove(`show`),e.style.display=`none`,Ei?.remove(),Ei=null,document.body.classList.remove(`modal-open`),Oi&&=(document.removeEventListener(`keydown`,Oi),null)}function Mi(e){if(!Y||!Di)return;let t=Di;Di=null,ji(Y),t(e)}function X(e){let{title:t,message:n,confirmText:r=`OK`,cancelText:i=`Avbryt`,danger:a=!1}=e,o=ki();o.querySelector(`#cd-title`).textContent=t,o.querySelector(`#cd-message`).textContent=n,o.querySelector(`#cd-cancel`).textContent=i;let s=o.querySelector(`#cd-confirm`);return s.textContent=r,s.className=`btn ${a?`btn-danger`:`btn-primary`}`,new Promise(e=>{Di=e,Ai(o)})}function Z(e){if(!e||typeof e!=`string`)return null;let t=Number(e);return Number.isFinite(t)?t:null}async function Ni(e,{id:t}={}){e.replaceChildren(A());let n=[],r=[],i=[],a=[];try{let e=await Promise.all([Rn(),me(),he(),ge()]);n=e[0].data,r=e[1].data,i=e[2].data,a=e[3].data}catch(t){b(`stevneadmin.render`,t),e.replaceChildren(k(`Kunne ikkje laste skjema.`));return}let o=null;if(t){let{data:n,error:r}=await _e(t);if(r||!n){e.replaceChildren(k(`Stevne ikkje funne.`));return}if(o=n,!await N()&&!await at(o.klubbid??void 0)){e.replaceChildren(k(`Ingen tilgang til dette stevnet.`));return}}let s=t?`Rediger stevne: ${j(o?.navn??``)}`:`Nytt stevne`,c=o??{},l=c.dato??``,u=c.tid?c.tid.slice(0,5):``,d=P(n,c.klubbid),f=P(r,c.stevnetypeid),p=P(i,c.innledendekastemetodeid),m=P(i,c.avsluttendekastemetodeid),h=P(a,c.kategoriid);e.innerHTML=`
    ...
    </div>`,e.querySelector(`#kaster-skjema`).addEventListener(`submit`,async n=>{n.preventDefault();let r=new FormData(n.target),i={fornavn:r.get(`fornavn`).trim(),etternavn:r.get(`etternavn`).trim(),kjonnid:Z(r.get(`kjonnid`)),klubbid:Z(r.get(`klubbid`)),klasseid:Z(r.get(`klasseid`)),epost:r.get(`epost`).trim()||null,telefon:r.get(`telefon`).trim()||null,medlemsnummer:r.get(`medlemsnummer`)?Number(r.get(`medlemsnummer`)):null,eraktiv:r.get(`eraktiv`)===`on`},{data:a,error:o}=t?await fn(t,i):await dn(i);if(o){vi(e,q(o));return}yi(e,`Utøvaren er lagra.`),t||setTimeout(()=>{location.hash=`#/kaster/${a.id}/admin`},1500)}),e.querySelector(`#slett-knapp`)?.addEventListener(`click`,async()=>{if(!await X({title:`Slett utøvar`,message:`Slett «${a?.fornavn} ${a?.etternavn}»? Dette kan ikkje angrast.`,danger:!0}))return;let{error:n}=await pn(t);if(n){vi(e,q(n));return}location.hash=`#/kastere`})}async function Fi(e,{id:t}={}){if(!t){e.replaceChildren(k(`Manglande ID.`));return}e.replaceChildren(A());let{data:n,error:r}=await Bn(t);if(r||!n){e.replaceChildren(k(`Klubb ikkje funne.`));return}if(!await N()&&!await at(t)){e.replaceChildren(k(`Ingen tilgang til denne klubben.`));return}e.innerHTML=`
    ...
    </div>`,e.querySelector(`#klubb-skjema`).addEventListener(`submit`,async n=>{n.preventDefault();let r=new FormData(n.target),{error:i}=await Vn(t,{navn:r.get(`navn`).trim(),kortnavn:r.get(`kortnavn`).trim(),logourl:r.get(`logourl`).trim()||null,eraktiv:r.get(`eraktiv`)===`on`});if(i){vi(e,q(i));return}yi(e,`Klubben er lagra.`)})}function Ii(e,t,n,r,i,a){return e?!n&&!t?`<div class="alert alert-warning">
    ...
    </div>`}function Li(e,t,n,r){if(!e||t)return``;let i=new Set(n.map(e=>e.kasterid));return`
    ...
    </form>`}function Ri(e){return e.length?`
    <div class="mt-4 mb-3">
    ...
    </div>`:``}function zi(e,t){if(!e.length)return`<p class="empty-state">Ingen påmeldingar enno.</p>`;let n=e.map(e=>`<tr>
    ...
    <tbody>${n}</tbody>
  </table>`}function Bi(e,t,n,r,i,a){e.querySelector(`#pamelding-skjema`)?.addEventListener(`submit`,async n=>{n.preventDefault();let o=e.querySelector(`#pm-feil`);if(o.classList.add(`d-none`),r==null)return;let{error:s}=await Er(a,r,i);if(s){o.textContent=`Feil ved påmelding.`,o.classList.remove(`d-none`);return}Vi(e,t)});let o=e.querySelector(`#admin-pamelding-skjema`);o?.addEventListener(`submit`,async n=>{n.preventDefault();let r=e.querySelector(`#admin-pm-feil`);r.classList.add(`d-none`);let s=new FormData(o),c=Number(s.get(`admin_kasterid`));if(!c){r.textContent=`Vel ein utøvar.`,r.classList.remove(`d-none`);return}let{error:l}=await Er(a,c,i);if(l){r.textContent=`Feil ved påmelding.`,r.classList.remove(`d-none`);return}Vi(e,t)}),e.querySelector(`#avmeld-knapp`)?.addEventListener(`click`,async()=>{if(r==null)return;let i=n.find(e=>e.kasterid===r);if(!i||!await X({title:`Avmeld`,message:`Vil du melde deg av?`}))return;let{error:a}=await Dr(i.id);a||Vi(e,t)}),e.querySelectorAll(`.fjern-pm`).forEach(n=>{n.addEventListener(`click`,async()=>{if(!await X({title:`Fjern påmelding`,message:`Fjern påmelding?`}))return;let r=Number(n.dataset.id);if(!r)return;let{error:i}=await Dr(r);i||Vi(e,t)})})}async function Vi(e,t={}){let n=t.id;if(!n){e.replaceChildren(k(`Manglande stevne-ID.`));return}let r=Number(n);e.replaceChildren(A(`Laster påmelding…`));try{let[n,i]=await Promise.all([M(),ue(r)]);if(i.error||!i.data){e.replaceChildren(k(`Stevnet finst ikkje.`));return}let a=i.data,o=n?.profil?.rolle===`admin`,s=n?.profil?.rolle===`klubbadmin`,c=o||s,l=a.dato?{fraDato:new Date(new Date(a.dato+`T12:00:00`).getTime()-2*864e5).toISOString().slice(0,10),tilDato:new Date(new Date(a.dato+`T12:00:00`).getTime()+2*864e5).toISOString().slice(0,10)}:null,u=c?o?tn():n&&n.klubber.length?an(n.klubber):Promise.resolve({data:[],error:null}):Promise.resolve({data:[],error:null}),[d,f,p]=await Promise.all([Tr(r),a.klubbid!=null&&l?de(a.klubbid,l.fraDato,l.tilDato,r):Promise.resolve({data:[],error:null}),u]),m=d.data,h=f.data,g=p.data,_=n?.profil?.kasterid??null,v=n?.profil?.kobling_status===`godkjent`,y=_!=null&&m.some(e=>e.kasterid===_),b=a.dato?O(a.dato):``;e.innerHTML=`
      <div class="container py-4 pm-side">
        <h2 class="mb-1">${j(a.navn??``)}</h2>
        <p class="text-muted mb-4">${b}${a.sted?` · `+j(a.sted):``}</p>
        ${Ii(n,c,v,y,a.erfullfort??!1,r)}
        ${Li(c,a.erfullfort??!1,m,g)}
        ${Ri(h)}
        <h5 class="mt-4 mb-2">Påmeldingar (${m.length})</h5>
        ${zi(m,c)}
      </div>`,n&&Bi(e,t,m,_,n.user.id,r)}catch(t){b(`pamelding.render`,t),e.replaceChildren(k(`Kunne ikkje laste påmelding.`))}}async function Hi(e){await y.removeChannel(e)}var Ui=null;function Wi(){return Ui||(Ui=document.createElement(`div`),Ui.id=`toast-container`,document.body.appendChild(Ui)),Ui}function Q(e,t=`info`){let n=document.createElement(`div`);n.className=`toast-item toast-${t}`,n.textContent=e,n.addEventListener(`click`,()=>n.remove()),Wi().appendChild(n),setTimeout(()=>{n.remove()},4e3)}async function Gi(e,t,n,r,i){let{pointValues:a,erArrangor:o=!1,erDeltakar:s=!1,onBekreft:c=null,onKampBekreft:l,omgangEl:u=null,p3ks:d=null,hcp1:f=0,hcp2:p=0}=i;if(d&&t.er_tre_spelarar)return Yi(e,t,n,r,d,{pointValues:a,erArrangor:o,erDeltakar:s,onBekreft:c,onKampBekreft:l,omgangEl:u});let m=[],h=null,g=null,_=t.er_bekreftet||t.er_walkover,v=o||s&&!t.er_bekreftet;await x(),O();let y=[n?.id,r?.id].filter(e=>e!=null),b=ci(t.id,y,async()=>{await x(),O()},async()=>{t.er_bekreftet=!0,await x(),O(),await l?.()});window.addEventListener(`hashchange`,()=>void Hi(b),{once:!0});async function x(){let e=[n?.id,r?.id].filter(e=>e!=null);if(!e.length)return;let{data:i}=await ti(e),a={};for(let e of i)a[e.omgang]||(a[e.omgang]={omgang:e.omgang,s1:0,s2:0,r1:0,r2:0}),e.kamp_spelar_id===n?.id?(a[e.omgang].s1=e.score??0,a[e.omgang].r1=e.antall_ringer??0):(a[e.omgang].s2=e.score??0,a[e.omgang].r2=e.antall_ringer??0);m=Object.values(a).sort((e,t)=>e.omgang-t.omgang);let[o,s]=C();_=T(o,s)||t.er_bekreftet||t.er_walkover}function S(){return[m.reduce((e,t)=>e+t.s1,0),m.reduce((e,t)=>e+t.s2,0)]}function C(){let[e,t]=S();return[e+f,t+p]}function w(){return[m.reduce((e,t)=>e+t.r1,0),m.reduce((e,t)=>e+t.r2,0)]}function T(e,n){return t.fase===`innledende`?e>=21||n>=21:e>=21&&e-n>=2||n>=21&&n-e>=2}function E(){return m.length>0?m[m.length-1].omgang+1:1}function D(e,t){let n=new Set,r=new Set;return e!==null&&(a.forEach(t=>{t!==e&&n.add(t)}),[1,2,4].includes(e)?a.forEach(e=>r.add(e)):[1,2,4].forEach(e=>r.add(e))),t!==null&&(a.forEach(e=>{e!==t&&r.add(e)}),[1,2,4].includes(t)?a.forEach(e=>n.add(e)):[1,2,4].forEach(e=>n.add(e))),{p1Dis:n,p2Dis:r}}function O(){e.innerHTML=``;let[i,a]=C(),[l,d]=w(),f=E(),{p1Dis:p,p2Dis:y}=D(h,g),b=v&&!_&&(h!==null||g!==null),x=_&&!t.er_bekreftet&&(o||s)&&!!c,S=m.length*2;u&&(u.textContent=t.er_bekreftet?`Fullført`:_?`Ferdig`:`Omgang ${f}`);let T=$(`div`,null,`sb-wrap`);if(T.appendChild(ee(Ki(n,`Spelar 1`),i,l,S,h,p,!v,1)),T.appendChild(ee(Ki(r,`Spelar 2`),a,d,S,g,y,!v,2)),e.appendChild(T),v){let t=$(`div`,null,`sb-angre-rad`);m.length>0&&t.appendChild(qi(m.map(e=>e.omgang),ne));let n=$(`button`,`↩`,`sb-angre-btn`);n.title=`Angre val for denne omgangen`,n.disabled=h===null&&g===null,n.addEventListener(`click`,()=>{h=null,g=null,O()}),t.appendChild(n),e.appendChild(t)}if(x)e.appendChild(Ji(()=>c()));else if(v){let t=$(`button`,`Neste omgang`,`sb-neste-btn`);t.disabled=!b,t.addEventListener(`click`,async()=>{t.disabled=!0,t.textContent=`Lagrer…`;try{await te()}finally{t.disabled=!1,t.textContent=`Neste omgang`}}),e.appendChild(t)}e.querySelectorAll(`[data-spelar]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.spelar??`0`),n=parseInt(e.dataset.val??`0`);t===1?h=n:g=n,O()})})}function ee(e,t,n,r,i,o,s,c){let l=$(`div`,null,`sb-spelar-panel`);l.appendChild($(`div`,e,`sb-spelar-navn`)),l.appendChild($(`div`,String(t),`sb-score`));let u=r>0?Math.round(n/r*100):0;if(l.appendChild($(`p`,`Ring: ${n} av ${r} ( ${u}% )`,`sb-ringer-info`)),!s){let e=$(`div`,null,`sb-knappar`);for(let t of a){let n=$(`button`,String(t),`sb-poeng-btn`);n.dataset.spelar=String(c),n.dataset.val=String(t),o.has(t)&&(n.disabled=!0),i===t&&n.classList.add(`sb-valgt`),e.appendChild(n)}l.appendChild(e)}return l}async function te(){let e=E(),t=h??0,i=g??0,a=t===6?2:+(t===3||t===4),o=i===6?2:+(i===3||i===4),s=[];n?.id&&s.push({kamp_spelar_id:n.id,omgang:e,score:t,antall_ringer:a}),r?.id&&s.push({kamp_spelar_id:r.id,omgang:e,score:i,antall_ringer:o});let{error:c}=await ri(s);if(c){Q(`Feil ved lagring`,`error`);return}m.push({omgang:e,s1:t,s2:i,r1:a,r2:o}),h=null,g=null;let[l,u]=C();T(l,u)&&(_=!0),O()}async function ne(e){if(!await X({title:`Slett omgangar`,message:`Slett omgang ${e} og alle etter? Dette kan ikkje angrast.`,danger:!0}))return;let{error:i}=await ai([n?.id,r?.id].filter(e=>e!=null),e);if(i){Q(`Feil ved sletting`,`error`);return}if(t.er_bekreftet){let{error:e}=await ii(t.id);if(e){Q(`Feil ved oppdatering av kampstatus`,`error`);return}t.er_bekreftet=!1}m=m.filter(t=>t.omgang<e),h=null,g=null;let[a,o]=C();_=T(a,o),O()}}function $(e,t,n){let r=document.createElement(e);return t!=null&&(r.textContent=t),n&&(r.className=n),r}function Ki(e,t=`Spelar`){return e?.kaster?`${e.kaster.fornavn} ${e.kaster.etternavn}`:t}function qi(e,t){let n=$(`div`,null,`sb-omg-btns`);for(let r of e){let e=$(`button`,String(r),`sb-omg-btn`);e.title=`Slett frå omgang ${r}`,e.addEventListener(`click`,()=>t(r)),n.appendChild(e)}return n}function Ji(e){let t=$(`button`,`Bekreft kamp`,`sb-neste-btn sb-neste-btn--bekreft`);return t.addEventListener(`click`,async()=>{t.disabled=!0,t.textContent=`Lagrar…`;try{await e()}finally{t.disabled=!1,t.textContent=`Bekreft kamp`}}),t}async function Yi(e,t,n,r,i,a){let{pointValues:o,erArrangor:s=!1,erDeltakar:c=!1,onBekreft:l=null,onKampBekreft:u,omgangEl:d=null}=a,f=s||c&&!t.er_bekreftet,p=[n,r,i].filter(e=>e!=null),m=p.map(e=>e.id).filter(e=>e!=null),h=[],g=[],_=[null,null,null];function v(e){return h.filter(t=>t.kamp_spelar_id===p[e]?.id).reduce((e,t)=>e+(t.score??0),0)}function y(){if(!h.length)return[];let e=Math.max(...h.map(e=>e.omgang)),t=new Set([0,1,2].filter(e=>p[e])),n=[],r=[0,0,0];for(let i=1;i<=e;i++){for(let e of t){let t=h.find(t=>t.kamp_spelar_id===p[e].id&&t.omgang===i);t&&(r[e]+=t.score??0)}let e=!0;for(;e&&t.size>1;){e=!1;for(let i of[...t]){let a=[...t].filter(e=>e!==i),o=Math.min(...a.map(e=>r[e]));if(r[i]>=21&&r[i]-o>=2){n.push(i),t.delete(i),e=!0;break}}}}return t.size===1&&n.length===2&&n.push([...t][0]),n}async function b(){if(!m.length)return;let{data:e}=await ti(m);h=e,g=y()}await b();let x=ci(t.id,m,async()=>{await b(),C()},async()=>{t.er_bekreftet=!0,await b(),C(),await u?.()});window.addEventListener(`hashchange`,()=>void Hi(x),{once:!0});function S(e){let t=p.map(()=>new Set),n=e.filter(e=>_[e]!==null);if(!n.length)return t;let r=n.some(e=>[1,2,4].includes(_[e])),i=n.some(e=>[3,6].includes(_[e]));for(let n of e)_[n]===null?r?o.forEach(e=>t[n].add(e)):i&&[1,2,4].forEach(e=>t[n].add(e)):o.forEach(e=>{e!==_[n]&&t[n].add(e)});return t}function C(){e.innerHTML=``;let n=p.map((e,t)=>v(t)),r=[0,1,2].filter(e=>p[e]&&!g.includes(e)),i=g.length===p.length,a=h.length?Math.max(...h.map(e=>e.omgang)):0,s=S(r);d&&(d.textContent=t.er_bekreftet?`Fullført`:i?`Ferdig`:`Omgang ${a+1}`);let c=$(`div`,null,`sb-wrap sb-wrap--3p`);if(p.forEach((e,r)=>{let a=g.includes(r),l=a?g.indexOf(r)+1:null,u=$(`div`,null,`sb-spelar-panel${a?` sb-spelar-panel--vann`:``}`);if(u.appendChild($(`div`,Ki(e),`sb-spelar-navn`)),u.appendChild($(`div`,String(n[r]),`sb-score`)),l&&u.appendChild($(`div`,`${l}. plass`,`sb-plass-badge`)),!a&&f&&!i&&!t.er_bekreftet){let e=$(`div`,null,`sb-knappar`);for(let t of o){let n=$(`button`,String(t),`sb-poeng-btn`);n.dataset.spelar=String(r),n.dataset.val=String(t),_[r]===t&&n.classList.add(`sb-valgt`),s[r]?.has(t)&&(n.disabled=!0),e.appendChild(n)}u.appendChild(e)}c.appendChild(u)}),e.appendChild(c),f&&!i&&!t.er_bekreftet){let t=$(`div`,null,`sb-angre-rad`);if(h.length>0){let e=[...new Set(h.map(e=>e.omgang))].sort((e,t)=>e-t);t.appendChild(qi(e,T))}let n=$(`button`,`↩`,`sb-angre-btn`);n.title=`Angre val for denne omgangen`,n.disabled=r.every(e=>_[e]===null),n.addEventListener(`click`,()=>{_=[null,null,null],C()}),t.appendChild(n),e.appendChild(t);let i=r.some(e=>_[e]!==null),a=$(`button`,`Neste omgang`,`sb-neste-btn`);a.disabled=!i,a.addEventListener(`click`,async()=>{a.disabled=!0,a.textContent=`Lagrer…`;try{await w()}finally{a.disabled=!1,a.textContent=`Neste omgang`}}),e.appendChild(a)}i&&!t.er_bekreftet&&l&&f?e.appendChild(Ji(()=>l(g.map(e=>p[e].kasterid)))):t.er_bekreftet&&e.appendChild($(`div`,`Kamp fullført`,`alert alert-success mt-2`)),e.querySelectorAll(`[data-spelar]`).forEach(e=>{e.addEventListener(`click`,()=>{_[parseInt(e.dataset.spelar??`0`)]=parseInt(e.dataset.val??`0`),C()})})}async function w(){let e=[0,1,2].filter(e=>p[e]&&!g.includes(e)),t=h.length?Math.max(...h.map(e=>e.omgang))+1:1,{error:n}=await ri(e.map(e=>{let n=_[e]??0;return{kamp_spelar_id:p[e].id,omgang:t,score:n,antall_ringer:n===6?2:+(n===3||n===4)}}));if(n){Q(`Feil ved lagring`,`error`);return}_=[null,null,null],await b(),C()}async function T(e){if(!await X({title:`Slett omgangar`,message:`Slett omgang ${e} og alle etter? Dette kan ikkje angrast.`,danger:!0}))return;let{error:n}=await ai(m,e);if(n){Q(`Feil ved sletting`,`error`);return}if(t.er_bekreftet){let{error:e}=await ii(t.id);if(e){Q(`Feil ved oppdatering av kampstatus`,`error`);return}t.er_bekreftet=!1}_=[null,null,null],await b(),C()}C()}function Xi(e){let t=[...e];for(let e=t.length-1;e>0;e--){let n=Math.floor(Math.random()*(e+1));[t[e],t[n]]=[t[n],t[e]]}return t}function Zi(e){return e===2||e===4?!0:e<2?!1:e%3==0?Zi(Math.floor(e/3)*2):e%2==0?Zi(e/2):!1}function Qi(e){return e%3==0?{c3:e/3,c2:0}:e%2==0?{c3:0,c2:e/2}:{c3:0,c2:0}}function $i(e){if(e<2)return[];let t=[],n=Math.floor(e/2);for(let r=e%3;r<=n&&r<=3&&t.filter(e=>e.c2===0).length<2;r+=3){let n=(e-r)/3;if(n<1)break;Zi(r+2*n)&&t.push({walkovers:r,c3:n,c2:0})}for(let r=e%2;r<=n&&r<=3&&t.filter(e=>e.c3===0).length<2;r+=2){let n=(e-r)/2;if(n<1)break;let i=r+n;!t.some(e=>e.walkovers===r&&e.c3===0&&e.c2===n)&&Zi(i)&&t.push({walkovers:r,c3:0,c2:n})}return t.sort((e,t)=>e.walkovers-t.walkovers||t.c3-e.c3),t}function ea(e){return e===2?!0:$i(e).length>0}function ta(e){let t=Math.ceil(e*.5),n=Math.floor(e*.8),r=[];for(let i=n;i>=t;i--){let t=e-i;t>=2&&ea(i)&&ea(t)&&r.push({nA:i,nB:t})}return r}function na(e,{runde1:t=null,walkovers1:n=null}={}){let r=[],i=e,a=1,o=!0;for(;i>2;){let e,s,c;if(o){let r=t;!r&&n!==null&&(r={walkovers:n,c3:Math.floor((i-n)/3),c2:0}),r||=$i(i)[0]??{walkovers:i%3,c3:Math.floor(i/3),c2:0},c=r.walkovers,e=r.c3,s=r.c2,o=!1}else{let t=Qi(i);e=t.c3,s=t.c2,c=0}let l=e*2+s+c;r.push({runde:a,spelarar:i,baner:e+s,treSpelarar:e>0,walkovers:c,vidare:l}),i=l,a++}return r}function ra(e,{medSeeding:t=!0,isRunde1:n=!1,walkoverTall:r=null,runde1Oppsett:i=null}={}){let a=[],o=[...e];if(n){let e=i?.walkovers??r??o.length%3;if(e>0){let t=o.slice(0,e);o=o.slice(e);for(let e of t)a.push({spelarar:[e.kasterid],erWalkover:!0,erTreSpelarar:!1})}}let s=o.length,c,l;if(i&&n)c=i.c3,l=i.c2;else if(n)c=Math.floor(s/3),l=0;else{let e=Qi(s);c=e.c3,l=e.c2}let u=c+l;if(t&&u>0){let e=Xi(o.slice(0,u)),t=Xi(o.slice(u,2*u)),n=Xi(o.slice(2*u)),r=0;for(let i=0;i<u;i++){let o=i<c,s=[e[i],t[i]];o&&n[r]&&s.push(n[r++]),a.push({spelarar:s.map(e=>e.kasterid),erWalkover:!1,erTreSpelarar:o})}}else{let e=Xi(o),t=0;for(let n=0;n<c;n++)a.push({spelarar:e.slice(t,t+3).map(e=>e.kasterid),erWalkover:!1,erTreSpelarar:!0}),t+=3;for(let n=0;n<l;n++)a.push({spelarar:e.slice(t,t+2).map(e=>e.kasterid),erWalkover:!1,erTreSpelarar:!1}),t+=2}return a}function ia(){return crypto.randomUUID()}async function aa(e,t,n,r,i=0,a=null){let o=t.map(()=>ia()),s=i,c=t.map((t,i)=>({match_id:o[i],stevneid:e,fase:`avsluttende`,runde_nummer:n,gruppe_navn:r??null,bane_nummer:t.erWalkover?null:++s,er_bekreftet:!1,er_walkover:t.erWalkover,er_tre_spelarar:t.erTreSpelarar,runde_navn:a})),{data:l,error:u}=await y.from(`kamp`).insert(c).select(`id, match_id`);if(u)throw Error(`Feil ved innsetting av cup-kampar: `+u.message);let d=Object.fromEntries(l.map(e=>[e.match_id,e.id])),f=[];for(let e=0;e<t.length;e++){let n=d[o[e]];t[e].spelarar.forEach((e,t)=>{f.push({kampid:n,kasterid:e,posisjon:t+1,score_poeng:0,kamp_poeng:0,antall_ringer:0})})}let{error:p}=await y.from(`kamp_spelar`).insert(f);if(p)throw Error(`Feil ved innsetting av cup-spelarar: `+p.message);return l.length}async function oa(e,t,n,r=null){let i=[`A`,`B`,`C`],a=0;for(let o of t){let t=ra(o.spelarar,{medSeeding:n,isRunde1:!0,runde1Oppsett:o.runde1Oppsett??null}),{data:s}=await y.from(`kamp`).select(`bane_nummer`).eq(`stevneid`,e).eq(`fase`,`avsluttende`).eq(`runde_nummer`,1).not(`bane_nummer`,`is`,null).order(`bane_nummer`,{ascending:!1}).limit(1),c=s?.[0]?.bane_nummer??0,l=0;if(r&&o.gruppeNavn){let e=i.indexOf(o.gruppeNavn);for(let t=0;t<e;t++){let e=r[i[t]];e&&(l+=(e.c3??0)+(e.c2??0))}}let u=Math.max(c,l),d=o.spelarar.length===4;a+=await aa(e,t,1,o.gruppeNavn,u,d?`Semifinale`:null)}return a}async function sa(e,t,n,r){let{data:i}=await y.from(`kamp`).select(`runde_nummer`).eq(`stevneid`,e).eq(`fase`,`avsluttende`).eq(`gruppe_navn`,t).order(`runde_nummer`,{ascending:!1}).limit(1),a=(i?.[0]?.runde_nummer??0)+1,o=r,s=o.length===4,c=ra(o,{medSeeding:n,isRunde1:!1}),{data:l}=await y.from(`kamp`).select(`bane_nummer`).eq(`stevneid`,e).eq(`fase`,`avsluttende`).eq(`runde_nummer`,a).not(`bane_nummer`,`is`,null).order(`bane_nummer`,{ascending:!1}).limit(1);return{rundeNummer:a,antallKampar:await aa(e,c,a,t,l?.[0]?.bane_nummer??0,s?`Semifinale`:null)}}async function ca(e,t){let{data:n}=await y.from(`kamp`).select(`id, runde_nummer, spelarar:kamp_spelar(id, kasterid, score_poeng, posisjon, omgangar:kamp_omgang(score))`).eq(`stevneid`,e).eq(`fase`,`avsluttende`).eq(`gruppe_navn`,t).eq(`runde_navn`,`Semifinale`).eq(`er_bekreftet`,!0);if(!n?.length)throw Error(`Semifinalane er ikkje bekrefta.`);let r=n,i=r[0].runde_nummer+1,a=[],o=[];for(let e of r){let t=[...e.spelarar??[]].sort((e,t)=>{let n=e.omgangar?.length?e.omgangar.reduce((e,t)=>e+(t.score??0),0):e.score_poeng??0;return(t.omgangar?.length?t.omgangar.reduce((e,t)=>e+(t.score??0),0):t.score_poeng??0)-n});t[0]&&a.push(t[0].kasterid),t[1]&&o.push(t[1].kasterid)}let{data:s}=await y.from(`kamp`).select(`bane_nummer`).eq(`stevneid`,e).eq(`fase`,`avsluttende`).eq(`runde_nummer`,i).not(`bane_nummer`,`is`,null).order(`bane_nummer`,{ascending:!1}).limit(1),c=s?.[0]?.bane_nummer??0,l={match_id:ia(),stevneid:e,fase:`avsluttende`,runde_nummer:i,gruppe_navn:t,bane_nummer:c+1,runde_navn:`Finale`,er_bekreftet:!1,er_walkover:!1,er_tre_spelarar:!1},u={match_id:ia(),stevneid:e,fase:`avsluttende`,runde_nummer:i,gruppe_navn:t,bane_nummer:c+2,runde_navn:`Bronsefinale`,er_bekreftet:!1,er_walkover:!1,er_tre_spelarar:!1},{data:d,error:f}=await y.from(`kamp`).insert([l,u]).select(`id, runde_navn`);if(f)throw Error(`Feil: `+f.message);let p=d,m=p.find(e=>e.runde_navn===`Finale`).id,h=p.find(e=>e.runde_navn===`Bronsefinale`).id,g=[...a.filter(e=>e!=null).map((e,t)=>({kampid:m,kasterid:e,posisjon:t+1,score_poeng:0,kamp_poeng:0,antall_ringer:0})),...o.filter(e=>e!=null).map((e,t)=>({kampid:h,kasterid:e,posisjon:t+1,score_poeng:0,kamp_poeng:0,antall_ringer:0}))],{error:_}=await y.from(`kamp_spelar`).insert(g);if(_)throw Error(`Feil: `+_.message)}async function la(e){let{data:t}=await y.from(`kamp`).select(`stevneid, runde_navn, gruppe_navn`).eq(`id`,e).single();!t||t.runde_navn!==`Semifinale`||!t.gruppe_navn||await Zr(t.stevneid,t.gruppe_navn)&&await ca(t.stevneid,t.gruppe_navn)}var ua=[1,2,3,4,6];async function da(e,{id:t}){let n=t;e.replaceChildren(A(`Laster…`));let r,i;try{let[t,a]=await Promise.all([Hr(n),M()]);if(!t.data){e.replaceChildren(k(`Kamp ikkje funne.`));return}r=t.data,i=a}catch(t){b(`render:kamp`,t),e.replaceChildren(k(`Feil ved lasting av kamp.`));return}let a=(r.spelarar??[]).map(e=>e.kasterid).filter(e=>e!=null),o=await Ur(r.stevneid,a),s=document.querySelector(`.topp-header`);s&&s.classList.add(`skjult`),e.classList.add(`sb-fullskjerm-modus`),window.addEventListener(`hashchange`,()=>{s&&s.classList.remove(`skjult`),e.classList.remove(`sb-fullskjerm-modus`)},{once:!0});let c=r.spelarar??[],l=i?.profil?.kasterid??null,u=i?.profil?.rolle??null,d=u===`admin`||u===`klubbadmin`,f=l!=null&&c.some(e=>e.kasterid===l),p=c.find(e=>e.posisjon===1)??c[0]??null,m=c.find(e=>e.posisjon===2)??c[1]??null,h=r.er_tre_spelarar?c.find(e=>e.posisjon===3)??c[2]??null:null,g=p?o.get(p.kasterid)??0:0,_=m?o.get(m.kasterid)??0:0,v=r.stevne?.navn??``;function y(e,t,n){return`
    ...
    `}e.innerHTML=y(`Omgang 1`,`<div id="sb-container" class="sb-page"></div>`,`sb-omgang-tittel`),e.addEventListener(`click`,e=>{e.target.closest(`.sb-tilbake-btn`)&&(history.length>1?history.back():window.close())});let x=e.querySelector(`#sb-container`),S=e.querySelector(`#sb-omgang-tittel`);async function C(){if(d){let{data:e}=await Wr(r.stevneid,r.bane_nummer??0);return e}if(l==null)return null;let{data:e}=await Gr(r.stevneid,l);return e}async function w(e){return e.er_walkover?!1:d?e.bane_nummer===r.bane_nummer:l==null?!1:Kr(e.id,l)}function T(){sessionStorage.setItem(`ventar-neste-${n}`,`1`),e.innerHTML=y(`Fullført`,`<div class="sb-ventar-innhald">
      ...
      </div>`);let t=oi(r.stevneid,n,async e=>{await w(e)&&(await Hi(t),location.hash=`#/kamp/${e.id}`)});window.addEventListener(`hashchange`,()=>{sessionStorage.removeItem(`ventar-neste-${n}`),Hi(t)},{once:!0})}async function E(){let n=await C();n?location.hash=`#/kamp/${n.id}`:d||f?T():da(e,{id:t})}function D(t){e.querySelector(`.sb-feil-banner`)?.remove();let n=document.createElement(`div`);n.className=`sb-feil-banner alert alert-danger m-2`,n.textContent=t,e.prepend(n)}async function O(e){let t={p1:p?{spelarId:p.id,kasterid:p.kasterid,scorePoeng:p.score_poeng}:null,p2:m?{spelarId:m.id,kasterid:m.kasterid,scorePoeng:m.score_poeng}:null};if(r.fase===`avsluttende`){let{error:r}=await Jr({kampId:n,...t,orderedKasterids:e??null});if(r){D(`Feil ved bekreftelse av kamp.`);return}await la(n)}else{let{error:e}=await qr({kampId:n,...t,hcp1:g,hcp2:_,erWalkover:r.er_walkover});if(e){D(`Feil ved bekreftelse av kamp.`);return}}await E()}if(r.er_bekreftet&&sessionStorage.getItem(`ventar-neste-${n}`)){await E();return}x&&await Gi(x,r,p,m,{pointValues:ua,erArrangor:d,erDeltakar:f,onBekreft:O,onKampBekreft:d||f?E:void 0,omgangEl:S,p3ks:h,hcp1:g,hcp2:_})}function fa(e,t,n){let r=(t??[]).filter(t=>t.spelarar?.some(t=>t.kasterid===e)).sort((e,t)=>e.runde_nummer-t.runde_nummer);return r.length?r.map(t=>{let r=t.spelarar?.find(t=>t.kasterid===e),i=t.spelarar?.find(t=>t.kasterid!==e),a=t.er_walkover&&(!i||!i.kaster),o=a?`Walkover`:i?.kaster?`${j(i.kaster.fornavn)} ${j(i.kaster.etternavn)}`:`—`,s=a?``:i?.kasterid?n[i.kasterid]??``:``,c=s?`${o} (${s})`:o,l=`${a?21:Ir(r)} - ${a?0:Ir(i)}`;return`<tr>
      <td class="text-center">${t.runde_nummer}</td>
      <td class="text-center">${t.bane_nummer??``}</td>
      <td>${c}</td>
      <td class="text-center">${l}</td>
    </tr>`}).join(``):`<tr><td colspan="4" class="text-muted small fst-italic text-center">Ingen kampar</td></tr>`}function pa(e,t,n,r={}){if(e.er_bekreftet)return!1;if(e.er_walkover)return!0;if(n)return!1;let i=r[t[0]?.kasterid]??0,a=r[t[1]?.kasterid]??0,o=Ir(t[0]),s=t[1]?Ir(t[1]):0;return o+i>=21||s+a>=21}function ma(e,t){return`
    ...
    </div>`}function ha(e){return e.querySelector(`.org-hovud-innhald`)?.classList.contains(`org-vis-stilling`)?`stilling`:`kampar`}function ga(e,t){let n=e.querySelector(`.org-hovud-innhald`);n&&(n.classList.toggle(`org-vis-stilling`,t===`stilling`),e.querySelectorAll(`.org-tab-btn`).forEach(e=>{let n=e.dataset.tab===t;e.classList.toggle(`btn-primary`,n),e.classList.toggle(`btn-outline-primary`,!n)}))}function _a(e){let t=e.querySelector(`.org-hovud-innhald`);t&&e.querySelectorAll(`.org-tab-btn`).forEach(n=>{n.addEventListener(`click`,()=>{let r=n.dataset.tab===`stilling`;t.classList.toggle(`org-vis-stilling`,r),e.querySelectorAll(`.org-tab-btn`).forEach(e=>{e.classList.toggle(`btn-primary`,e.dataset.tab===n.dataset.tab),e.classList.toggle(`btn-outline-primary`,e.dataset.tab!==n.dataset.tab)})})})}function va(e,t,n,r={}){let{tableId:i=`stilling-tabell`,isAdmin:a=!1,stevneid:o=null,harHcp:s=!1,harGrupper:c=!1,harEliminasjon:l=!1,harAntallKamper:u=!1}=r,d=u?`th-32`:`th-28`,f=new Map;for(let t of e){let e=c?t.gruppe?.navn??`_`:`_`;f.has(e)||f.set(e,[]),f.get(e).push(t)}let p=f.size>1||!f.has(`_`),m=u?`${e.length} spelarar`:`Stilling`,h=[...f.entries()].sort(([e],[t])=>e===`_`?1:t===`_`?-1:e.localeCompare(t)).flatMap(([,e])=>e.map((e,t)=>({...e,posInGroup:t+1}))),g=[{label:`#`,thClass:d,cellClass:e=>l&&e.runde_eliminert!=null?`avsl-elim-plass`:void 0,render:e=>String(e.posInGroup)},{label:`S`,thClass:d,render:e=>String(e.startnummer??``)},{label:`NAMN`,render:e=>j(e.navn??`Spelar ${e.kasterid}`)},...u?[{label:`ANT.`,thClass:`th-50 stilling-tal`,cellClass:`stilling-tal`,render:e=>String(e.antall_kamper??0)}]:[],{label:`KP`,thClass:`th-44 stilling-tal`,cellClass:`stilling-tal`,render:e=>String(e.kamp_poeng??0)},{label:`SP`,thClass:`th-44 stilling-tal`,cellClass:`stilling-tal`,render:e=>String(e.score_poeng??0)},...s?[{label:`HCP`,thClass:`th-44 stilling-tal`,cellClass:e=>a?`stilling-tal stilling-hcp-celle`:`stilling-tal`,cellAttrs:a?e=>({"data-kasterid":String(e.kasterid),"data-stevneid":String(o)}):void 0,render:e=>{let t=e.hcp??0;return t>0?String(t):`—`}}]:[]],_=g.length,v=null,y=R({columns:g,rows:h,tableClass:`table table-sm kamp-tabell mb-0`,theadClass:`org-thead`,rowClass:`stilling-spelar-rad`,rowAttrs:e=>({"data-kasterid":String(e.kasterid)}),sectionHeader:e=>{let t=c?e.gruppe?.navn??`_`:`_`;if(t===v||(v=t,!p||t===`_`))return null;let n=document.createElement(`tr`),r=n.insertCell();return r.colSpan=_,r.className=`fw-semibold ps-2`,r.textContent=`Gruppe ${e.gruppe?.navn??``}`,n},detailRowClass:`stilling-detalj`,detailRowAttrs:()=>({hidden:``}),detailCellClass:`p-0`,detailRow:e=>{let r=document.createElement(`table`);r.className=`stilling-detalj-tabell table table-sm table-bordered mb-0`;let i=r.createTHead().insertRow();return[[`Runde`,!0],[`Bane`,!0],[`Motstandar`,!1],[`Resultat`,!0]].forEach(([e,t])=>{let n=document.createElement(`th`);n.textContent=e,t&&(n.className=`text-center`),i.appendChild(n)}),r.createTBody().innerHTML=fa(e.kasterid,t,n),r}});y.id=i;let b=document.createElement(`div`),x=document.createElement(`h6`);return x.className=`text-center fw-bold mb-1`,x.textContent=m,b.appendChild(x),b.appendChild(y),b.outerHTML}function ya(e,t,n=new Set){let r=e.querySelector(`#${t}`);if(!r)return;n.forEach(e=>{let t=r.querySelector(`tr.stilling-detalj[data-kasterid="${e}"]`),n=r.querySelector(`tr.stilling-spelar-rad[data-kasterid="${e}"]`);t&&t.removeAttribute(`hidden`),n&&(n.classList.add(`stilling-aktiv`),n.setAttribute(`aria-expanded`,`true`))}),r.querySelectorAll(`tr.stilling-spelar-rad`).forEach(e=>{e.setAttribute(`tabindex`,`0`),e.hasAttribute(`aria-expanded`)||e.setAttribute(`aria-expanded`,`false`)});function i(e){let t=e.dataset.kasterid;if(!t)return;let i=r.querySelector(`tr.stilling-detalj[data-kasterid="${t}"]`);if(!i)return;let a=!!i.hidden;i.hidden=!a,e.classList.toggle(`stilling-aktiv`,a),e.setAttribute(`aria-expanded`,String(a)),a?n.add(t):n.delete(t)}r.addEventListener(`click`,e=>{let t=e.target.closest(`tr.stilling-spelar-rad`);t&&i(t)}),r.addEventListener(`keydown`,e=>{if(e.key!==`Enter`&&e.key!==` `)return;let t=e.target.closest(`tr.stilling-spelar-rad`);t&&(e.preventDefault(),i(t))})}function ba(e,t,n,r,i){return function(){let a=location.hash;t.some(t=>a===`#/stevne/${e}/${t}`)?r(n,e):i()}}function xa(e,t){return`
  ...
  `}function Sa(e,t){let{alleInnlBekrefta:n,harAvslKampar:r,harGruppefordeling:i,harPrekonfigurertFormat:a=!1}=t,o=e.stevne_fase,s=``;return o===`avsluttende`?i&&i&&!r&&(s=`<button id="endre-gruppeinndeling-btn" class="btn btn-sm btn-outline-secondary">Endre gruppeinndeling</button>`):s=n?`
  ...
  `}function Ca(e,t){let n={},r=new Set;for(let i of e){let[,e]=i.er_walkover?Fr(i.spelarar,t):[null,null];for(let a of i.spelarar??[])!a.kasterid||!a.kaster||i.er_walkover&&a.kasterid===e?.kasterid||(r.add(a.kasterid),n[a.kasterid]||(n[a.kasterid]={kasterid:a.kasterid,navn:`${a.kaster.fornavn} ${a.kaster.etternavn}`,startnummer:t[a.kasterid]??null,kamp_poeng:0,score_poeng:0,antall_kamper:0}),i.er_bekreftet&&(n[a.kasterid].kamp_poeng+=a.kamp_poeng,n[a.kasterid].score_poeng+=a.score_poeng,n[a.kasterid].antall_kamper+=1))}return{spelMap:n,ekteKasterids:r}}function wa(e,t,n,r){let{spelMap:i}=Ca(e,r);return Ta(t.map(e=>({kasterid:e.kasterid,navn:n[e.kasterid]??`Spelar ${e.kasterid}`,startnummer:e.startnummer,plassering:e.plassering,runde_eliminert:e.runde_eliminert,kamp_poeng:i[e.kasterid]?.kamp_poeng??0,score_poeng:i[e.kasterid]?.score_poeng??0,gruppe:e.gruppe})),e)}function Ta(e,t){let n=t.filter(e=>e.er_bekreftet);return[...e].sort((e,t)=>{if(e.plassering!=null&&t.plassering!=null)return e.plassering-t.plassering;let r=e.runde_eliminert==null;if(r!==(t.runde_eliminert==null))return r?-1:1;if(!r){let n=(t.runde_eliminert??0)-(e.runde_eliminert??0);if(n!==0)return n}let i=e.plassering??1/0,a=t.plassering??1/0;if(i!==a)return i-a;if(t.kamp_poeng!==e.kamp_poeng)return(t.kamp_poeng??0)-(e.kamp_poeng??0);if(t.score_poeng!==e.score_poeng)return(t.score_poeng??0)-(e.score_poeng??0);let o=0,s=0;for(let r of n){let n=r.spelarar?.find(t=>t.kasterid===e.kasterid),i=r.spelarar?.find(e=>e.kasterid===t.kasterid);n&&i&&(o+=n.kamp_poeng??0,s+=i.kamp_poeng??0)}if(o!==s)return s-o;let c=e=>n.flatMap(t=>t.spelarar?.filter(t=>t.kasterid===e)??[]).map(e=>Ir(e)).sort((e,t)=>t-e),l=c(e.kasterid),u=c(t.kasterid);for(let e=0;e<Math.min(l.length,u.length);e++)if(u[e]!==l[e])return u[e]-l[e];return(e.startnummer??1/0)-(t.startnummer??1/0)})}function Ea(){return crypto.randomUUID()}async function Da(e,t,n){let{data:r,error:i}=await y.from(`pamelding`).select(`id, kasterid, kaster(klubbid)`).eq(`stevneid`,e).order(`id`);if(i)throw Error(`Feil ved henting av påmelding: `+i.message);if(!r?.length)throw Error(`Ingen spelarar påmelde.`);for(let e=r.length-1;e>0;e--){let t=Math.floor(Math.random()*(e+1));[r[e],r[t]]=[r[t],r[e]]}let a=r.length,o={},s=r.map((t,n)=>{o[n+1]=t.kasterid;let r=t.kaster?.klubbid??null;return{stevneid:e,kasterid:t.kasterid,klubbid:r,startnummer:n+1}});await y.from(`resultat`).delete().eq(`stevneid`,e);let{error:c}=await y.from(`resultat`).insert(s);if(c)throw Error(`Feil ved lagring av startnummer: `+c.message);return t.toLowerCase().includes(`gloppen`)?Oa(e,o,a,n):ka(e,o,a)}async function Oa(e,t,n,r){let i=(n%2==0?n:n+1)/2,a=0;for(let o=1;o<=r;o++){let r=[],s=[];for(let t=1;t<=i;t++){let a=(t-1+o-1)%i+1,c=(t-1+2*(o-1))%i+1+i,l=c>n;r.push({match_id:Ea(),stevneid:e,fase:`innledende`,runde_nummer:o,bane_nummer:t,er_bekreftet:!1,er_walkover:l}),s.push({p1Pos:a,p2Pos:c,erWalkover:l})}let{data:c,error:l}=await y.from(`kamp`).insert(r).select(`id, bane_nummer`);if(l)throw Error(`Feil ved innsetting av kampar (runde ${o}): `+l.message);let u=Object.fromEntries(c.map(e=>[e.bane_nummer,e.id])),d=[];for(let e=0;e<i;e++){let n=u[e+1],{p1Pos:r,p2Pos:i,erWalkover:a}=s[e];d.push({kampid:n,kasterid:t[r],posisjon:1,score_poeng:0,kamp_poeng:0,antall_ringer:0}),a||d.push({kampid:n,kasterid:t[i],posisjon:2,score_poeng:0,kamp_poeng:0,antall_ringer:0})}let{error:f}=await y.from(`kamp_spelar`).insert(d);if(f)throw Error(`Feil ved innsetting av spelarar (runde ${o}): `+f.message);a+=c.length}return a}async function ka(e,t,n){let r=[],i=[],a=1;for(let t=1;t<=n;t+=2){let o=t+1>n;r.push({match_id:Ea(),stevneid:e,fase:`innledende`,runde_nummer:1,bane_nummer:a,er_bekreftet:!1,er_walkover:o}),i.push({p1Pos:t,p2Pos:o?null:t+1,erWalkover:o}),a++}let{data:o,error:s}=await y.from(`kamp`).insert(r).select(`id, bane_nummer`);if(s)throw Error(`Feil ved innsetting av Swiss runde 1: `+s.message);let c=Object.fromEntries(o.map(e=>[e.bane_nummer,e.id])),l=[];for(let e=0;e<i.length;e++){let n=c[e+1],{p1Pos:r,p2Pos:a,erWalkover:o}=i[e];l.push({kampid:n,kasterid:t[r],posisjon:1,score_poeng:0,kamp_poeng:0,antall_ringer:0}),o||l.push({kampid:n,kasterid:t[a],posisjon:2,score_poeng:0,kamp_poeng:0,antall_ringer:0})}let{error:u}=await y.from(`kamp_spelar`).insert(l);if(u)throw Error(`Feil ved innsetting av Swiss spelarar: `+u.message);return o.length}async function Aa(e){let{data:t,error:n}=await y.from(`kamp`).select(`id, runde_nummer, er_bekreftet, er_walkover, spelarar:kamp_spelar(kasterid, kamp_poeng, score_poeng, posisjon)`).eq(`stevneid`,e).eq(`fase`,`innledende`).order(`runde_nummer`);if(n)throw Error(`Feil ved henting av kampar: `+n.message);let r=t,i=Math.max(...t.map(e=>e.runde_nummer))+1,a=new Set;for(let e of r)for(let t of e.spelarar??[])t.kasterid!=null&&a.add(t.kasterid);let o=[...a],s={};for(let e of o)s[e]=o.filter(t=>t!==e);for(let e of r){let t=(e.spelarar??[]).filter(e=>e.kasterid!=null);if(t.length===2){let e=t[0].kasterid,n=t[1].kasterid;s[e]=s[e].filter(e=>e!==n),s[n]=s[n].filter(t=>t!==e)}}let c={};for(let e of o)c[e]=0;for(let e of t){if(!e.er_walkover)continue;let t=(e.spelarar??[]).find(e=>e.posisjon===1);t?.kasterid!=null&&(c[t.kasterid]=(c[t.kasterid]??0)+1)}let l=Ta(o.map(e=>{let n=0,i=0;for(let t of r){let r=(t.spelarar??[]).find(t=>t.kasterid===e);r&&(n+=r.kamp_poeng??0,i+=0)}for(let n of t){let t=(n.spelarar??[]).find(t=>t.kasterid===e);t&&(i+=t.score_poeng??0)}return{kasterid:e,kamp_poeng:n,score_poeng:i}}),r);function u(e){for(let t=e.length-1;t>=0;t--)if((c[e[t].kasterid]??0)<1)return e[t];return null}function d(e,t){if(e.length===0)return t;if(e.length%2==1){let n=u(e);return n?(c[n.kasterid]++,t.push({p1:n.kasterid,p2:null,erWalkover:!0}),d(e.filter(e=>e.kasterid!==n.kasterid),t)||(c[n.kasterid]--,t.pop(),null)):null}for(let n=0;n<e.length;n++){let r=e[n];for(let i=n+1;i<e.length;i++){let n=e[i];if(s[r.kasterid]?.includes(n.kasterid)){t.push({p1:r.kasterid,p2:n.kasterid,erWalkover:!1});let i=d(e.filter(e=>e.kasterid!==r.kasterid&&e.kasterid!==n.kasterid),t);if(i)return i;t.pop()}}}return null}let f=d(l,[]);if(!f)throw Error(`Paring er ikkje mogleg. Alle moglege motstandarar er allereie spela.`);f.sort((e,t)=>!!e.erWalkover-+!!t.erWalkover);let p=f.map((t,n)=>({match_id:Ea(),stevneid:e,fase:`innledende`,runde_nummer:i,bane_nummer:n+1,er_bekreftet:!1,er_walkover:t.erWalkover})),{data:m,error:h}=await y.from(`kamp`).insert(p).select(`id, bane_nummer`);if(h)throw Error(`Feil ved innsetting av ny Swiss-runde: `+h.message);let g=Object.fromEntries(m.map(e=>[e.bane_nummer,e.id])),_=[];for(let e=0;e<f.length;e++){let{p1:t,p2:n,erWalkover:r}=f[e],i=g[e+1];_.push({kampid:i,kasterid:t,posisjon:1,score_poeng:0,kamp_poeng:0,antall_ringer:0}),r||_.push({kampid:i,kasterid:n,posisjon:2,score_poeng:0,kamp_poeng:0,antall_ringer:0})}let{error:v}=await y.from(`kamp_spelar`).insert(_);if(v)throw Error(`Feil ved innsetting av Swiss spelarar: `+v.message);return{rundeNummer:i,antallKampar:m.length}}async function ja(e,{id:t,isAdmin:n=!1},r=null){e.replaceChildren(A());try{let[i,a,o]=await Promise.all([fe(t),Or(t),M()]);if(i.error||!i.data){e.replaceChildren(k(`Stevne ikkje funne.`));return}let s=i.data,c=s.stevne_fase??null,l=c===null||c===`ikke_startet`,u=s.kastemetodeInnl?.navn??`—`,d=u.toLowerCase().includes(`gloppen`);if(r&&l&&n){r.innerHTML=`<button id="start-stevne-btn" class="btn btn-sm btn-success">Start stevne</button>`;let e=r.querySelector(`#start-stevne-btn`);e.addEventListener(`click`,async()=>{if(a<2){Q(`Stevnet må ha minst 2 spelarar for å startast.`,`error`);return}if(d&&!s.antall_runder_innl){Q(`Du må setje antal rundar for innledande fase. Gå til Innstillingar for å endre.`,`error`);return}let n=await kr(t);if(n>0&&!await X({title:`Ubekrefta spelarar`,message:`${n} spelar(ar) er ikkje bekrefta. Vil du starte stevnet likevel?`}))return;e.disabled=!0,e.textContent=`Starter…`;try{await Da(t,u,s.antall_runder_innl??1)}catch(t){Q(`Feil ved kampgenerering: `+(t instanceof Error?t.message:String(t)),`error`),e.disabled=!1,e.textContent=`Start stevne`;return}let{error:r}=await pe(t,`innledende`);if(r){Q(`Feil ved oppdatering av fase.`,`error`),e.disabled=!1,e.textContent=`Start stevne`;return}location.hash=`#/stevne/${t}/innledende`})}e.innerHTML=`
      ...
      <div id="info-handling-knapper" class="mb-3 d-flex gap-2 flex-wrap"></div>`;let f=e.querySelector(`#info-handling-knapper`),p=s.erfullfort??!1;if(o?.profil?.kobling_status===`godkjent`&&!p){let e=document.createElement(`a`);e.href=`#/stevne/${t}/pamelding`,e.className=`btn btn-sm btn-primary`,e.textContent=`Meld deg på`,f.appendChild(e)}let m=document.createElement(`a`);m.href=`#/stevne/${t}/pamelding`,m.className=`btn btn-sm btn-outline-secondary`,m.textContent=`Sjå påmeldingar`,f.appendChild(m)}catch(t){b(`stevne-info.render`,t),e.replaceChildren(k(`Kunne ikkje laste info.`))}}function Ma(e){return[...e].sort((e,t)=>{let n=(e.klubb?.navn??``).localeCompare(t.klubb?.navn??``,`nb`);if(n!==0)return n;let r=(e.etternavn??``).localeCompare(t.etternavn??``,`nb`);return r===0?(e.fornavn??``).localeCompare(t.fornavn??``,`nb`):r})}function Na(e,t,n){let r=t.toLowerCase();return e.filter(e=>n.has(e.id)?!1:!r||a(e).toLowerCase().includes(r)||(e.klubb?.navn??``).toLowerCase().includes(r))}function Pa(e){let t=document.createElement(`div`);t.className=`d-flex flex-column flex-grow-1`;let n=document.createElement(`h6`);n.textContent=e,n.className=`fw-bold mb-1`;let r=document.createElement(`div`);r.className=`border rounded deltaker-tabell-wrapper flex-grow-1 overflow-auto`;let i=document.createElement(`table`);return i.className=`table table-sm table-hover table-bordered mb-0`,r.appendChild(i),t.appendChild(n),t.appendChild(r),{kolonne:t,tabell:i,tittelEl:n}}function Fa(e,t,n,r,i){let o=document.createElement(`tr`),s=document.createElement(`td`);if(s.className=`text-center th-40`,t){let e=document.createElement(`span`);e.className=`text-success fw-bold`,e.textContent=`✓`,s.appendChild(e)}else if(!i){let t=document.createElement(`button`);t.textContent=`✓`,t.className=`btn btn-outline-danger btn-sm rounded-circle p-0 lh-1 deltaker-bekreft-btn`,t.title=`Bekreft spelar`,t.addEventListener(`click`,t=>{t.stopPropagation(),r(e)}),s.appendChild(t)}let c=document.createElement(`td`);c.textContent=a(e);let l=document.createElement(`td`);l.textContent=e.klubb?.navn??``;let u=document.createElement(`td`);if(u.className=`text-center th-40`,!i){let t=document.createElement(`button`);t.innerHTML=`&times;`,t.className=`btn btn-danger btn-sm rounded-circle p-0 lh-1 deltaker-fjern-btn`,t.title=`Fjern spelar`,t.addEventListener(`click`,t=>{t.stopPropagation(),n(e)}),u.appendChild(t)}return o.appendChild(s),o.appendChild(c),o.appendChild(l),o.appendChild(u),o}function Ia(e,t,n){let r=document.createElement(`tr`),i=document.createElement(`td`);i.textContent=a(e);let o=document.createElement(`td`);return o.textContent=e.klubb?.navn??`Ingen klubb`,n||(r.classList.add(`deltaker-rad`),r.addEventListener(`click`,()=>t(e))),r.appendChild(i),r.appendChild(o),r}function La(e,t){let n=document.createElement(`tr`),r=document.createElement(`td`);return r.className=`text-center text-muted fst-italic py-3`,r.textContent=e,r.colSpan=t,n.appendChild(r),n}async function Ra(e,{id:t,isAdmin:n=!1}){e.replaceChildren(A());try{let[r,i,a]=await Promise.all([Ce(t),tn(),Ar(t)]);if(r.error||!r.data){e.replaceChildren(k(`Stevne ikkje funne.`));return}if(i.error){e.replaceChildren(k(`Kunne ikkje laste kasterliste.`));return}let o=r.data.stevne_fase??null,s=n&&(o===null||o===`ikke_startet`),c=i.data,l=new Map;for(let e of a.data)e.kasterid!=null&&l.set(e.kasterid,e.er_bekreftet??!1);e.innerHTML=`
      <div>
      ...
      </div>`;let u=e.querySelector(`#spelarar-layout`),d=document.createElement(`div`);d.className=`col-md-6 d-flex flex-column`;let f=document.createElement(`input`);f.type=`text`,f.placeholder=`Søk etter navn eller klubb…`,f.className=`form-control mb-2`;let{kolonne:p,tabell:m}=Pa(`Tilgjengelege spelarar`);d.appendChild(f),d.appendChild(p),u.appendChild(d);let h=document.createElement(`div`);h.className=`col-md-6 d-flex flex-column`;let g=document.createElement(`input`);g.type=`text`,g.className=`form-control mb-2 deltaker-search-spacer`,g.tabIndex=-1,g.disabled=!0;let{kolonne:_,tabell:v,tittelEl:y}=Pa(`Påmelde spelarar`);h.appendChild(g),h.appendChild(_),u.appendChild(h);function b(){v.innerHTML=``;let e=Ma(c.filter(e=>l.has(e.id)));if(y.textContent=`Påmelde spelarar: ${e.length}`,!e.length){v.appendChild(La(`Ingen spelarar påmelde`,4));return}for(let n of e)v.appendChild(Fa(n,l.get(n.id)??!1,async e=>{let{error:n}=await Nr(t,e.id);if(n){Q(`Feil ved fjerning: `+(n instanceof Error?n.message:String(n)),`error`);return}l.delete(e.id),b(),x()},async e=>{let{error:n}=await Mr(t,e.id);if(n){Q(`Feil ved bekreftelse: `+(n instanceof Error?n.message:String(n)),`error`);return}l.set(e.id,!0),b()},!s))}function x(){let e=Ma(Na(c,f.value,l));if(m.innerHTML=``,!e.length){m.appendChild(La(`Ingen spelarar funne`,2));return}for(let n of e)m.appendChild(Ia(n,async e=>{let{error:n}=await jr(t,e.id);if(n){Q(`Feil ved innmelding: `+(n instanceof Error?n.message:String(n)),`error`);return}l.set(e.id,!1),b(),x()},!s))}f.addEventListener(`input`,x),b(),x()}catch(t){b(`stevne-deltakere.render`,t),e.replaceChildren(k(`Kunne ikkje laste deltakarliste.`))}}var za=`modulepreload`,Ba=function(e){return`/dev/`+e},Va={},Ha=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}r=o(t.map(t=>{if(t=Ba(t,n),t in Va)return;Va[t]=!0;let r=t.endsWith(`.css`),i=r?`[rel="stylesheet"]`:``;if(n)for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${t}"]${i}`))return;let o=document.createElement(`link`);if(o.rel=r?`stylesheet`:za,r||(o.as=`script`),o.crossOrigin=``,o.href=t,a&&o.setAttribute(`nonce`,a),document.head.appendChild(o),r)return new Promise((e,n)=>{o.addEventListener(`load`,e),o.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})};async function Ua(e,{id:t,isAdmin:n=!1},r=null){e.replaceChildren(A());let{navn:i,error:a}=await Me(t);if(a){e.replaceChildren(k(`Stevne ikkje funne.`));return}if(i.includes(`gloppen`)){let{render:i}=await Ha(async()=>{let{render:e}=await import(`./gloppen-KDgweHCD.js`);return{render:e}},__vite__mapDeps([0,1,2]));await i(e,{id:t,isAdmin:n},r)}else if(i.includes(`nordhordland`)){let{render:i}=await Ha(async()=>{let{render:e}=await import(`./nordhordland-BRh5CA_F.js`);return{render:e}},__vite__mapDeps([3,1,2]));await i(e,{id:t,isAdmin:n},r)}else if(i.includes(`x-kast`)||i.includes(`minimatch`)||i.includes(`halvmatch`)||i.includes(`heilmatch`)){let{render:i}=await Ha(async()=>{let{render:e}=await import(`./xkast-Bc1GXdlI.js`);return{render:e}},[]);await i(e,{id:t,isAdmin:n},r)}else e.replaceChildren(k(`Ukjend innledande kastemetode: ${i||`(ikkje sett)`}`))}async function Wa(e,{id:t,isAdmin:n=!1},r=null){e.replaceChildren(A());let{navn:i,error:a}=await Ne(t);if(a){e.replaceChildren(k(`Stevne ikkje funne.`));return}if(i.includes(`cup`)){let{render:i}=await Ha(async()=>{let{render:e}=await import(`./cup-C6fcTVop.js`);return{render:e}},__vite__mapDeps([4,2]));await i(e,{id:t,isAdmin:n},r)}else if(i.includes(`kongelag`)){let{render:i}=await Ha(async()=>{let{render:e}=await import(`./kongelag-DkOjTXQA.js`);return{render:e}},[]);await i(e,{id:t,isAdmin:n},r)}else if(i.includes(`nordhordland`)){let{render:i}=await Ha(async()=>{let{render:e}=await import(`./nordhordland-rVImhQrj.js`);return{render:e}},[]);await i(e,{id:t,isAdmin:n},r)}else e.replaceChildren(k(`Ukjend avsluttande kastemetode: ${i||`(ikkje sett)`}`))}y.from(`kamp`).select(`id, er_walkover, spelarar:kamp_spelar(id, kasterid)`);function Ga(){let e=Math.floor(Math.random()*27),t=Math.floor(Math.random()*27);return e<21&&t<21?Math.random()<.5?[Math.floor(Math.random()*6)+21,t]:[e,Math.floor(Math.random()*6)+21]:[e,t]}async function Ka(e){let{data:t,error:n}=await y.from(`kamp`).select(`id, er_walkover, spelarar:kamp_spelar(id, kasterid)`).eq(`stevneid`,e).eq(`fase`,`innledende`).eq(`er_bekreftet`,!1);if(n){b(`autoFullforInnledendeKamper`,n);return}if(t?.length)for(let e of t){let[t,n]=e.spelarar??[],[r,i]=e.er_walkover?[21,0]:Ga(),[a,o]=Pr(r,i);try{let s=[y.from(`kamp`).update({er_bekreftet:!0}).eq(`id`,e.id)];t&&s.push(y.from(`kamp_spelar`).update({score_poeng:r,kamp_poeng:a}).eq(`id`,t.id)),n&&s.push(y.from(`kamp_spelar`).update({score_poeng:i,kamp_poeng:o}).eq(`id`,n.id)),await Promise.all(s)}catch(e){b(`autoFullforInnledendeKamper:update`,e)}}}async function qa(e,t){let{data:n,error:r}=await y.from(`kamp`).select(`id`).eq(`stevneid`,e).eq(`fase`,t);if(r){b(`slettKamperForFase:kamp`,r);return}let i=(n??[]).map(e=>e.id);if(!i.length)return;let{data:a,error:o}=await y.from(`kamp_spelar`).select(`id`).in(`kampid`,i);if(o){b(`slettKamperForFase:spelar`,o);return}let s=(a??[]).map(e=>e.id);if(s.length){let{error:e}=await y.from(`kamp_omgang`).delete().in(`kamp_spelar_id`,s);if(e){b(`slettKamperForFase:omgang`,e);return}let{error:t}=await y.from(`kamp_spelar`).delete().in(`kampid`,i);if(t){b(`slettKamperForFase:spelarDel`,t);return}}let{error:c}=await y.from(`kamp`).delete().in(`id`,i);c&&b(`slettKamperForFase:kampDel`,c)}async function Ja(e){await qa(e,`avsluttende`),await qa(e,`innledende`);let{error:t}=await y.from(`resultat`).delete().eq(`stevneid`,e);if(t){b(`nullstillStevne:resultat`,t);return}let{error:n}=await y.from(`stevne`).update({stevne_fase:`ikke_startet`,runde1_format:null}).eq(`id`,e);n&&b(`nullstillStevne:stevne`,n)}async function Ya(e,{id:t}){e.replaceChildren(A());try{let[n,r]=await Promise.all([Oe(t),ke()]);if(n.error||!n.data){e.replaceChildren(k(`Stevne ikkje funne.`));return}let i=n.data,a=r.data,o=a.filter(e=>e.er_innledende),s=a.filter(e=>e.er_avsluttende);function c(e,t){return e.map(e=>`<option value="${e.id}"${e.id===t?` selected`:``}>${j(e.navn)}</option>`).join(``)}e.innerHTML=`
      <div>
  ...
  `);async function Xa(e){let{data:t,error:n}=await y.from(`stevne`).select(`
      id, navn, sted, dato, erfullfort, resultaturl, juryleder, klubbid,
      stevnetype:stevnetypeid(navn),
      kategori:kategoriid(navn),
      kontakt:kontaktkasterid(fornavn, etternavn),
      innledende:innledendekastemetodeid(navn),
      avsluttende:avsluttendekastemetodeid(navn)
    `).eq(`id`,e).maybeSingle();return n&&b(`hentStevneMedDetaljer`,n),{data:t,error:n}}y.from(`resultat`).select(`kasterid, startnummer, hcp`);async function Za(e){let{data:t,error:n}=await y.from(`resultat`).select(`kasterid, startnummer, hcp`).eq(`stevneid`,e);return n&&b(`hentResultatForInnledende`,n),{data:t??[],error:n}}async function Qa(e,t,n){let{error:r}=await y.from(`resultat`).update({hcp:n}).eq(`stevneid`,e).eq(`kasterid`,t);return r&&b(`oppdaterResultatHcp`,r),{error:r}}y.from(`resultat`).select(`
  kasterid, startnummer, plassering, runde_eliminert,
  gruppe:gruppeid(id, navn)
`);async function $a(e){let{data:t,error:n}=await y.from(`resultat`).select(`
      kasterid, startnummer, plassering, runde_eliminert,
      gruppe:gruppeid(id, navn)
    `).eq(`stevneid`,e);return n&&b(`hentResultatForAvsluttende`,n),{data:t??[],error:n}}async function eo(e){let{data:t,error:n}=await y.from(`gruppe`).select(`id, navn`).in(`navn`,e);return n&&b(`hentGrupper`,n),{data:t??[],error:n}}async function to(e,t){if(!t.length)return{error:null};let n=(await Promise.all(t.map(t=>y.from(`resultat`).update({gruppeid:t.gruppeid}).eq(`stevneid`,e).eq(`kasterid`,t.kasterid)))).find(e=>e.error)?.error??null;return n&&b(`setGruppeInndeling`,n),{error:n}}async function no(e,t){if(!t.length)return{error:null};let n=(await Promise.all(t.map((t,n)=>y.from(`resultat`).update({plassering:n+1}).eq(`stevneid`,e).eq(`kasterid`,t.kasterid)))).find(e=>e.error)?.error??null;return n&&b(`skrivPlaseringar`,n),{error:n}}async function ro(e){let{error:t}=await y.from(`resultat`).update({gruppeid:null}).eq(`stevneid`,e);return t&&b(`clearGruppeInndeling`,t),{error:t}}async function io(e){let{data:t,error:n}=await y.from(`resultat`).select(`
      plassering, nc_poeng,
      kaster:kasterid(id, fornavn, etternavn),
      klubb:klubbid(navn),
      klasse:klasseid(navn),
      gruppe:gruppeid(navn)
    `).eq(`stevneid`,e).order(`plassering`);return n&&b(`hentResultaterForStevne`,n),{data:t??[],error:n}}function ao(e,t){let n=new Map;for(let r of e){let e=r.gruppe?.navn??`–`,i=r.klasse?.navn??null,a=t?`${i??``}|${e}`:e,o=t?`${i?i+` `:``}${e}`:e;n.has(a)||n.set(a,{label:o,rader:[]}),n.get(a).rader.push(r)}return[...n.values()].sort((e,t)=>e.label.localeCompare(t.label,`nb`))}function oo(e){let t=e.rader.map(e=>`
    <div class="res-rad">
    ...
    </div>`}function so(e){let t=e.rader.map(e=>{let t=e.kaster,n=t?`<a href="#/kastere/${s(t)}" class="res-kaster-lenke">${j(a(t))}</a>`:`–`;return`
      <tr>
    ...
    <div class="res-tabell-seksjon">
    ...
    </div>`}async function co(e,{id:t}){e.replaceChildren(A(`Laster resultat…`));try{let[n,r]=await Promise.all([Xa(t),io(t)]);if(n.error||!n.data){e.replaceChildren(k(`Kunne ikkje laste stevnet.`));return}if(r.error){e.replaceChildren(k(`Kunne ikkje laste resultat.`));return}let i=n.data,a=r.data;if(!a.length){e.replaceChildren(L(i.erfullfort?`Ingen resultat registrert.`:`Turneringa er ikkje avslutta enno.`));return}let o=ao(a,(i.dato?new Date(i.dato+`T12:00:00`).getFullYear():9999)<2026),s=a.length;e.innerHTML=`
      <div class="res-side">
      ...
      </div>`}catch(t){b(`stevne-resultat.render`,t),e.replaceChildren(k(`Kunne ikkje laste resultat.`))}}var lo=[{key:`info`,label:`Info`,adminOnly:!1},{key:`deltakere`,label:`Deltakere`,adminOnly:!0},{key:`innledende`,label:`Innledande`,adminOnly:!1},{key:`avsluttende`,label:`Avsluttande`,adminOnly:!1},{key:`resultat`,label:`Sluttresultat`,adminOnly:!1},{key:`innstillinger`,label:`Innstillingar`,adminOnly:!0}],uo=new Set(lo.filter(e=>e.adminOnly).map(e=>e.key)),fo={info:ja,deltakere:Ra,innledende:Ua,avsluttende:Wa,innstillinger:Ya,resultat:co},po={ikke_startet:`<span class="badge bg-secondary">Ikkje starta</span>`,innledende:`<span class="badge bg-primary">Innledande fase</span>`,avsluttende:`<span class="badge bg-success">Avsluttande fase</span>`};function mo(e,t,n,r){return`<ul class="nav nav-tabs mb-3">${lo.filter(e=>n||!e.adminOnly).filter(e=>e.key!==`avsluttende`||r).map(({key:n,label:r})=>`
      <li class="nav-item">
        <a class="nav-link${t===n?` active`:``}"
           href="#/stevne/${e}/${n}">${r}</a>
      </li>`).join(``)}</ul>`}var ho=null;async function go(e,{id:t,tab:n=`info`}){ho&&=(await Hi(ho),null),e.replaceChildren(A());try{let{data:r,error:i}=await Ce(t);if(i||!r){e.replaceChildren(k(`Stevne ikkje funne.`));return}let a=await N()||await at(),o=r.avsluttendekastemetodeid!=null,s=!a&&uo.has(n)?`info`:n,c=po[r.stevne_fase??`ikke_startet`]??``;e.innerHTML=`
      ...
      </div>`;let l=e.querySelector(`#org-banner-knappar`),u=e.querySelector(`#org-subside`);await(fo[s]??ja)(u,{id:t,isAdmin:a},l),ho=we(t,t=>{let n=e.querySelector(`#fase-badge`);n&&(n.innerHTML=po[t??`ikke_startet`]??``)})}catch(t){b(`stevne.render`,t),e.replaceChildren(k(`Kunne ikkje laste stevnet.`))}}{let e=document.querySelector(`.header-versjon`);if(e){e.textContent+=` [DEV]`;let t=document.createElement(`span`);t.className=`dev-banner`,t.textContent=`TEST`,e.after(t)}}var _o=document.getElementById(`app`);function vo(e,t){return async(n,r)=>{if(!await M()){location.hash=`#/logginn`;return}if(e===`admin`&&!await N()){n.replaceChildren(k(`Ingen tilgang.`));return}if(e===`klubbadmin`&&!await N()&&!await at()){n.replaceChildren(k(`Ingen tilgang.`));return}await t(n,r)}}var yo=[{pattern:/^\/logginn$/,side:Cr,params:()=>({})},{pattern:/^\/minside$/,side:vo(`bruker`,_i),params:()=>({})},{pattern:/^\/admin$/,side:vo(`admin`,Si),params:()=>({})},{pattern:/^\/stevne\/ny$/,side:vo(`klubbadmin`,Ni),params:()=>({})},{pattern:/^\/stevne\/(\d+)\/admin$/,side:vo(`klubbadmin`,Ni),params:e=>({id:e[1]})},{pattern:/^\/kamp\/(\d+)$/,side:da,params:e=>({id:Number(e[1])})},{pattern:/^\/stevne\/(\d+)\/pamelding$/,side:Vi,params:e=>({id:e[1]})},{pattern:/^\/stevne\/(\d+)(?:\/([^/]*))?$/,side:go,params:e=>({id:Number(e[1]),tab:e[2]??`info`})},{pattern:/^\/kaster\/ny$/,side:vo(`klubbadmin`,Pi),params:()=>({})},{pattern:/^\/kaster\/(\d+)\/admin$/,side:vo(`klubbadmin`,Pi),params:e=>({id:e[1]})},{pattern:/^\/klubber\/(\d+)\/admin$/,side:vo(`klubbadmin`,Fi),params:e=>({id:e[1]})},{pattern:/^\/terminliste$/,side:Ct,params:()=>({})},{pattern:/^\/norgescupen$/,side:It,params:()=>({})},{pattern:/^\/norgesranking$/,side:Jt,params:()=>({})},{pattern:/^\/rekorder$/,side:ar,params:()=>({})},{pattern:/^\/nmvinnere$/,side:br,params:()=>({})},{pattern:/^\/kastere\/(\d+)(-[^/]*)?$/,side:In,params:e=>({id:e[1]})},{pattern:/^\/kastere$/,side:In,params:()=>({})},{pattern:/^\/klubber\/(\d+)(-[^/]*)?$/,side:Zn,params:e=>({id:e[1]})},{pattern:/^\/klubber$/,side:Zn,params:()=>({})},{pattern:/^\/?$/,side:Be,params:()=>({})}];function bo(){let e=location.hash.replace(/^#/,``)||`/`;for(let t of yo){let n=e.match(t.pattern);if(n){t.side(_o,t.params(n));return}}_o.replaceChildren(k(`Side ikkje funne.`))}async function xo(){let e=await M(),t=document.getElementById(`meny-logginn-item`),n=document.getElementById(`meny-minside-item`),r=document.getElementById(`meny-admin-item`),i=document.getElementById(`meny-loggut-item`);if(e){t.classList.add(`d-none`);let a=e.profil?.rolle===`admin`;n.classList.toggle(`d-none`,a),r.classList.toggle(`d-none`,!a),i.classList.remove(`d-none`)}else t.classList.remove(`d-none`),n.classList.add(`d-none`),r.classList.add(`d-none`),i.classList.add(`d-none`)}window.addEventListener(`hashchange`,bo),document.addEventListener(`DOMContentLoaded`,()=>{document.getElementById(`menyLoggUtKnapp`).addEventListener(`click`,async()=>{await ot(),location.hash=`#/`}),xo(),bo()}),document.addEventListener(`authStateChanged`,()=>{xo()});export{A as $,Hi as A,ei as B,oa as C,ta as D,na as E,zr as F,Ir as G,si as H,Yr as I,De as J,Te as K,Rr as L,$r as M,qr as N,$i as O,Zr as P,j as Q,Xr as R,Ta as S,sa as T,Pr as U,Br as V,Fr as W,Ee as X,pe as Y,Fe as Z,Sa as _,Qa as a,va as b,Ka as c,ya as d,k as et,_a as f,ba as g,ha as h,Za as i,X as j,Q as k,Aa as l,Ca as m,eo as n,to as o,wa as p,Pe as q,$a as r,no as s,ro as t,b as tt,pa as u,ma as v,ca as w,ga as x,xa as y,Vr as z};