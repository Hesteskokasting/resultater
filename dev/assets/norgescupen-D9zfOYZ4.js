  ...
  <p>Kun klasse 1. Dei 4 beste poengsummene frå kvar klubb er teljande.</p>`;function H(e,t,n){return`
    ...
    <div class="content-page res-side">
      ...
      <div id="nc-content"></div>
        ...
        </section>`;let e=l.querySelector(`#nc-single-table-container`),s=c?O(z.results,z.tournaments,c,a,o,t):[];s.length?(e.innerHTML=G(s),m(e)):e.replaceChildren(r(c?`Ingen resultater funnet.`:`Ingen data.`)),l.querySelector(`#nc-single-section`).addEventListener(`click`,e=>{let t=e.target.closest(`[data-class]`);t&&(R.classNum=Number(t.dataset.class),i())})}}i(),e.querySelector(`#nc-year`).addEventListener(`change`,async t=>{R.year=Number(t.target.value),R.classNum=1,y(R),e.querySelector(`#nc-cuptype`).value=R.cupType,e.querySelector(`#nc-content`).replaceChildren(s());let n=await F(R.year);if(!n){e.querySelector(`#nc-content`).replaceChildren(o(`Feil ved henting av data.`));return}z=n,i()}),e.querySelector(`#nc-cuptype`).addEventListener(`change`,e=>{R.cupType=e.target.value,R.classNum=1,y(R),i()}),e.querySelector(`#nc-view-tabs-container`).addEventListener(`click`,e=>{let t=e.target.closest(`[data-view]`);t&&(R.view=t.dataset.view,i())})}export{J as render};