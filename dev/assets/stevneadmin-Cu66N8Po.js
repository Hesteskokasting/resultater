import{t as e}from"./logError-Cjb5zwtM.js";import{a as t,i as n}from"./authService-9qLZxJYd.js";import{$ as r,A as i,C as a,E as o,G as s,J as c,P as l,Q as u,S as d,W as f,q as p,w as m,z as h}from"./index-0-lbj3eN.js";import{i as g,n as _,r as v,t as y}from"./adminForms-BY-1se95.js";import{r as b}from"./klubbService-CPrDwSKB.js";import{t as x}from"./LoadingState-DU0ZcPlb.js";import{t as S}from"./buildDropdownOptions-lgetK572.js";import{t as C}from"./formNum-BTZ_qfFL.js";async function w(T,E={}){let D=E.id===void 0?void 0:Number(E.id);T.replaceChildren(x());let O=[],k=[],A=[],j=[],M=[];try{let e=await Promise.all([b(),f(),l(),i(),o()]);O=e[0].data,k=e[1].data,A=e[2].data,j=e[3].data,M=e[4].data}catch(t){e(`stevneadmin.render`,t),T.replaceChildren(r(`Kunne ikkje laste skjema.`));return}let N=null;if(D){let{data:e,error:i}=await h(D);if(i||!e){T.replaceChildren(r(`Stevne ikkje funne.`));return}if(N=e,!await n()&&!await t(N.klubbid??void 0)){T.replaceChildren(r(`Ingen tilgang til dette stevnet.`));return}}let P=D?`Rediger stevne: ${u(N?.navn??``)}`:`Nytt stevne`,F=N??{},I=F.dato??``,L=F.tid?F.tid.slice(0,5):D?``:`11:00`,R=F.kategoriid??M.find(e=>e.navn===`Singel`)?.id,z=S(O,F.klubbid),B=S(k,F.stevnetypeid),V=S(A,F.innledendekastemetodeid),H=S(j,F.avsluttendekastemetodeid),U=S(M,R);T.innerHTML=`
    <div class="container py-4 admin-form-lg">
      <h2 class="mb-4">${P}</h2>
      <form id="tournament-form">
        ${_(`Namn*`,`<input type="text" class="form-control" name="navn" value="${u(F.navn)}" required>`)}
        ${_(`Stad`,`<input type="text" class="form-control" name="sted" value="${u(F.sted)}">`)}
        ...
        ${_(`Innbydelses-URL`,`<input type="url" class="form-control" name="innbydelseurl" value="${u(F.innbydelseurl)}">`)}
        ${_(`Resultat-URL`,`<input type="url" class="form-control" name="resultaturl" value="${u(F.resultaturl)}">`)}
        ${D?`
    ...
    </div>`,T.querySelector(`#tournament-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=new FormData(e.target),n={navn:t.get(`navn`).trim(),sted:t.get(`sted`).trim()||null,dato:t.get(`dato`)||null,tid:t.get(`tid`)||null,klubbid:C(t.get(`klubbid`)),stevnetypeid:C(t.get(`stevnetypeid`)),innledendekastemetodeid:C(t.get(`innledendekastemetodeid`)),avsluttendekastemetodeid:C(t.get(`avsluttendekastemetodeid`)),kategoriid:C(t.get(`kategoriid`)),ernm:t.get(`ernm`)===`on`,ernorgesranking:t.get(`ernorgesranking`)===`on`,erekskludertfrarekorder:t.get(`erekskludertfrarekorder`)===`on`,innbydelseurl:t.get(`innbydelseurl`).trim()||null,resultaturl:t.get(`resultaturl`).trim()||null},{data:r,error:i}=D?await c(D,n):await a(n);if(i){v(T,y(i));return}g(T,`Stevnet er lagra.`),D||setTimeout(()=>{location.hash=`#/stevne/${r.id}/rediger`},1500)}),T.querySelector(`#delete-button`)?.addEventListener(`click`,async()=>{if(!await d({title:`Slett stevne`,message:`Slett «${N?.navn}»? Dette kan ikkje angrast.`,danger:!0}))return;let{error:e}=await m(D);if(e){v(T,y(e));return}location.hash=`#/terminliste`}),T.querySelector(`#complete-button`)?.addEventListener(`click`,async()=>{if(!await d({title:`Fullfør turnering`,message:`Fullfør «${N?.navn}»? Du kan ikkje lenger endre kampar og resultat for stevnet.`,danger:!0}))return;let{error:e}=await p(D);if(e){v(T,y(e));return}g(T,`Stevnet er fullført.`),setTimeout(()=>{w(T,E)},1500)}),T.querySelector(`#reopen-button`)?.addEventListener(`click`,async()=>{if(!await d({title:`Gjenåpne turnering`,message:`Gjenåpne «${N?.navn}»? Kampar og resultat kan då endres igjen.`}))return;let{error:e}=await s(D);if(e){v(T,y(e));return}g(T,`Stevnet er gjenåpna.`),setTimeout(()=>{w(T,E)},1500)})}export{w as render};