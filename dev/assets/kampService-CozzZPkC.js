import{n as e,t}from"./logError-ByTg738k.js";import{t as n}from"./verifiedWrite-DY57lQG_.js";import{Jn as r}from"./index-ScyQGOLm.js";e.from(`kamp_spelar`).select(`
  id, kasterid,
  kamp:kampid(
...
`);async function a(n){let{data:r,error:i}=await e.from(`kamp`).select(`
    ...
    `).eq(`stevneid`,n).eq(`fase`,`innledende`).order(`runde_nummer`).order(`bane_nummer`);return i&&t(`getInitialRoundMatches`,i),{data:r??[],error:i}}async function o(t,n){if(!t.length)return{error:null};let r=e.from(`kamp_omgang`).delete().in(`kamp_spelar_id`,t);return O(`deleteMatchRounds`,n==null?r:r.eq(`omgang`,n))}async function s(t,n,r){let i=r===void 0?{score_poeng:n}:{score_poeng:n,kamp_poeng:r};return O(`updateMatchPlayerScoreFast`,e.from(`kamp_spelar`).update(i).eq(`id`,t))}async function c(n){let{data:r,error:i}=await e.from(`kamp`).select(`
      id, stevneid, fase, runde_nummer, runde_navn, bane_nummer,
      er_bekreftet, er_walkover, er_tre_spelarar,
      stevne:stevneid(navn, klubbid),
      spelarar:kamp_spelar(
    ...
    `).eq(`id`,n).maybeSingle();return i&&t(`getMatch`,i),{data:r,error:i}}async function l(n,r){if(!r.length)return{startNumberMap:{},positionMap:{},hcpMap:new Map};let{data:i,error:a}=await e.from(`resultat`).select(`kasterid, startnummer, posisjon, hcp`).eq(`stevneid`,n).in(`kasterid`,r);a&&t(`getMatchResultInfo`,a);let o={},s={},c=new Map;for(let e of i??[])e.kasterid!=null&&(e.startnummer!=null&&(o[e.kasterid]=e.startnummer),e.posisjon!=null&&(s[e.kasterid]=e.posisjon),c.set(e.kasterid,e.hcp??0));return{startNumberMap:o,positionMap:s,hcpMap:c}}async function u(n){if(!n.length)return{};let{data:r,error:i}=await e.from(`resultat`).select(`stevneid, kasterid, startnummer`).in(`stevneid`,n);i&&t(`getStartNumbersForTournaments`,i);let a={};for(let e of r??[])e.kasterid!=null&&e.startnummer!=null&&(a[`${e.stevneid}:${e.kasterid}`]=e.startnummer);return a}async function d(n,r){let{data:i,error:a}=await e.from(`kamp`).select(`id`).eq(`stevneid`,n).eq(`bane_nummer`,r).eq(`er_bekreftet`,!1).eq(`er_walkover`,!1).order(`runde_nummer`).limit(1).maybeSingle();return a&&t(`getNextMatchForOrganizer`,a),{data:i,error:a}}async function f(n,r){let{data:i,error:a}=await e.from(`kamp_spelar`).select(`kampid`).eq(`kasterid`,r);if(a)return t(`getNextMatchForParticipant:minekampar`,a),{data:null,error:a};let o=(i??[]).map(e=>e.kampid).filter(e=>e!=null);if(!o.length)return{data:null,error:null};let{data:s,error:c}=await e.from(`kamp`).select(`id`).in(`id`,o).eq(`stevneid`,n).eq(`er_bekreftet`,!1).eq(`er_walkover`,!1).order(`runde_nummer`).limit(1).maybeSingle();return c&&t(`getNextMatchForParticipant`,c),{data:s,error:c}}async function p(t,n){let{data:r}=await e.from(`kamp_spelar`).select(`id`).eq(`kampid`,t).eq(`kasterid`,n).maybeSingle();return!!r}function m(e,t){if(!e)return null;let{baseScore:n}=t??{};return{playerIds:e.members.map(e=>e.id),kasterid:e.rep.kasterid,...n==null?{}:{baseScore:n}}}function h(e){let{roundData:t,sides:n,hcp:i=[],erWalkover:a=!1,placements:o}=e,s=new Map;for(let e of n)for(let t of e?.playerIds??[])s.set(t,{score_poeng:0,kamp_poeng:0,antall_ringer:0,kamp_plassering:null});let c=n.map(()=>0),l=e=>n[e]?.playerIds[0];if(a){c[0]=21;let e=l(0);e!=null&&(s.get(e).score_poeng=21)}else if(t.length){for(let e of t){let t=e.kamp_spelar_id;if(t==null)continue;let r=s.get(t);if(!r)continue;r.score_poeng+=e.score??0,r.antall_ringer+=e.antall_ringer??0;let i=n.findIndex(e=>e?.playerIds.includes(t));i!==-1&&(c[i]=(c[i]??0)+(e.score??0))}n.forEach((e,t)=>{let n=i[t]??0;if(!n)return;c[t]=(c[t]??0)+n;let r=l(t);r!=null&&(s.get(r).score_poeng+=n)})}else n.forEach((e,t)=>{if(!e)return;c[t]=e.baseScore??0;let n=l(t);n!=null&&(s.get(n).score_poeng=e.baseScore??0)});if(n.filter(Boolean).length<=2){let[e,t]=r(c[0]??0,c[1]??0);for(let t of n[0]?.playerIds??[])s.get(t).kamp_poeng=e;for(let e of n[1]?.playerIds??[])s.get(e).kamp_poeng=t}return n.forEach((e,t)=>{let r=o?.[t]??1+n.filter((e,n)=>e&&(c[n]??0)>(c[t]??0)).length;for(let t of e?.playerIds??[])s.get(t).kamp_plassering=r}),{updates:s,totals:c}}function g(e,t){let n=e.map((e,n)=>({side:e,total:t[n]??0})).filter(e=>e.side!=null);return n.length<2?null:n.reduce((e,t)=>t.total<=e.total?t:e).side.kasterid??null}var _=`Kampen er allereie stadfesta av ein annan deltakar.`;async function v(n){let{sides:r,hcp:i,erWalkover:a,placements:o}=n,s=r.flatMap(e=>e?.playerIds??[]),c={updates:new Map,totals:r.map(()=>0)};if(!s.length)return{error:null,...c};let l=[],u=r,d=r.every(e=>!e||e.baseScore!=null);if(!a&&!d){let{data:n,error:i}=await e.from(`kamp_omgang`).select(`kamp_spelar_id, score, antall_ringer`).in(`kamp_spelar_id`,s);if(i)return t(`confirmMatch:omgangar`,i),{error:i,...c};if(l=n??[],!l.length){let{data:t}=await e.from(`kamp_spelar`).select(`id, score_poeng`).in(`id`,s),n=new Map((t??[]).map(e=>[e.id,e.score_poeng??0]));u=r.map(e=>{if(!e||e.baseScore!=null)return e;let t=e.playerIds.filter(e=>n.has(e));return t.length?{...e,baseScore:t.reduce((e,t)=>e+n.get(t),0)}:e})}}let{updates:f,totals:p}=h({roundData:l,sides:u,hcp:i,erWalkover:a,placements:o});return{error:null,updates:f,totals:p}}async function y(r){let i=(await Promise.all([...r.entries()].map(([t,r])=>n(e.from(`kamp_spelar`).update(r).eq(`id`,t).select(`id`),_)))).find(e=>e.error)?.error??null;return i&&t(`confirmMatch:spelarar`,i),{error:i}}async function b(n){let{kampId:r,sides:i,hcp:a,erWalkover:o=!1,ranking:s,outcome:c}=n,{error:l,updates:u,totals:d}=await v({sides:i,hcp:a,erWalkover:o,placements:s?i.map(e=>s.find(t=>t.kasterid===e?.kasterid)?.plassering??null):void 0});if(l)return{error:l};if(c.type===`innledende`){let{data:n,error:i}=await e.rpc(`bekreft_innledende_kamp`,{p_kamp_id:r,p_scores:[...u.entries()].map(([e,t])=>({kamp_spelar_id:e,...t}))});return i&&t(`confirmMatch:innledende`,i),!i&&n===!1?{error:Error(_)}:{error:i}}let{error:f}=await y(u);if(f)return{error:f};if(c.type===`cup-derived`){let n=s?.length===3?s[2]?.kasterid??null:g(i,d),{error:a}=await e.rpc(`bekreft_avsluttende_kamp_deltakar`,{p_kamp_id:r,p_eliminert_kasterid:n??void 0});return a&&t(`confirmMatch:cup-derived`,a),{error:a}}return C(r,c)}e.from(`kamp`).select(`
