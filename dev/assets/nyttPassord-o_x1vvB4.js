import{t as e}from"./logError-ByTg738k.js";import{A as t,E as n,Pt as r,R as i,fr as a,hr as o,z as s}from"./index-BzkBRoPl.js";import{t as c}from"./navigation-DSwGXsno.js";var l=e=>e?`Set passord`:`Nytt passord`,u=e=>`
  <h2>${l(e)}</h2>
  ...
  </form>${e&&!o.isNativePlatform()?`<p class="account-hint">eller</p>
  ...
  <a href="#/minside" class="btn btn-primary">Gå til Min side</a>`;async function p(o){o.replaceChildren(a(`Opnar lenka…`));let l=c(`type`)===`invite`,p=c(`token_hash`);if(p){let{error:t}=await s(p,l?`invite`:`recovery`);t&&e(`nyttPassord.verifyEmailToken`,t)}let m=document.createElement(`div`);if(m.className=`container py-4 account-container`,!await n()){m.innerHTML=u(l),o.replaceChildren(m);return}m.innerHTML=d(l),o.replaceChildren(m);let h=m.querySelector(`#np-password`),g=m.querySelector(`#np-password2`),_=m.querySelector(`#np-error`),v=m.querySelector(`#np-submit`),y=m.querySelector(`#np-google`);y?.addEventListener(`click`,async()=>{_.classList.add(`d-none`),y.disabled=!0;let{error:n}=await t();n&&(e(`nyttPassord.linkGoogleIdentity`,n),_.textContent=`Kunne ikkje koble til Google: ${r(n)}`,_.classList.remove(`d-none`),y.disabled=!1)}),m.querySelector(`#np-form`).addEventListener(`submit`,async t=>{if(t.preventDefault(),_.classList.add(`d-none`),h.value!==g.value){_.textContent=`Passorda er ikkje like.`,_.classList.remove(`d-none`);return}v.disabled=!0;let{error:n}=await i(h.value);if(n){e(`nyttPassord.updatePassword`,n),_.textContent=`Kunne ikkje lagre passordet: ${r(n)}`,_.classList.remove(`d-none`),v.disabled=!1;return}m.innerHTML=f(l)})}export{p as render};