import{t as e}from"./dist-DIpltt4c.js";import{t}from"./logError-ByTg738k.js";import{Bt as n,E as r,L as i,O as a,R as o,vr as s}from"./index-ScyQGOLm.js";import{t as c}from"./navigation-DSwGXsno.js";var l=e=>e?`Set passord`:`Nytt passord`,u=e=>`
  <h2>${l(e)}</h2>
  ...
  <a href="#/minside" class="btn btn-primary">Gå til Min side</a>`;async function p(e){e.replaceChildren(s(`Opnar lenka…`));let l=c(`type`)===`invite`,p=c(`token_hash`);if(p){let{error:e}=await o(p,l?`invite`:`recovery`);e&&t(`nyttPassord.verifyEmailToken`,e)}let m=document.createElement(`div`);if(m.className=`container py-4 account-container`,!await r()){m.innerHTML=u(l),e.replaceChildren(m);return}m.innerHTML=d(l),e.replaceChildren(m);let h=m.querySelector(`#np-password`),g=m.querySelector(`#np-password2`),_=m.querySelector(`#np-error`),v=m.querySelector(`#np-submit`),y=m.querySelector(`#np-google`);y?.addEventListener(`click`,async()=>{_.classList.add(`d-none`),y.disabled=!0;let{error:e}=await a();e&&(t(`nyttPassord.linkGoogleIdentity`,e),_.textContent=`Kunne ikkje koble til Google: ${n(e)}`,_.classList.remove(`d-none`),y.disabled=!1)}),m.querySelector(`#np-form`).addEventListener(`submit`,async e=>{if(e.preventDefault(),_.classList.add(`d-none`),h.value!==g.value){_.textContent=`Passorda er ikkje like.`,_.classList.remove(`d-none`);return}v.disabled=!0;let{error:r}=await i(h.value);if(r){t(`nyttPassord.updatePassword`,r),_.textContent=`Kunne ikkje lagre passordet: ${n(r)}`,_.classList.remove(`d-none`),v.disabled=!1;return}m.innerHTML=f(l)})}export{p as render};