import{n as e,t}from"./logError-CB4-2Lin.js";import{A as n,F as r,G as i,H as a,I as o,Lt as s,W as c,Yt as l,c as u,g as d,j as f,k as p,l as m,nt as h,r as g,s as _,t as v,zt as y}from"./index-B3Z0SKG3.js";import{t as b}from"./LoadingState-C6NB62Ct.js";import{d as x,r as S}from"./kasterService-D9jqvobU.js";import{t as C}from"./EmptyState-CCNgsnix.js";import{t as w}from"./SearchSelect-ubpkwWhs.js";import{i as ee,n as T,r as E}from"./kaster-2cwCS5i9.js";import{n as D,t as O}from"./accountService-BWtrr2u0.js";import{f as k,l as A}from"./kampService-BOB7VeHW.js";import{t as te}from"./Tabs-DZCBJPb0.js";import{t as j,u as ne}from"./navigationService-rYTe-Tbe.js";import{n as re}from"./ScoreboardButton-cO_q_Bk1.js";function M(e){return`
    ...
    </div>`}function N(){return`<div class="alert alert-info mb-4">Koblingforespørselen din ventar på godkjenning frå ein administrator.</div>`}function P(e,t){let n=e.querySelector(`#thrower-error`);w({slot:e.querySelector(`#thrower-search-slot`),loadItems:async()=>{let{data:e}=await S();return e.map(e=>({id:e.id,label:ee(e),sublabel:e.klubb?.navn??null}))},placeholder:`Søk på navn…`,onSelect:e=>{e!=null&&(async()=>{n.classList.add(`d-none`);let{error:r}=await h(t,e);if(r){n.textContent=`Kunne ikkje sende forespørsel.`,n.classList.remove(`d-none`);return}o(),await g()})()}})}function F(e,t){return t.status===`godkjent`&&t.profil?.kasterid!=null?t.profil.kasterid:t.status===`venter`?(e.innerHTML=N(),null):(e.innerHTML=M(t.status),P(e,t.user.id),null)}function I(e,t,n){let r=F(e,t);return r==null?null:(e.innerHTML=`
    <div class="card mb-4">
      <div class="card-body">
        <h5 class="card-title">${u(n)}</h5>
        <div data-slot="content"><div class="skeleton-block skeleton-block--card"></div></div>
      </div>
    </div>`,{throwerId:r,slot:e.querySelector(`[data-slot="content"]`)})}function L(e){let t=document.createElement(`div`);return t.innerHTML=e,t}var R=[{key:`innledende`,label:`Innleiande`},{key:`avsluttende`,label:`Avsluttande`}];function z(e){let t=R.findIndex(t=>t.key===e.kamp?.fase);return t===-1?R.length:t}function B(e,t,n,r){let i=n==null?void 0:r[`${n}:${t}`];return e.filter(e=>{if(e.kasterid==null||e.kasterid===t)return!1;let a=n==null?void 0:r[`${n}:${e.kasterid}`];return i==null||a==null||a!==i})}function V(e,t,n){return`<table class="table table-sm mb-3">
    ...
    </table>`}function H(e,t,n){if(!e.length)return null;let r=new Map;for(let t of e){let e=t.stevneid??`unknown`;r.has(e)||r.set(e,{name:`${t.stevne?.navn??``} – X-kast`,courts:[]}),r.get(e).courts.push(t)}return[...r.values()].map(({name:e,courts:r})=>`
      <p class="fw-semibold mb-1 mt-2">${u(e)}</p>
      ${V(r,t,n)}`).join(``)}async function U(e){let[{data:t,error:n},r]=await Promise.all([A(e),ne(e)]);if(n){let e=document.createElement(`p`);return e.className=`text-muted`,e.textContent=`Kunne ikkje laste kampar.`,e}let i=t.filter(e=>!e.kamp?.er_walkover),a=[...new Set(i.map(e=>e.kamp?.stevneid).filter(e=>e!=null))],o=await k(a),s=i.filter(e=>e.kamp?.stevne?.erfullfort===!1).sort((e,t)=>(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),c=i.filter(e=>e.kamp?.stevne?.erfullfort===!0).sort((e,t)=>(t.kamp?.stevne?.dato??``).localeCompare(e.kamp?.stevne?.dato??``)||z(e)-z(t)||(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),l=(t,n)=>{let r=t.kamp,i=r?.stevneid,a=B(r?.spelarar??[],e,i,o),s=a.length?a.map(e=>u(E(e.kaster))).join(` / `):`–`;return`<tr>
      ...
      </tbody></table>`,f=(e,t)=>{let n=R.map(({key:t,label:n})=>({label:n,matches:e.filter(e=>e.kamp?.fase===t)})).filter(e=>e.matches.length),r=n.reduce((e,t)=>e+t.matches.length,0)===e.length;return n.length<2||!r?d(e,t):n.map(({label:e,matches:n})=>`
      ...
      <p class="fw-semibold mb-1 mt-2">${u(e)}</p>
      ${n?f(r,t):d(r,t)}`).join(``)},m=p(s,t=>{if(!t.kamp?.er_bekreftet)return re(t.kamp?.id??``,j(),`scoreboard-btn--touch`);let n=t.kamp.stevneid,r=t.kamp.spelarar?.find(t=>t.kasterid===e)?.score_poeng,i=B(t.kamp.spelarar??[],e,n,o)[0]?.score_poeng;return r==null||i==null?`–`:`<span class="fw-semibold">${r} – ${i}</span>`}),h=p(c,e=>`<a href="#/kamp/${e.kamp?.id??``}" class="btn btn-sm btn-outline-secondary"${j()}>Vis</a>`,!0),g=r.data.filter(e=>e.fase===`innledende`),_=g.filter(e=>e.stevne?.erfullfort===!1).sort((e,t)=>(e.bane_nummer??0)-(t.bane_nummer??0)),v=g.filter(e=>e.stevne?.erfullfort===!0).sort((e,t)=>(t.stevne?.dato??``).localeCompare(e.stevne?.dato??``)||(e.bane_nummer??0)-(t.bane_nummer??0)),y=e=>`#/stevne/${e.stevneid}/innledende`,b=H(_,e,t=>{if(!t.er_bekreftet)return`<a href="${y(t)}" class="btn btn-sm btn-primary">Opne bane</a>`;let n=t.deltakarar.find(t=>t.kasterid===e)?.poeng;return n==null?`–`:`<span class="fw-semibold">${n}</span>`}),x=H(v,e,e=>`<a href="${y(e)}" class="btn btn-sm btn-outline-secondary">Vis</a>`),S=[m,b].filter(Boolean).join(``),C=[h,x].filter(Boolean).join(``);return te({tabs:[{id:`active`,label:`Aktive (${s.length+_.length})`,panel:L(S||`<p class="text-muted">Ingen aktive kampar.</p>`)},{id:`completed`,label:`Ferdige (${c.length+v.length})`,panel:L(C||`<p class="text-muted">Ingen ferdige kampar enno.</p>`)}]})}async function W(e,t){let n=I(e,t,`Mine kampar og banar`);if(!n)return;let r=await U(n.throwerId);n.slot.replaceChildren(r)}async function G(e){let{data:t,error:n}=await d(e),r=new Map;if(n){let e=document.createElement(`p`);return e.className=`text-muted`,e.textContent=`Kunne ikkje laste påmeldingar.`,{node:e,registeredMap:r}}let i=t.filter(e=>e.stevne?.erfullfort!==!0);if(!i.length)return{node:C(`Ingen påmeldingar enno.`),registeredMap:r};let a=[...i].sort((e,t)=>(e.stevne?.dato??``).localeCompare(t.stevne?.dato??``)),o=document.createElement(`div`);o.className=`stevne-kort-liste`;for(let e of a){let t=e.stevne?.id;t!=null&&r.set(t,e.id),o.appendChild(_({title:e.stevne?.navn??``,href:t==null?`#`:`#/stevne/${t}/info`,date:y(e.stevne?.dato),status:`upcoming`,registrationSlotId:t??void 0}))}return{node:o,registeredMap:r}}async function K(e,t){let n=I(e,t,`Påmeldingar`);if(!n)return;let{node:r,registeredMap:i}=await G(n.throwerId);n.slot.replaceChildren(r),m(e,n.throwerId,t.user.id,i)}async function q(n){let{data:r,error:i}=await e.from(`bruker_profil`).select(`varsle_stevne_start, varsle_kamp_opprettet`).eq(`id`,n).maybeSingle();return i&&t(`getNotificationPreferences`,i),{data:r,error:i}}async function ie(n,r,i){let a=r===`varsle_stevne_start`?{varsle_stevne_start:i}:{varsle_kamp_opprettet:i},{error:o}=await e.from(`bruker_profil`).update(a).eq(`id`,n);return o&&t(`updateNotificationPreference`,o),{error:o}}function J(e){return`
    ...
    </div>`,!n)return;let r=e.querySelector(`[data-slot="notifications"]`),{data:i}=await q(t.user.id);if(!i){r.innerHTML=`<p class="text-muted">Kunne ikkje laste varslingsinnstillingar.</p>`;return}r.innerHTML=J(i),Y(e,t.user.id)}var X=`Resultater og statistikk for koblet utøver blir ikkje slettet.`;async function oe(e){let{data:t,error:n}=await x(e);return n||!t?``:`
    <h5 class="card-title">Kontoen er koblet til utøvarprofil</h5>
    <p class="mb-1"><strong>${u(E(t))}</strong> · ${u(t.klubb?.navn??``)}</p>
    <a href="#/kastere/${T(t)}" class="btn btn-sm btn-outline-primary mt-1">Vis profil</a>`}function se(e){let t=e?`Bytt passord`:`Opprett passord`;return`
    ...
    <div class="card mb-4"><div class="card-body">${se(a)}</div></div>
    ${i?`
    ...
    </div></div>`,le(e),de(e,n),r==null)return;ue(e);let o=e.querySelector(`[data-slot="thrower"]`),s=e.querySelector(`[data-slot="accounts"]`);try{let[e,t]=await Promise.all([oe(r),D()]);o.innerHTML=e,s.innerHTML=t.error?`<p class="text-muted">Kunne ikkje laste kontoar.</p>`:ce(t.data,n.user.id)}catch(e){t(`minsideKonto.render`,e),s.innerHTML=`<p class="text-muted">Kunne ikkje laste kontoar.</p>`}}var Q=[{key:`kampar`,label:`Kampar`},{key:`pameldingar`,label:`Påmeldingar`},{key:`innstillingar`,label:`Innstillingar`},{key:`konto`,label:`Konto`}],fe=new Set(Q.map(e=>e.key)),pe={kampar:W,pameldingar:K,innstillingar:ae,konto:Z};function me(e){return`<ul class="nav nav-underline mypage-nav mb-3">${Q.map(({key:t,label:n})=>`
      <li class="nav-item">
        <a class="nav-link${e===t?` active`:``}"
           href="#/minside/${t}">${n}</a>
      </li>`).join(``)}</ul>`}async function $(e,n){v(()=>$(e,n));let i=String(n.tab??`kampar`);e.replaceChildren(b(`Laster min side…`));try{let t=await r();if(!t){location.hash=`#/logginn`;return}let{profil:n,user:a}=t,o=n?.kobling_status??`ingen`,s=i===`varslingar`?`innstillingar`:i,c=fe.has(s)?s:`kampar`;e.innerHTML=`
      <div class="mypage-container">
        <h2 class="mb-1">Min side</h2>
        <p class="text-muted mb-3">${u(a.email??``)}</p>
        ${me(c)}
        <div id="minside-subpage"></div>
      </div>`;let l=e.querySelector(`#minside-subpage`);await pe[c](l,{user:a,profil:n,status:o})}catch(n){t(`minside.render`,n),e.replaceChildren(s(`Kunne ikkje laste min side.`))}}export{$ as render};