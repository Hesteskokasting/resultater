import{t as e}from"./logError-D5z16FyH.js";import{$ as t,D as n,M as r,X as i,Z as a,_t as o,a as s,bt as c,ft as l,j as u,rt as d,st as f,vt as p,wt as m,xt as h}from"./index-BWk7I7cT.js";import{i as g,n as _,r as v,t as y}from"./adminForms-BKTRhXRJ.js";import{r as b}from"./klubbService-CaXvOdL5.js";import{t as x}from"./LoadingState-CllUVMAe.js";import{t as S}from"./buildDropdownOptions-Buw85vjp.js";import{t as C}from"./formNum-qslqOEb0.js";async function w(T,E={}){let D=E.id===void 0?void 0:Number(E.id);T.replaceChildren(x());let O=[],k=[],A=[],j=[],M=[];try{let e=await Promise.all([b(),o(),f(),d(),t()]);O=e[0].data,k=e[1].data,A=e[2].data,j=e[3].data,M=e[4].data}catch(t){e(`stevneadmin.render`,t),T.replaceChildren(m(`Kunne ikkje laste skjema.`));return}let N=null;if(D){let{data:e,error:t}=await l(D);if(t||!e){T.replaceChildren(m(`Stevne ikkje funne.`));return}if(N=e,!await u()&&!await r(N.klubbid??void 0)){T.replaceChildren(m(`Ingen tilgang til dette stevnet.`));return}}let P=D?`Rediger stevne: ${s(N?.navn??``)}`:`Nytt stevne`,F=N??{},I=F.dato??``,L=F.tid?F.tid.slice(0,5):D?``:`11:00`,R=F.kategoriid??M.find(e=>e.navn===`Singel`)?.id,z=S(O,F.klubbid),B=S(k,F.stevnetypeid),V=S(A,F.innledendekastemetodeid),H=S(j,F.avsluttendekastemetodeid),U=S(M,R);T.innerHTML=`
    <div class="container py-4 admin-form-lg">
      <h2 class="mb-4">${P}</h2>
      <form id="tournament-form">
        ${_(`Namn*`,`<input type="text" class="form-control" name="navn" value="${s(F.navn)}" required>`)}
        ${_(`Stad`,`<input type="text" class="form-control" name="sted" value="${s(F.sted)}">`)}
        ...
        ${_(`Resultat-URL`,`<input type="url" class="form-control" name="resultaturl" value="${s(F.resultaturl)}">`)}
        ${D?`
