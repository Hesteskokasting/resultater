import{$ as e,A as t,B as n,C as r,D as i,E as a,G as o,H as s,I as c,J as l,K as u,M as d,O as f,P as p,Q as m,R as h,T as g,V as _,X as v,Y as y,Z as b,_ as x,b as S,d as C,et as w,f as T,g as E,h as D,j as O,k,n as A,o as j,p as M,r as N,s as ee,t as P,tt as F,u as I,v as te,w as L,x as R,z}from"./index-BFZ4Td0n.js";import{t as B}from"./ScoreNumberpad-C-b3sWkY.js";function V(e,{visSpelarliste:t=!0,initNa:n=null,initFormat:r=null}={}){let a=typeof e==`number`?e:e.length,o=typeof e==`number`?[]:e.map((e,t)=>({...e,cupPlassering:t+1})),s=i(a),c=n===a?a:n!=null&&s.some(e=>e.nA===n)?n:s[0]?.nA??a,l=a-c,u=s.filter(e=>f(e.nA).some(e=>e.c3>0)),d=s.filter(e=>!f(e.nA).some(e=>e.c3>0)),p=f(a).length>0,m=(e,t)=>e.map((e,r)=>{let i=e.nA===c&&n!==a,o=t+r;return`
  ...
  `}function H(e,t,n=0,r=0){let i=e.slice(0,t),a=e.slice(t);function o(e,t=0){return e.map((e,n)=>{let r=n<t;return`
    ...
    </div>`}function ae(e){return e.map(e=>({kasterid:e.kasterid??0,kamp_poeng:e.kamp_poeng??0,score_poeng:e.score_poeng??0,posisjon:e.posisjon,antall_ringer:e.antall_ringer,omgangar:e.omgangar,kaster:e.kaster}))}function oe(e,t,r,i,a){for(let s of r){let r=s.spelarar.slice().sort((e,t)=>(e.posisjon??0)-(t.posisjon??0));if(e.querySelector(`#plus-${s.id}`)?.addEventListener(`click`,async()=>{let e=r[0],t=r[1];B(e?.kaster?`${e.kaster.fornavn} ${e.kaster.etternavn}`:`—`,t?.kaster?`${t.kaster.fornavn} ${t.kaster.etternavn}`:`—`,o(e),o(t),async(n,r)=>{let i=[];e?.id&&i.push(z(e.id,n)),t?.id&&i.push(z(t.id,r)),await Promise.all(i),await a()})}),e.querySelector(`#scoreboard-${s.id}`)?.addEventListener(`click`,()=>{window.open(`#/kamp/${s.id}`,`_blank`)}),e.querySelector(`#bekrft-${s.id}`)?.addEventListener(`click`,async e=>{if(s.er_tre_spelarar)J(s,r,t,async()=>{await $(t,s),await a()});else{let n=e.currentTarget;n.disabled=!0,n.textContent=`Lagrer…`;try{await se(t,s,r,a)||(n.disabled=!1,n.textContent=`Bekreft`)}catch{n.disabled=!1,n.textContent=`Bekreft`}}}),i&&s.er_bekreftet&&!s.er_tre_spelarar){let i=r[0],c=r[1],l=i?.kaster?`${i.kaster.fornavn} ${i.kaster.etternavn}`:`—`,u=c?.kaster?`${c.kaster.fornavn} ${c.kaster.etternavn}`:`—`,d=r.map(e=>e.kasterid).filter(e=>e!=null),f=()=>{B(l,u,o(i),o(c),async(e,r)=>{let o=[i?.id,c?.id].filter(e=>e!=null);if(o.length){let{error:e}=await _(o);if(e){k(`DB-feil ved sletting av omgangar`,`error`);return}}let l=[];if(i?.id&&l.push(z(i.id,e)),c?.id&&l.push(z(c.id,r)),(await Promise.all(l)).find(e=>e.error)?.error){k(`DB-feil ved oppdatering av score`,`error`);return}let u=e>=r?i?.kasterid:c?.kasterid,f=e>=r?c?.kasterid:i?.kasterid;await n({stevneId:t,rundeNummer:s.runde_nummer,rundeNavn:s.runde_navn,allKasterids:d,nyVinnarId:u,nyTaparId:f}),await a()})};e.querySelectorAll(`[data-endre-score="${s.id}"]`).forEach(e=>e.addEventListener(`click`,f))}}}async function se(e,t,n,r){let i=n[0],a=n[1],{data:s}=await h(t.id),c=s.find(e=>e.id===i?.id),l=s.find(e=>e.id===a?.id),u=o(c??i),f=o(l??a);if(u===0&&f===0&&!await O({title:`Ingen score registrert`,message:`Vil du bekrefte kampen likevel?`}))return!1;let p=u>=f?i:a,m=u>=f?a:i,g=n.map(e=>e.kasterid).filter(e=>e!=null),_=p?.kasterid==null?[]:[p.kasterid],{error:v}=await d({kampId:t.id,stevneId:e,rundeNummer:t.runde_nummer,rundeNavn:t.runde_navn,allKasterids:g,eliminertId:m?.kasterid??null,vidareIds:_});return v?(k(`DB-feil ved bekreft`,`error`),!1):(await $(e,t),await r(),!0)}async function $(e,t){t.runde_navn!==`Semifinale`||!t.gruppe_navn||await p(e,t.gruppe_navn)&&await L(e,t.gruppe_navn)}export{re as render};