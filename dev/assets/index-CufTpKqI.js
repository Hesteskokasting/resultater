const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/gloppen-DMaOeWaa.js","assets/innledendeBase-WbkfazM8.js","assets/ScoreNumberpad-C-b3sWkY.js","assets/nordhordland-B-Ri_tRp.js","assets/cup-2yH8U84B.js"])))=>i.map(i=>d[i]);
...
`),y.from(`kamp_spelar`).select(`id, kasterid, score_poeng, antall_ringer, omgangar:kamp_omgang(score, antall_ringer)`);async function Yr(e){let{data:t,error:n}=await y.from(`kamp`).select(`
    ...
    </div>`}function di(){return`<div class="alert alert-info mb-4">Koblingforespørselen din ventar på godkjenning frå ein administrator.</div>`}async function fi(e){let{data:t,error:n}=await on(e);return n||!t?``:`
    <div class="card mb-4">
      <div class="card-body">
    ...
    </div>`}async function pi(e){let{data:t,error:n}=await wr(e);return n?`<p class="text-muted">Kunne ikkje laste påmeldingar.</p>`:t.length?`
    <div class="card mb-4">
      <div class="card-body">
    ...
    </div>`:`<p class="empty-state">Ingen påmeldingar enno.</p>`}async function mi(e){let{data:t,error:n}=await Lr(e);if(n){let e=document.createElement(`p`);return e.className=`text-muted`,e.textContent=`Kunne ikkje laste kampar.`,e}let r=t.filter(e=>!e.kamp?.er_walkover),i=r.filter(e=>e.kamp?.stevne?.erfullfort===!1&&!e.kamp?.er_bekreftet).sort((e,t)=>(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),a=r.filter(e=>e.kamp?.er_bekreftet).sort((e,t)=>(e.kamp?.runde_nummer??0)-(t.kamp?.runde_nummer??0)),o=(t,n)=>{let r=t.kamp,i=(r?.spelarar??[]).find(t=>t.kasterid!==e),a=i?.kaster?j(`${i.kaster.fornavn} ${i.kaster.etternavn}`):`–`;return`<tr>
    ...
    </div>`;let t=e.querySelector(`#admin-innhald`);async function n(n){e.querySelectorAll(`[data-fane]`).forEach(e=>{e.classList.toggle(`active`,e.dataset.fane===n)}),A(`Laster...`),n===`kobling`&&await Si(t),n===`brukarar`&&await Ci(t),n===`klubbadmin`&&await wi(t)}e.querySelector(`#admin-faner`).addEventListener(`click`,e=>{let t=e.target.closest(`[data-fane]`);t?.dataset.fane&&n(t.dataset.fane)}),n(`kobling`)}async function Si(e){let{data:t,error:n}=await Ue();if(n){e.innerHTML=`<div class="alert alert-danger">${j(q(n))}</div>`;return}if(!t.length){e.replaceChildren(L(`Ingen ventande forespørslar.`));return}let r=t.map(e=>e.id),i=t.map(e=>e.kobling_kasterid).filter(e=>e!==null),[{data:a},{data:o}]=await Promise.all([We(r),ln(i)]),s=Object.fromEntries((a??[]).map(e=>[e.id,e.epost])),c=new Map((o??[]).map(e=>[e.id,e]));e.innerHTML=`<table class="table table-hover">
  ...
  `,document.body.appendChild(Y),Y.querySelector(`#cd-cancel`).addEventListener(`click`,()=>{ji(!1)}),Y.querySelector(`#cd-confirm`).addEventListener(`click`,()=>{ji(!0)}),Y)}function ki(e){Ti=document.createElement(`div`),Ti.className=`modal-backdrop show`,document.body.appendChild(Ti),document.body.classList.add(`modal-open`),e.style.display=`block`,e.classList.add(`show`),e.querySelector(`#cd-confirm`)?.focus(),Di=e=>{e.key===`Escape`&&(e.preventDefault(),ji(!1))},document.addEventListener(`keydown`,Di)}function Ai(e){e.classList.remove(`show`),e.style.display=`none`,Ti?.remove(),Ti=null,document.body.classList.remove(`modal-open`),Di&&=(document.removeEventListener(`keydown`,Di),null)}function ji(e){if(!Y||!Ei)return;let t=Ei;Ei=null,Ai(Y),t(e)}function X(e){let{title:t,message:n,confirmText:r=`OK`,cancelText:i=`Avbryt`,danger:a=!1}=e,o=Oi();o.querySelector(`#cd-title`).textContent=t,o.querySelector(`#cd-message`).textContent=n,o.querySelector(`#cd-cancel`).textContent=i;let s=o.querySelector(`#cd-confirm`);return s.textContent=r,s.className=`btn ${a?`btn-danger`:`btn-primary`}`,new Promise(e=>{Ei=e,ki(o)})}function Z(e){if(!e||typeof e!=`string`)return null;let t=Number(e);return Number.isFinite(t)?t:null}async function Mi(e,{id:t}={}){e.replaceChildren(A());let n=[],r=[],i=[],a=[];try{let e=await Promise.all([Rn(),me(),he(),ge()]);n=e[0].data,r=e[1].data,i=e[2].data,a=e[3].data}catch(t){b(`stevneadmin.render`,t),e.replaceChildren(k(`Kunne ikkje laste skjema.`));return}let o=null;if(t){let{data:n,error:r}=await _e(t);if(r||!n){e.replaceChildren(k(`Stevne ikkje funne.`));return}if(o=n,!await N()&&!await at(o.klubbid??void 0)){e.replaceChildren(k(`Ingen tilgang til dette stevnet.`));return}}let s=t?`Rediger stevne: ${j(o?.navn??``)}`:`Nytt stevne`,c=o??{},l=c.dato??``,u=c.tid?c.tid.slice(0,5):``,d=P(n,c.klubbid),f=P(r,c.stevnetypeid),p=P(i,c.innledendekastemetodeid),m=P(i,c.avsluttendekastemetodeid),h=P(a,c.kategoriid);e.innerHTML=`
    ...
    </div>`,e.querySelector(`#kaster-skjema`).addEventListener(`submit`,async n=>{n.preventDefault();let r=new FormData(n.target),i={fornavn:r.get(`fornavn`).trim(),etternavn:r.get(`etternavn`).trim(),kjonnid:Z(r.get(`kjonnid`)),klubbid:Z(r.get(`klubbid`)),klasseid:Z(r.get(`klasseid`)),epost:r.get(`epost`).trim()||null,telefon:r.get(`telefon`).trim()||null,medlemsnummer:r.get(`medlemsnummer`)?Number(r.get(`medlemsnummer`)):null,eraktiv:r.get(`eraktiv`)===`on`},{data:a,error:o}=t?await fn(t,i):await dn(i);if(o){_i(e,q(o));return}vi(e,`Utøvaren er lagra.`),t||setTimeout(()=>{location.hash=`#/kaster/${a.id}/admin`},1500)}),e.querySelector(`#slett-knapp`)?.addEventListener(`click`,async()=>{if(!await X({title:`Slett utøvar`,message:`Slett «${a?.fornavn} ${a?.etternavn}»? Dette kan ikkje angrast.`,danger:!0}))return;let{error:n}=await pn(t);if(n){_i(e,q(n));return}location.hash=`#/kastere`})}async function Pi(e,{id:t}={}){if(!t){e.replaceChildren(k(`Manglande ID.`));return}e.replaceChildren(A());let{data:n,error:r}=await Bn(t);if(r||!n){e.replaceChildren(k(`Klubb ikkje funne.`));return}if(!await N()&&!await at(t)){e.replaceChildren(k(`Ingen tilgang til denne klubben.`));return}e.innerHTML=`
    ...
    </div>`,e.querySelector(`#klubb-skjema`).addEventListener(`submit`,async n=>{n.preventDefault();let r=new FormData(n.target),{error:i}=await Vn(t,{navn:r.get(`navn`).trim(),kortnavn:r.get(`kortnavn`).trim(),logourl:r.get(`logourl`).trim()||null,eraktiv:r.get(`eraktiv`)===`on`});if(i){_i(e,q(i));return}vi(e,`Klubben er lagra.`)})}function Fi(e,t,n,r,i,a){return e?!n&&!t?`<div class="alert alert-warning">
    ...
    `}e.innerHTML=y(`Omgang 1`,`<div id="sb-container" class="sb-page"></div>`,`sb-omgang-tittel`),e.addEventListener(`click`,e=>{e.target.closest(`.sb-tilbake-btn`)&&(history.length>1?history.back():window.close())});let x=e.querySelector(`#sb-container`),S=e.querySelector(`#sb-omgang-tittel`);async function C(){if(d){let{data:e}=await Wr(r.stevneid,r.bane_nummer??0);return e}if(l==null)return null;let{data:e}=await Gr(r.stevneid,l);return e}async function w(e){return e.er_walkover?!1:d?e.bane_nummer===r.bane_nummer:l==null?!1:Kr(e.id,l)}function T(){sessionStorage.setItem(`ventar-neste-${n}`,`1`),e.innerHTML=y(`Fullført`,`<div class="sb-ventar-innhald">
      ...
      </div>`);let t=ai(r.stevneid,n,async e=>{await w(e)&&(await Vi(t),location.hash=`#/kamp/${e.id}`)});window.addEventListener(`hashchange`,()=>{sessionStorage.removeItem(`ventar-neste-${n}`),Vi(t)},{once:!0})}async function E(){let n=await C();n?location.hash=`#/kamp/${n.id}`:d||f?T():Xi(e,{id:t})}function D(t){e.querySelector(`.sb-feil-banner`)?.remove();let n=document.createElement(`div`);n.className=`sb-feil-banner alert alert-danger m-2`,n.textContent=t,e.prepend(n)}async function O(e){let t={p1:p?{spelarId:p.id,kasterid:p.kasterid,scorePoeng:p.score_poeng}:null,p2:m?{spelarId:m.id,kasterid:m.kasterid,scorePoeng:m.score_poeng}:null};if(r.fase===`avsluttende`){let{error:r}=await Jr({kampId:n,...t,orderedKasterids:e??null});if(r){D(`Feil ved bekreftelse av kamp.`);return}}else{let{error:e}=await qr({kampId:n,...t,hcp1:g,hcp2:_,erWalkover:r.er_walkover});if(e){D(`Feil ved bekreftelse av kamp.`);return}}await E()}if(r.er_bekreftet&&sessionStorage.getItem(`ventar-neste-${n}`)){await E();return}x&&await Wi(x,r,p,m,{pointValues:Yi,erArrangor:d,erDeltakar:f,onBekreft:O,onKampBekreft:d||f?E:void 0,omgangEl:S,p3ks:h,hcp1:g,hcp2:_})}function Zi(e,t,n){let r=(t??[]).filter(t=>t.spelarar?.some(t=>t.kasterid===e)).sort((e,t)=>e.runde_nummer-t.runde_nummer);return r.length?r.map(t=>{let r=t.spelarar?.find(t=>t.kasterid===e),i=t.spelarar?.find(t=>t.kasterid!==e),a=t.er_walkover&&(!i||!i.kaster),o=a?`Walkover`:i?.kaster?`${j(i.kaster.fornavn)} ${j(i.kaster.etternavn)}`:`—`,s=a?``:i?.kasterid?n[i.kasterid]??``:``,c=s?`${o} (${s})`:o,l=`${a?21:Ir(r)} - ${a?0:Ir(i)}`;return`<tr>
      <td class="text-center">${t.runde_nummer}</td>
      <td class="text-center">${t.bane_nummer??``}</td>
      <td>${c}</td>
      <td class="text-center">${l}</td>
    </tr>`}).join(``):`<tr><td colspan="4" class="text-muted small fst-italic text-center">Ingen kampar</td></tr>`}function Qi(e,t,n,r={}){if(e.er_bekreftet)return!1;if(e.er_walkover)return!0;if(n)return!1;let i=r[t[0]?.kasterid]??0,a=r[t[1]?.kasterid]??0,o=Ir(t[0]),s=t[1]?Ir(t[1]):0;return o+i>=21||s+a>=21}function $i(e,t){return`
  ...
  `}function ca(e,t){let n={},r=new Set;for(let i of e){let[,e]=i.er_walkover?Fr(i.spelarar,t):[null,null];for(let a of i.spelarar??[])!a.kasterid||!a.kaster||i.er_walkover&&a.kasterid===e?.kasterid||(r.add(a.kasterid),n[a.kasterid]||(n[a.kasterid]={kasterid:a.kasterid,navn:`${a.kaster.fornavn} ${a.kaster.etternavn}`,startnummer:t[a.kasterid]??null,kamp_poeng:0,score_poeng:0,antall_kamper:0}),i.er_bekreftet&&(n[a.kasterid].kamp_poeng+=a.kamp_poeng,n[a.kasterid].score_poeng+=a.score_poeng,n[a.kasterid].antall_kamper+=1))}return{spelMap:n,ekteKasterids:r}}function la(e,t,n,r){let{spelMap:i}=ca(e,r);return ua(t.map(e=>({kasterid:e.kasterid,navn:n[e.kasterid]??`Spelar ${e.kasterid}`,startnummer:e.startnummer,plassering:e.plassering,runde_eliminert:e.runde_eliminert,kamp_poeng:i[e.kasterid]?.kamp_poeng??0,score_poeng:i[e.kasterid]?.score_poeng??0,gruppe:e.gruppe})),e)}function ua(e,t){let n=t.filter(e=>e.er_bekreftet);return[...e].sort((e,t)=>{let r=e.runde_eliminert==null;if(r!==(t.runde_eliminert==null))return r?-1:1;if(!r){let n=(t.runde_eliminert??0)-(e.runde_eliminert??0);if(n!==0)return n;let r=e.plassering??1/0,i=t.plassering??1/0;if(r!==i)return r-i}if(t.kamp_poeng!==e.kamp_poeng)return(t.kamp_poeng??0)-(e.kamp_poeng??0);if(t.score_poeng!==e.score_poeng)return(t.score_poeng??0)-(e.score_poeng??0);let i=0,a=0;for(let r of n){let n=r.spelarar?.find(t=>t.kasterid===e.kasterid),o=r.spelarar?.find(e=>e.kasterid===t.kasterid);n&&o&&(i+=n.kamp_poeng??0,a+=o.kamp_poeng??0)}if(i!==a)return a-i;let o=e=>n.flatMap(t=>t.spelarar?.filter(t=>t.kasterid===e)??[]).map(e=>Ir(e)).sort((e,t)=>t-e),s=o(e.kasterid),c=o(t.kasterid);for(let e=0;e<Math.min(s.length,c.length);e++)if(c[e]!==s[e])return c[e]-s[e];return(e.startnummer??1/0)-(t.startnummer??1/0)})}function da(){return crypto.randomUUID()}async function fa(e,t,n){let{data:r,error:i}=await y.from(`pamelding`).select(`id, kasterid, kaster(klubbid)`).eq(`stevneid`,e).order(`id`);if(i)throw Error(`Feil ved henting av påmelding: `+i.message);if(!r?.length)throw Error(`Ingen spelarar påmelde.`);for(let e=r.length-1;e>0;e--){let t=Math.floor(Math.random()*(e+1));[r[e],r[t]]=[r[t],r[e]]}let a=r.length,o={},s=r.map((t,n)=>{o[n+1]=t.kasterid;let r=t.kaster?.klubbid??null;return{stevneid:e,kasterid:t.kasterid,klubbid:r,startnummer:n+1}});await y.from(`resultat`).delete().eq(`stevneid`,e);let{error:c}=await y.from(`resultat`).insert(s);if(c)throw Error(`Feil ved lagring av startnummer: `+c.message);return t.toLowerCase().includes(`gloppen`)?pa(e,o,a,n):ma(e,o,a)}async function pa(e,t,n,r){let i=(n%2==0?n:n+1)/2,a=0;for(let o=1;o<=r;o++){let r=[],s=[];for(let t=1;t<=i;t++){let a=(t-1+o-1)%i+1,c=(t-1+2*(o-1))%i+1+i,l=c>n;r.push({match_id:da(),stevneid:e,fase:`innledende`,runde_nummer:o,bane_nummer:t,er_bekreftet:!1,er_walkover:l}),s.push({p1Pos:a,p2Pos:c,erWalkover:l})}let{data:c,error:l}=await y.from(`kamp`).insert(r).select(`id, bane_nummer`);if(l)throw Error(`Feil ved innsetting av kampar (runde ${o}): `+l.message);let u=Object.fromEntries(c.map(e=>[e.bane_nummer,e.id])),d=[];for(let e=0;e<i;e++){let n=u[e+1],{p1Pos:r,p2Pos:i,erWalkover:a}=s[e];d.push({kampid:n,kasterid:t[r],posisjon:1,score_poeng:0,kamp_poeng:0,antall_ringer:0}),a||d.push({kampid:n,kasterid:t[i],posisjon:2,score_poeng:0,kamp_poeng:0,antall_ringer:0})}let{error:f}=await y.from(`kamp_spelar`).insert(d);if(f)throw Error(`Feil ved innsetting av spelarar (runde ${o}): `+f.message);a+=c.length}return a}async function ma(e,t,n){let r=[],i=[],a=1;for(let t=1;t<=n;t+=2){let o=t+1>n;r.push({match_id:da(),stevneid:e,fase:`innledende`,runde_nummer:1,bane_nummer:a,er_bekreftet:!1,er_walkover:o}),i.push({p1Pos:t,p2Pos:o?null:t+1,erWalkover:o}),a++}let{data:o,error:s}=await y.from(`kamp`).insert(r).select(`id, bane_nummer`);if(s)throw Error(`Feil ved innsetting av Swiss runde 1: `+s.message);let c=Object.fromEntries(o.map(e=>[e.bane_nummer,e.id])),l=[];for(let e=0;e<i.length;e++){let n=c[e+1],{p1Pos:r,p2Pos:a,erWalkover:o}=i[e];l.push({kampid:n,kasterid:t[r],posisjon:1,score_poeng:0,kamp_poeng:0,antall_ringer:0}),o||l.push({kampid:n,kasterid:t[a],posisjon:2,score_poeng:0,kamp_poeng:0,antall_ringer:0})}let{error:u}=await y.from(`kamp_spelar`).insert(l);if(u)throw Error(`Feil ved innsetting av Swiss spelarar: `+u.message);return o.length}async function ha(e){let{data:t,error:n}=await y.from(`kamp`).select(`id, runde_nummer, er_bekreftet, er_walkover, spelarar:kamp_spelar(kasterid, kamp_poeng, score_poeng, posisjon)`).eq(`stevneid`,e).eq(`fase`,`innledende`).order(`runde_nummer`);if(n)throw Error(`Feil ved henting av kampar: `+n.message);let r=t,i=Math.max(...t.map(e=>e.runde_nummer))+1,a=new Set;for(let e of r)for(let t of e.spelarar??[])t.kasterid!=null&&a.add(t.kasterid);let o=[...a],s={};for(let e of o)s[e]=o.filter(t=>t!==e);for(let e of r){let t=(e.spelarar??[]).filter(e=>e.kasterid!=null);if(t.length===2){let e=t[0].kasterid,n=t[1].kasterid;s[e]=s[e].filter(e=>e!==n),s[n]=s[n].filter(t=>t!==e)}}let c={};for(let e of o)c[e]=0;for(let e of t){if(!e.er_walkover)continue;let t=(e.spelarar??[]).find(e=>e.posisjon===1);t?.kasterid!=null&&(c[t.kasterid]=(c[t.kasterid]??0)+1)}let l=ua(o.map(e=>{let n=0,i=0;for(let t of r){let r=(t.spelarar??[]).find(t=>t.kasterid===e);r&&(n+=r.kamp_poeng??0,i+=0)}for(let n of t){let t=(n.spelarar??[]).find(t=>t.kasterid===e);t&&(i+=t.score_poeng??0)}return{kasterid:e,kamp_poeng:n,score_poeng:i}}),r);function u(e){for(let t=e.length-1;t>=0;t--)if((c[e[t].kasterid]??0)<1)return e[t];return null}function d(e,t){if(e.length===0)return t;if(e.length%2==1){let n=u(e);return n?(c[n.kasterid]++,t.push({p1:n.kasterid,p2:null,erWalkover:!0}),d(e.filter(e=>e.kasterid!==n.kasterid),t)||(c[n.kasterid]--,t.pop(),null)):null}for(let n=0;n<e.length;n++){let r=e[n];for(let i=n+1;i<e.length;i++){let n=e[i];if(s[r.kasterid]?.includes(n.kasterid)){t.push({p1:r.kasterid,p2:n.kasterid,erWalkover:!1});let i=d(e.filter(e=>e.kasterid!==r.kasterid&&e.kasterid!==n.kasterid),t);if(i)return i;t.pop()}}}return null}let f=d(l,[]);if(!f)throw Error(`Paring er ikkje mogleg. Alle moglege motstandarar er allereie spela.`);f.sort((e,t)=>!!e.erWalkover-+!!t.erWalkover);let p=f.map((t,n)=>({match_id:da(),stevneid:e,fase:`innledende`,runde_nummer:i,bane_nummer:n+1,er_bekreftet:!1,er_walkover:t.erWalkover})),{data:m,error:h}=await y.from(`kamp`).insert(p).select(`id, bane_nummer`);if(h)throw Error(`Feil ved innsetting av ny Swiss-runde: `+h.message);let g=Object.fromEntries(m.map(e=>[e.bane_nummer,e.id])),_=[];for(let e=0;e<f.length;e++){let{p1:t,p2:n,erWalkover:r}=f[e],i=g[e+1];_.push({kampid:i,kasterid:t,posisjon:1,score_poeng:0,kamp_poeng:0,antall_ringer:0}),r||_.push({kampid:i,kasterid:n,posisjon:2,score_poeng:0,kamp_poeng:0,antall_ringer:0})}let{error:v}=await y.from(`kamp_spelar`).insert(_);if(v)throw Error(`Feil ved innsetting av Swiss spelarar: `+v.message);return{rundeNummer:i,antallKampar:m.length}}async function ga(e,{id:t,isAdmin:n=!1},r=null){e.replaceChildren(A());try{let[i,a,o]=await Promise.all([fe(t),Or(t),M()]);if(i.error||!i.data){e.replaceChildren(k(`Stevne ikkje funne.`));return}let s=i.data,c=s.stevne_fase??null,l=c===null||c===`ikke_startet`,u=s.kastemetodeInnl?.navn??`—`,d=u.toLowerCase().includes(`gloppen`);if(r&&l&&n){r.innerHTML=`<button id="start-stevne-btn" class="btn btn-sm btn-success">Start stevne</button>`;let e=r.querySelector(`#start-stevne-btn`);e.addEventListener(`click`,async()=>{if(a<2){Q(`Stevnet må ha minst 2 spelarar for å startast.`,`error`);return}if(d&&!s.antall_runder_innl){Q(`Du må setje antal rundar for innledande fase. Gå til Innstillingar for å endre.`,`error`);return}let n=await kr(t);if(n>0&&!await X({title:`Ubekrefta spelarar`,message:`${n} spelar(ar) er ikkje bekrefta. Vil du starte stevnet likevel?`}))return;e.disabled=!0,e.textContent=`Starter…`;try{await fa(t,u,s.antall_runder_innl??1)}catch(t){Q(`Feil ved kampgenerering: `+(t instanceof Error?t.message:String(t)),`error`),e.disabled=!1,e.textContent=`Start stevne`;return}let{error:r}=await pe(t,`innledende`);if(r){Q(`Feil ved oppdatering av fase.`,`error`),e.disabled=!1,e.textContent=`Start stevne`;return}location.hash=`#/stevne/${t}/innledende`})}e.innerHTML=`
      ...
      </div>`;let u=e.querySelector(`#spelarar-layout`),d=document.createElement(`div`);d.className=`col-md-6 d-flex flex-column`;let f=document.createElement(`input`);f.type=`text`,f.placeholder=`Søk etter navn eller klubb…`,f.className=`form-control mb-2`;let{kolonne:p,tabell:m}=ya(`Tilgjengelege spelarar`);d.appendChild(f),d.appendChild(p),u.appendChild(d);let h=document.createElement(`div`);h.className=`col-md-6 d-flex flex-column`;let g=document.createElement(`input`);g.type=`text`,g.className=`form-control mb-2 deltaker-search-spacer`,g.tabIndex=-1,g.disabled=!0;let{kolonne:_,tabell:v,tittelEl:y}=ya(`Påmelde spelarar`);h.appendChild(g),h.appendChild(_),u.appendChild(h);function b(){v.innerHTML=``;let e=_a(c.filter(e=>l.has(e.id)));if(y.textContent=`Påmelde spelarar: ${e.length}`,!e.length){v.appendChild(Sa(`Ingen spelarar påmelde`,4));return}for(let n of e)v.appendChild(ba(n,l.get(n.id)??!1,async e=>{let{error:n}=await Nr(t,e.id);if(n){Q(`Feil ved fjerning: `+(n instanceof Error?n.message:String(n)),`error`);return}l.delete(e.id),b(),x()},async e=>{let{error:n}=await Mr(t,e.id);if(n){Q(`Feil ved bekreftelse: `+(n instanceof Error?n.message:String(n)),`error`);return}l.set(e.id,!0),b()},!s))}function x(){let e=_a(va(c,f.value,l));if(m.innerHTML=``,!e.length){m.appendChild(Sa(`Ingen spelarar funne`,2));return}for(let n of e)m.appendChild(xa(n,async e=>{let{error:n}=await jr(t,e.id);if(n){Q(`Feil ved innmelding: `+(n instanceof Error?n.message:String(n)),`error`);return}l.set(e.id,!1),b(),x()},!s))}f.addEventListener(`input`,x),b(),x()}catch(t){b(`stevne-deltakere.render`,t),e.replaceChildren(k(`Kunne ikkje laste deltakarliste.`))}}var wa=`modulepreload`,Ta=function(e){return`/dev/`+e},Ea={},Da=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}r=o(t.map(t=>{if(t=Ta(t,n),t in Ea)return;Ea[t]=!0;let r=t.endsWith(`.css`),i=r?`[rel="stylesheet"]`:``;if(n)for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${t}"]${i}`))return;let o=document.createElement(`link`);if(o.rel=r?`stylesheet`:wa,r||(o.as=`script`),o.crossOrigin=``,o.href=t,a&&o.setAttribute(`nonce`,a),document.head.appendChild(o),r)return new Promise((e,n)=>{o.addEventListener(`load`,e),o.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})};async function Oa(e,{id:t,isAdmin:n=!1},r=null){e.replaceChildren(A());let{navn:i,error:a}=await Me(t);if(a){e.replaceChildren(k(`Stevne ikkje funne.`));return}if(i.includes(`gloppen`)){let{render:i}=await Da(async()=>{let{render:e}=await import(`./gloppen-DMaOeWaa.js`);return{render:e}},__vite__mapDeps([0,1,2]));await i(e,{id:t,isAdmin:n},r)}else if(i.includes(`nordhordland`)){let{render:i}=await Da(async()=>{let{render:e}=await import(`./nordhordland-B-Ri_tRp.js`);return{render:e}},__vite__mapDeps([3,1,2]));await i(e,{id:t,isAdmin:n},r)}else if(i.includes(`x-kast`)||i.includes(`minimatch`)||i.includes(`halvmatch`)||i.includes(`heilmatch`)){let{render:i}=await Da(async()=>{let{render:e}=await import(`./xkast-DncJ5zuY.js`);return{render:e}},[]);await i(e,{id:t,isAdmin:n},r)}else e.replaceChildren(k(`Ukjend innledande kastemetode: ${i||`(ikkje sett)`}`))}async function ka(e,{id:t,isAdmin:n=!1},r=null){e.replaceChildren(A());let{navn:i,error:a}=await Ne(t);if(a){e.replaceChildren(k(`Stevne ikkje funne.`));return}if(i.includes(`cup`)){let{render:i}=await Da(async()=>{let{render:e}=await import(`./cup-2yH8U84B.js`);return{render:e}},__vite__mapDeps([4,2]));await i(e,{id:t,isAdmin:n},r)}else if(i.includes(`kongelag`)){let{render:i}=await Da(async()=>{let{render:e}=await import(`./kongelag-Bu4wtmI0.js`);return{render:e}},[]);await i(e,{id:t,isAdmin:n},r)}else if(i.includes(`nordhordland`)){let{render:i}=await Da(async()=>{let{render:e}=await import(`./nordhordland-BRVzg0-i.js`);return{render:e}},[]);await i(e,{id:t,isAdmin:n},r)}else e.replaceChildren(k(`Ukjend avsluttande kastemetode: ${i||`(ikkje sett)`}`))}y.from(`kamp`).select(`id, er_walkover, spelarar:kamp_spelar(id, kasterid)`);function Aa(){let e=Math.floor(Math.random()*27),t=Math.floor(Math.random()*27);return e<21&&t<21?Math.random()<.5?[Math.floor(Math.random()*6)+21,t]:[e,Math.floor(Math.random()*6)+21]:[e,t]}async function ja(e){let{data:t,error:n}=await y.from(`kamp`).select(`id, er_walkover, spelarar:kamp_spelar(id, kasterid)`).eq(`stevneid`,e).eq(`fase`,`innledende`).eq(`er_bekreftet`,!1);if(n){b(`autoFullforInnledendeKamper`,n);return}if(t?.length)for(let e of t){let[t,n]=e.spelarar??[],[r,i]=e.er_walkover?[21,0]:Aa(),[a,o]=Pr(r,i);try{let s=[y.from(`kamp`).update({er_bekreftet:!0}).eq(`id`,e.id)];t&&s.push(y.from(`kamp_spelar`).update({score_poeng:r,kamp_poeng:a}).eq(`id`,t.id)),n&&s.push(y.from(`kamp_spelar`).update({score_poeng:i,kamp_poeng:o}).eq(`id`,n.id)),await Promise.all(s)}catch(e){b(`autoFullforInnledendeKamper:update`,e)}}}async function Ma(e,t){let{data:n,error:r}=await y.from(`kamp`).select(`id`).eq(`stevneid`,e).eq(`fase`,t);if(r){b(`slettKamperForFase:kamp`,r);return}let i=(n??[]).map(e=>e.id);if(!i.length)return;let{data:a,error:o}=await y.from(`kamp_spelar`).select(`id`).in(`kampid`,i);if(o){b(`slettKamperForFase:spelar`,o);return}let s=(a??[]).map(e=>e.id);if(s.length){let{error:e}=await y.from(`kamp_omgang`).delete().in(`kamp_spelar_id`,s);if(e){b(`slettKamperForFase:omgang`,e);return}let{error:t}=await y.from(`kamp_spelar`).delete().in(`kampid`,i);if(t){b(`slettKamperForFase:spelarDel`,t);return}}let{error:c}=await y.from(`kamp`).delete().in(`id`,i);c&&b(`slettKamperForFase:kampDel`,c)}async function Na(e){await Ma(e,`avsluttende`),await Ma(e,`innledende`);let{error:t}=await y.from(`resultat`).delete().eq(`stevneid`,e);if(t){b(`nullstillStevne:resultat`,t);return}let{error:n}=await y.from(`stevne`).update({stevne_fase:`ikke_startet`,runde1_format:null}).eq(`id`,e);n&&b(`nullstillStevne:stevne`,n)}async function Pa(e,{id:t}){e.replaceChildren(A());try{let[n,r]=await Promise.all([Oe(t),ke()]);if(n.error||!n.data){e.replaceChildren(k(`Stevne ikkje funne.`));return}let i=n.data,a=r.data,o=a.filter(e=>e.er_innledende),s=a.filter(e=>e.er_avsluttende);function c(e,t){return e.map(e=>`<option value="${e.id}"${e.id===t?` selected`:``}>${j(e.navn)}</option>`).join(``)}e.innerHTML=`
      <div>
...
`);async function Ra(e){let{data:t,error:n}=await y.from(`resultat`).select(`
      kasterid, startnummer, plassering, runde_eliminert,
      gruppe:gruppeid(id, navn)
    `).eq(`stevneid`,e);return n&&b(`hentResultatForAvsluttende`,n),{data:t??[],error:n}}async function za(e){let{data:t,error:n}=await y.from(`gruppe`).select(`id, navn`).in(`navn`,e);return n&&b(`hentGrupper`,n),{data:t??[],error:n}}async function Ba(e,t){if(!t.length)return{error:null};let n=(await Promise.all(t.map(t=>y.from(`resultat`).update({gruppeid:t.gruppeid}).eq(`stevneid`,e).eq(`kasterid`,t.kasterid)))).find(e=>e.error)?.error??null;return n&&b(`setGruppeInndeling`,n),{error:n}}async function Va(e,t){if(!t.length)return{error:null};let n=(await Promise.all(t.map((t,n)=>y.from(`resultat`).update({plassering:n+1}).eq(`stevneid`,e).eq(`kasterid`,t.kasterid)))).find(e=>e.error)?.error??null;return n&&b(`skrivPlaseringar`,n),{error:n}}async function Ha(e){let{error:t}=await y.from(`resultat`).update({gruppeid:null}).eq(`stevneid`,e);return t&&b(`clearGruppeInndeling`,t),{error:t}}async function Ua(e){let{data:t,error:n}=await y.from(`resultat`).select(`
      plassering, nc_poeng,
      kaster:kasterid(id, fornavn, etternavn),
      klubb:klubbid(navn),
      klasse:klasseid(navn),
      gruppe:gruppeid(navn)
    `).eq(`stevneid`,e).order(`plassering`);return n&&b(`hentResultaterForStevne`,n),{data:t??[],error:n}}function Wa(e,t){let n=new Map;for(let r of e){let e=r.gruppe?.navn??`–`,i=r.klasse?.navn??null,a=t?`${i??``}|${e}`:e,o=t?`${i?i+` `:``}${e}`:e;n.has(a)||n.set(a,{label:o,rader:[]}),n.get(a).rader.push(r)}return[...n.values()].sort((e,t)=>e.label.localeCompare(t.label,`nb`))}function Ga(e){let t=e.rader.map(e=>`
    <div class="res-rad">
    ...
    </div>`}function Ka(e){let t=e.rader.map(e=>{let t=e.kaster,n=t?`<a href="#/kastere/${s(t)}" class="res-kaster-lenke">${j(a(t))}</a>`:`–`;return`
      <tr>
    ...
    <div class="res-tabell-seksjon">
    ...
    </div>`}async function qa(e,{id:t}){e.replaceChildren(A(`Laster resultat…`));try{let[n,r]=await Promise.all([Fa(t),Ua(t)]);if(n.error||!n.data){e.replaceChildren(k(`Kunne ikkje laste stevnet.`));return}if(r.error){e.replaceChildren(k(`Kunne ikkje laste resultat.`));return}let i=n.data,a=r.data;if(!a.length){e.replaceChildren(L(i.erfullfort?`Ingen resultat registrert.`:`Turneringa er ikkje avslutta enno.`));return}let o=Wa(a,(i.dato?new Date(i.dato+`T12:00:00`).getFullYear():9999)<2026),s=a.length;e.innerHTML=`
      <div class="res-side">
      ...
      </div>`}catch(t){b(`stevne-resultat.render`,t),e.replaceChildren(k(`Kunne ikkje laste resultat.`))}}var Ja=[{key:`info`,label:`Info`,adminOnly:!1},{key:`deltakere`,label:`Deltakere`,adminOnly:!0},{key:`innledende`,label:`Innledande`,adminOnly:!1},{key:`avsluttende`,label:`Avsluttande`,adminOnly:!1},{key:`resultat`,label:`Sluttresultat`,adminOnly:!1},{key:`innstillinger`,label:`Innstillingar`,adminOnly:!0}],Ya=new Set(Ja.filter(e=>e.adminOnly).map(e=>e.key)),Xa={info:ga,deltakere:Ca,innledende:Oa,avsluttende:ka,innstillinger:Pa,resultat:qa},Za={ikke_startet:`<span class="badge bg-secondary">Ikkje starta</span>`,innledende:`<span class="badge bg-primary">Innledande fase</span>`,avsluttende:`<span class="badge bg-success">Avsluttande fase</span>`};function Qa(e,t,n,r){return`<ul class="nav nav-tabs mb-3">${Ja.filter(e=>n||!e.adminOnly).filter(e=>e.key!==`avsluttende`||r).map(({key:n,label:r})=>`
      <li class="nav-item">
        <a class="nav-link${t===n?` active`:``}"
           href="#/stevne/${e}/${n}">${r}</a>
      </li>`).join(``)}</ul>`}var $a=null;async function eo(e,{id:t,tab:n=`info`}){$a&&=(await Vi($a),null),e.replaceChildren(A());try{let{data:r,error:i}=await Ce(t);if(i||!r){e.replaceChildren(k(`Stevne ikkje funne.`));return}let a=await N()||await at(),o=r.avsluttendekastemetodeid!=null,s=!a&&Ya.has(n)?`info`:n,c=Za[r.stevne_fase??`ikke_startet`]??``;e.innerHTML=`
      ...
      </div>`;let l=e.querySelector(`#org-banner-knappar`),u=e.querySelector(`#org-subside`);await(Xa[s]??ga)(u,{id:t,isAdmin:a},l),$a=we(t,t=>{let n=e.querySelector(`#fase-badge`);n&&(n.innerHTML=Za[t??`ikke_startet`]??``)})}catch(t){b(`stevne.render`,t),e.replaceChildren(k(`Kunne ikkje laste stevnet.`))}}{let e=document.querySelector(`.header-versjon`);if(e){e.textContent+=` [DEV]`;let t=document.createElement(`span`);t.className=`dev-banner`,t.textContent=`TEST`,e.after(t)}}var to=document.getElementById(`app`);function no(e,t){return async(n,r)=>{if(!await M()){location.hash=`#/logginn`;return}if(e===`admin`&&!await N()){n.replaceChildren(k(`Ingen tilgang.`));return}if(e===`klubbadmin`&&!await N()&&!await at()){n.replaceChildren(k(`Ingen tilgang.`));return}await t(n,r)}}var ro=[{pattern:/^\/logginn$/,side:Cr,params:()=>({})},{pattern:/^\/minside$/,side:no(`bruker`,gi),params:()=>({})},{pattern:/^\/admin$/,side:no(`admin`,xi),params:()=>({})},{pattern:/^\/stevne\/ny$/,side:no(`klubbadmin`,Mi),params:()=>({})},{pattern:/^\/stevne\/(\d+)\/admin$/,side:no(`klubbadmin`,Mi),params:e=>({id:e[1]})},{pattern:/^\/kamp\/(\d+)$/,side:Xi,params:e=>({id:Number(e[1])})},{pattern:/^\/stevne\/(\d+)\/pamelding$/,side:Bi,params:e=>({id:e[1]})},{pattern:/^\/stevne\/(\d+)(?:\/([^/]*))?$/,side:eo,params:e=>({id:Number(e[1]),tab:e[2]??`info`})},{pattern:/^\/kaster\/ny$/,side:no(`klubbadmin`,Ni),params:()=>({})},{pattern:/^\/kaster\/(\d+)\/admin$/,side:no(`klubbadmin`,Ni),params:e=>({id:e[1]})},{pattern:/^\/klubber\/(\d+)\/admin$/,side:no(`klubbadmin`,Pi),params:e=>({id:e[1]})},{pattern:/^\/terminliste$/,side:Ct,params:()=>({})},{pattern:/^\/norgescupen$/,side:It,params:()=>({})},{pattern:/^\/norgesranking$/,side:Jt,params:()=>({})},{pattern:/^\/rekorder$/,side:ar,params:()=>({})},{pattern:/^\/nmvinnere$/,side:br,params:()=>({})},{pattern:/^\/kastere\/(\d+)(-[^/]*)?$/,side:In,params:e=>({id:e[1]})},{pattern:/^\/kastere$/,side:In,params:()=>({})},{pattern:/^\/klubber\/(\d+)(-[^/]*)?$/,side:Zn,params:e=>({id:e[1]})},{pattern:/^\/klubber$/,side:Zn,params:()=>({})},{pattern:/^\/?$/,side:Be,params:()=>({})}];function io(){let e=location.hash.replace(/^#/,``)||`/`;for(let t of ro){let n=e.match(t.pattern);if(n){t.side(to,t.params(n));return}}to.replaceChildren(k(`Side ikkje funne.`))}async function ao(){let e=await M(),t=document.getElementById(`meny-logginn-item`),n=document.getElementById(`meny-minside-item`),r=document.getElementById(`meny-admin-item`),i=document.getElementById(`meny-loggut-item`);if(e){t.classList.add(`d-none`);let a=e.profil?.rolle===`admin`;n.classList.toggle(`d-none`,a),r.classList.toggle(`d-none`,!a),i.classList.remove(`d-none`)}else t.classList.remove(`d-none`),n.classList.add(`d-none`),r.classList.add(`d-none`),i.classList.add(`d-none`)}window.addEventListener(`hashchange`,io),document.addEventListener(`DOMContentLoaded`,()=>{document.getElementById(`menyLoggUtKnapp`).addEventListener(`click`,async()=>{await ot(),location.hash=`#/`}),ao(),io()}),document.addEventListener(`authStateChanged`,()=>{ao()});export{Yr as A,Te as B,Q as C,qr as D,Qr as E,Br as F,Fe as G,De as H,oi as I,k as J,j as K,Pr as L,Xr as M,Vr as N,Zr as O,$r as P,Fr as R,ua as S,X as T,pe as U,Pe as V,Ee as W,y as X,b as Y,sa as _,La as a,ra as b,ja as c,ia as d,na as f,aa as g,ea as h,Ia as i,Rr as j,zr as k,ha as l,ca as m,za as n,Ba as o,la as p,A as q,Ra as r,Va as s,Ha as t,Qi as u,$i as v,Vi as w,ta as x,oa as y,Ir as z};