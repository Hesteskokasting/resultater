import{t as e}from"./logError-ByTg738k.js";import{S as t,_r as n,b as r,f as i,i as a,vr as o,y as s,yr as c}from"./index-DSCLt71t.js";import{a as l,n as u,r as d}from"./klubbService-pPakrKDt.js";import{o as f,r as p}from"./kasterService-BN8H2rLx.js";import{t as m}from"./SearchInput-CDoDGwhR.js";import{t as h}from"./AdminLinkBar-DnnShfRb.js";import{t as g}from"./Table-B_UMcRWp.js";function _(e){let n=new Map;for(let r of e){if(!r.klubb?.id)continue;let e=t(r).toLowerCase(),i=n.get(r.klubb.id);i?i.push(e):n.set(r.klubb.id,[e])}return n}function v(e,t,n){let r=n.trim().toLowerCase();return r?e.filter(e=>e.navn.toLowerCase().includes(r)||(t.get(e.id)??[]).some(e=>e.includes(r))):e}function y(e,n){let r=n.trim().toLowerCase();return r?e.filter(e=>t(e).toLowerCase().includes(r)):e}var b={searchText:``};function x(e,t){return`
    ...
    </div>`}function S(e,i){let a=y(e,i);if(!a.length)return n(`Ingen aktive utøvarar funnet.`);let o=document.createElement(`div`);return o.className=`table-responsive`,o.appendChild(g({rows:a,columns:[{label:`#`,render:(e,t)=>String(t+1)},{label:`Utøvar`,render:e=>{let n=document.createElement(`a`);return n.href=`#/kastere/${r(e)}`,n.className=`app-link`,n.textContent=t(e),n}},{label:`Klasse`,render:e=>e.klasse?.navn??`–`},{label:`Nr.`,render:e=>String(e.medlemsnummer??`–`)}]})),o}async function C(t,n){b.searchText=``,t.replaceChildren(c(`Laster klubb...`));try{let[e,{data:r}]=await Promise.all([d(n),f(n)]);if(e.error||!e.data){t.replaceChildren(o(`Kunne ikkje laste klubb.`));return}let i=e.data;a(i.navn),t.innerHTML=x(i,r.length);let s=t.querySelector(`#club-detail-list`);function c(){s.replaceChildren(S(r,b.searchText))}m({slot:t.querySelector(`#club-detail-search-slot`),placeholder:`Søk på utøvar`,state:b,onInput:c}),c(),h(t,{href:`#/klubber/${n}/admin`,label:`Rediger klubb`,variant:`warning`,canShow:e=>e.profil?.role===`admin`||e.profil?.role===`klubbadmin`&&e.clubs.includes(n)})}catch(n){e(`klubber.renderDetail`,n),t.replaceChildren(o(`Kunne ikkje laste klubb.`))}}var w={showAll:!1,searchText:``};function T(e){return`
    <a href="#/klubber/${s(e)}" class="thrower-card">
    ...
    </div>`}function D(){return w.showAll?u():l()}async function O(t){t.replaceChildren(c(`Laster klubbar...`));try{let[{data:e,error:n},{data:r}]=await Promise.all([D(),p()]);if(n){t.replaceChildren(o(`Kunne ikkje laste klubbar.`));return}let i=_(r);t.innerHTML=E();let a=e,s=t.querySelector(`#club-grid`),c=t.querySelector(`#club-active-only`);function l(){let e=v(a,i,w.searchText);s.innerHTML=e.length?e.map(e=>T(e)).join(``):`<p class="empty-state">Ingen klubbar funnet.</p>`}m({slot:t.querySelector(`#club-search-slot`),placeholder:`Søk på klubbnavn eller utøvar`,state:w,onInput:l}),l(),c.addEventListener(`change`,async()=>{w.showAll=!c.checked;let{data:e,error:t}=await D();t||(a=e),l()}),h(t,{href:`#/klubber/ny`,label:`+ Ny klubb`,variant:`success`,canShow:e=>e.profil?.role===`admin`})}catch(n){e(`klubber.renderList`,n),t.replaceChildren(o(`Kunne ikkje laste klubbar.`))}}var k=async(e,t)=>{t.id?await C(e,Number(t.id)):await O(e)};export{k as render};