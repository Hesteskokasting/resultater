import{t as e}from"./logError-Cjb5zwtM.js";import{r as t}from"./authService-D2ZvDNn8.js";import{$ as n,D as r,I as i,Q as a,R as o,Z as s,at as c,r as l,t as u,tt as d}from"./index-VrrqWszT.js";import{t as f}from"./LoadingState-BCLCa55U.js";import{t as p}from"./buildDropdownOptions-hFRe-R8M.js";var m={column:`dato`,direction:`asc`};function h(e,t){switch(t){case`navn`:return e.navn??``;case`dato`:return e.dato??``;case`sted`:return e.sted??``;case`metode`:return[e.innledende?.navn,e.avsluttende?.navn].filter(e=>!!e).join(` `);case`organizer`:return e.klubb?.navn??``;case`type`:return e.stevnetype?.navn??``;case`klassifisering`:return e.kategori?.navn??``;default:return``}}function g(e){return[...e].sort((e,t)=>{let n=h(e,m.column),r=h(t,m.column),i=n.localeCompare(r,`nb`);return m.direction===`asc`?i:-i})}var _={year:new Date().getFullYear(),text:``,tournamentTypeId:``,throwingMethodId:``,clubId:``,categoryId:``},v=[],y=null,b=new Map;function x(e){return e.filter(e=>{if(_.text){let t=_.text.toLowerCase();if(![e.navn,e.sted,e.klubb?.navn,e.stevnetype?.navn,e.kategori?.navn,e.innledende?.navn,e.avsluttende?.navn].some(e=>e?.toLowerCase().includes(t)))return!1}if(_.tournamentTypeId&&String(e.stevnetype?.id)!==_.tournamentTypeId)return!1;if(_.throwingMethodId){let t=_.throwingMethodId;if(!(String(e.innledende?.id)===t||String(e.avsluttende?.id)===t))return!1}return!(_.clubId&&String(e.klubb?.id)!==_.clubId||_.categoryId&&String(e.kategori?.id)!==_.categoryId)})}async function S(e){await n(e.map(e=>({Dato:e.dato?new Date(e.dato).toLocaleDateString(`nb-NO`):``,Navn:e.navn??``,Sted:e.sted??``,Arrangør:e.klubb?.navn??``,Stevnetype:e.stevnetype?.navn??``,"Kastemetode (innledende)":e.innledende?.navn??``,"Kastemetode (avsluttende)":e.avsluttende?.navn??``,Kategori:e.kategori?.navn??``,NM:e.ernm?`Ja`:`Nei`,InnbydelseUrl:e.innbydelseurl??``})),`terminliste-${_.year}.xlsx`,`Terminliste`)}var C=[{id:`navn`,label:`Stevne`},{id:`dato`,label:`Dato`},{id:`sted`,label:`Sted`},{id:`metode`,label:`Metode`},{id:`organizer`,label:`Arrangør`},{id:`type`,label:`Type`},{id:`klassifisering`,label:`Klassifisering`}];function w(e){return m.column===e?m.direction===`asc`?`<span class="tl-sort-icon active">↑</span>`:`<span class="tl-sort-icon active">↓</span>`:`<span class="tl-sort-icon">↕</span>`}function T(e){let t=e.dato?new Date(e.dato+`T12:00:00`).toLocaleDateString(`nb-NO`):``,n=[e.innledende?.navn,e.avsluttende?.navn].filter(e=>!!e).join(` \\ `),r=e.ernm?`<span class="tl-nm-merke">NM</span> `:``,i=e.innbydelseurl?`<a href="${s(e.innbydelseurl)}" target="_blank" rel="noopener" class="tl-invitation-icon" title="Innbydelse">📄</a>`:``;return`<tr class="tl-tr">
  ...
  </tr></thead>`}${`<tbody>${g(e).map(T).join(``)}</tbody>`}</table>`}function D(e){return window.innerWidth>600?E(e):k(e)}function O(e){let t=d(e.dato),n=e.sted?`<p class="tl-detail">Sted: ${s(e.sted)}</p>`:``,r=e.klubb?`<p class="tl-detail">Arrangør: ${s(e.klubb.navn??``)}</p>`:``,i=e.stevnetype?`<p class="tl-detail">Type: ${s(e.stevnetype.navn??``)}</p>`:``,a=e.ernm?`<span class="tl-nm-merke">NM</span>`:``,o=e.innbydelseurl?`<a class="tl-invitation-link" href="${s(e.innbydelseurl)}" target="_blank" rel="noopener">Innbydelse 📄</a>`:``,c=e.resultaturl?`<a class="tournament-link" href="#/stevne/${e.id}/resultat">Vis resultat</a>`:``,l=e.dato&&new Date(e.dato+`T12:00:00`)>new Date,u=e.stevne_fase===null||e.stevne_fase===`ikke_startet`,f=y?.profil?.kobling_status===`godkjent`&&l&&u&&!e.erfullfort?`<span data-registration-slot="${e.id}"></span>`:``;return`
    <div class="tournament-card tl-kort">
      <a class="tl-name tl-name-link" href="#/stevne/${e.id}/resultat">${a}${s(e.navn??``)}</a>
      <p class="tournament-date">${t}</p>
      ${n}${r}${i}
      ${o}${c}${f}
    </div>
  `}function k(e){return e.length===0?`<p class="empty-state">Ingen stevner funnet med valgte filtre.</p>`:`<div class="tournament-list">${e.map(O).join(``)}</div>`}async function A(n){u(()=>A(n)),n.replaceChildren(f(`Laster terminliste…`));try{let[{data:u,error:d},{data:h},g]=await Promise.all([o(_.year),r(),t()]);if(y=g,b=g?.profil?.kasterid==null?new Map:await i(g.profil.kasterid),d){e(`terminliste.render`,d),n.replaceChildren(a(`Kunne ikkje laste terminliste.`));return}v=u??[];function C(e){return{year:`<select class="tl-select" id="tl-year${e}">${c(_.year,1983,new Date().getFullYear()+1)}</select>`,tournamentType:`<select class="tl-select" id="tl-tournamenttype${e}">${p(h.stevnetyper,_.tournamentTypeId,`Alle typer`)}</select>`,throwingMethod:`<select class="tl-select" id="tl-throwingmethod${e}">${p(h.kastemetoder,_.throwingMethodId,`Alle metoder`)}</select>`,organizer:`<select class="tl-select" id="tl-organizer${e}">${p(h.klubber,_.clubId,`Alle arrangører`)}</select>`,category:`<select class="tl-select" id="tl-category${e}">${p(h.kategorier,_.categoryId,`Alle kategorier`)}</select>`}}let w=C(``),T=C(`-mobil`);n.innerHTML=`
      <div class="terminliste">
        ...
        <div class="tl-filter-row">
          ${w.year}
          <input class="tl-input" id="tl-text" type="search" placeholder="Søk..." value="${s(_.text)}">
          ${w.tournamentType}
          ${w.throwingMethod}
          ${w.organizer}
          ${w.category}
        ...
        <div class="tl-mobile-row">
          <input class="tl-input" id="tl-text-mobile" type="search" placeholder="Søk..." value="${s(_.text)}">
    ...
    `;function E(){let e=x(v),t=n.querySelector(`.tl-list-container`);if(!t)return e;t.innerHTML=D(e);let r=n.querySelector(`.tl-count`);r&&(r.textContent=`${e.length} stevner`);let i=y?.profil?.kasterid,a=y?.user.id;return i!=null&&a&&l(t,i,a,b),e}if(E(),g?.profil&&(g.profil.role===`admin`||g.profil.role===`klubbadmin`)){let e=document.createElement(`div`);e.className=`mb-3 px-2 d-flex gap-2`,e.innerHTML=`<a href="#/stevne/ny" class="btn btn-sm btn-success">+ Nytt stevne</a>`,n.querySelector(`.terminliste`)?.prepend(e)}let O=n.querySelector(`.tl-list-container`),k=n.querySelector(`#tl-year`),A=n.querySelector(`#tl-text`),j=n.querySelector(`#tl-text-mobile`),M=n.querySelector(`#tl-tournamenttype`),N=n.querySelector(`#tl-throwingmethod`),P=n.querySelector(`#tl-organizer`),F=n.querySelector(`#tl-category`),I=n.querySelector(`#tl-excel-desktop`),L=n.querySelector(`#tl-excel-mobile`),R=n.querySelector(`#tl-filter-open`),z=n.querySelector(`#tl-sheet`),B=n.querySelector(`#tl-backdrop`),V=n.querySelector(`#tl-reset`),H=n.querySelector(`#tl-apply`),U=n.querySelector(`#tl-year-mobil`),W=n.querySelector(`#tl-tournamenttype-mobil`),G=n.querySelector(`#tl-throwingmethod-mobil`),K=n.querySelector(`#tl-organizer-mobil`),q=n.querySelector(`#tl-category-mobil`);O.addEventListener(`click`,e=>{let t=e.target.closest(`[data-column]`);if(!t)return;let n=t.dataset.column;m.column===n?m.direction=m.direction===`asc`?`desc`:`asc`:(m.column=n,m.direction=`asc`),E()});let J=null;function Y(){if(!n.querySelector(`.tl-list-container`)){window.removeEventListener(`resize`,Y);return}J!==null&&clearTimeout(J),J=setTimeout(E,200)}window.addEventListener(`resize`,Y);async function X(t){n.querySelector(`.tl-title`).textContent=`Terminliste ${_.year}`,n.querySelector(`.tl-list-container`).replaceChildren(f(`Laster...`));let{data:r,error:i}=await o(_.year);return i?(e(t,i),n.querySelector(`.tl-list-container`).replaceChildren(a(`Feil ved henting.`)),!1):(v=r??[],!0)}k.addEventListener(`change`,async()=>{_.year=Number(k.value),await X(`terminliste.yearChange`)&&E()}),A.addEventListener(`input`,()=>{_.text=A.value,E()}),j.addEventListener(`input`,()=>{_.text=j.value,A.value=j.value,E()}),M.addEventListener(`change`,()=>{_.tournamentTypeId=M.value,E()}),N.addEventListener(`change`,()=>{_.throwingMethodId=N.value,E()}),P.addEventListener(`change`,()=>{_.clubId=P.value,E()}),F.addEventListener(`change`,()=>{_.categoryId=F.value,E()});let Z=()=>S(x(v));I.addEventListener(`click`,Z),L.addEventListener(`click`,Z);function Q(){z.classList.add(`active`),B.classList.add(`active`)}function $(){z.classList.remove(`active`),B.classList.remove(`active`)}R.addEventListener(`click`,Q),B.addEventListener(`click`,$),V.addEventListener(`click`,()=>{_.text=``,_.tournamentTypeId=``,_.throwingMethodId=``,_.clubId=``,_.categoryId=``,W.value=``,G.value=``,K.value=``,q.value=``,j.value=``,A.value=``,E()}),H.addEventListener(`click`,async()=>{let e=Number(U.value),t=e!==_.year;_.year=e,_.tournamentTypeId=W.value,_.throwingMethodId=G.value,_.clubId=K.value,_.categoryId=q.value,$(),!(t&&!await X(`terminliste.applyFilter`))&&E()})}catch(t){e(`terminliste.render`,t),n.replaceChildren(a(`Kunne ikkje laste terminliste.`))}}export{A as render};