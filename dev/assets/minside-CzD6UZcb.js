import{n as e,t}from"./logError-CB4-2Lin.js";import{A as n,Bt as r,F as i,G as a,H as o,I as s,Lt as c,W as l,Xt as u,c as d,g as f,j as p,k as m,l as h,nt as g,r as _,s as v,t as y}from"./index-CozEoX1x.js";import{t as b}from"./LoadingState-C6NB62Ct.js";import{d as x,r as S}from"./kasterService-D9jqvobU.js";import{t as C}from"./EmptyState-CCNgsnix.js";import{t as w}from"./SearchSelect-ubpkwWhs.js";import{i as ee,n as T,r as E}from"./kaster-2cwCS5i9.js";import{n as D,r as O,t as k}from"./accountService-Bblha1aJ.js";import{f as A,l as te}from"./kampService-BOB7VeHW.js";import{t as ne}from"./Tabs-DZCBJPb0.js";import{t as j,u as re}from"./navigationService-DQ6CEVMS.js";import{n as M}from"./ScoreboardButton-cO_q_Bk1.js";function N(e){return`
    ...
    <div class="card mb-4">
      ...
      </tbody></table>`,f=(e,t)=>{let n=z.map(({key:t,label:n})=>({label:n,matches:e.filter(e=>e.kamp?.fase===t)})).filter(e=>e.matches.length),r=n.reduce((e,t)=>e+t.matches.length,0)===e.length;return n.length<2||!r?u(e,t):n.map(({label:e,matches:n})=>`
    ...
    </div>`}function ae(e,t){for(let[r,i]of[[`varsle-stevne-start`,`varsle_stevne_start`],[`varsle-kamp-opprettet`,`varsle_kamp_opprettet`]]){let o=e.querySelector(`#${r}`);o&&o.addEventListener(`change`,async()=>{let e=o.checked;o.disabled=!0,e&&await a();let{error:r}=await J(t,i,e);o.disabled=!1,r&&(o.checked=!e,n(`Kunne ikkje lagre varslingsinnstilling: ${m(r)}`,`error`))})}}async function oe(e,t){let n=u.isNativePlatform();if(e.innerHTML=`
    ${n?`
      <div class="card mb-4">
        <div class="card-body">
    ...
    <div class="card mb-4"><div class="card-body">
      ...
      <li class="nav-item">
        <a class="nav-link${e===t?` active`:``}"
           href="#/minside/${t}">${n}</a>
      </li>`).join(``)}</ul>`}async function $(e,n){y(()=>$(e,n));let r=String(n.tab??`kampar`);e.replaceChildren(b(`Laster min side…`));try{let t=await i();if(!t){location.hash=`#/logginn`;return}let{profil:n,user:a}=t,o=n?.kobling_status??`ingen`,s=r===`varslingar`?`innstillingar`:r,c=pe.has(s)?s:`kampar`;e.innerHTML=`
      ...
      </div>`,e.querySelector(`[data-slot="logout"]`).appendChild(O());let l=e.querySelector(`#minside-subpage`);await me[c](l,{user:a,profil:n,status:o})}catch(n){t(`minside.render`,n),e.replaceChildren(c(`Kunne ikkje laste min side.`))}}export{$ as render};