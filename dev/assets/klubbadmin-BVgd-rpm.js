import{M as e,a as t,j as n,wt as r}from"./index-BOGgiIrf.js";import{i,n as a,r as o,t as s}from"./adminForms-CWyQ7dys.js";import{i as c,n as l}from"./klubbService-BRsJvfyR.js";import{t as u}from"./LoadingState-C6NB62Ct.js";async function d(d,f={}){let p=f.id===void 0?void 0:Number(f.id);if(!p){d.replaceChildren(r(`Manglande ID.`));return}d.replaceChildren(u());let{data:m,error:h}=await l(p);if(h||!m){d.replaceChildren(r(`Klubb ikkje funne.`));return}if(!await n()&&!await e(p)){d.replaceChildren(r(`Ingen tilgang til denne klubben.`));return}d.innerHTML=`
    ...
    </div>`,d.querySelector(`#club-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=new FormData(e.target),n={navn:t.get(`navn`).trim(),kortnavn:t.get(`kortnavn`).trim(),logourl:t.get(`logourl`).trim()||null,eraktiv:t.get(`eraktiv`)===`on`},{error:r}=await c(p,n);if(r){o(d,s(r));return}i(d,`Klubben er lagra.`)})}export{d as render};