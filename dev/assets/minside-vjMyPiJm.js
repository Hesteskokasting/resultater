import{n as e,t}from"./logError-BO7RC_Nh.js";import{A as n,F as r,It as i,Jt as a,O as o,P as s,Rt as c,U as l,V as u,W as d,c as f,g as ee,k as p,n as m,o as te,s as h,t as g,tt as _}from"./index-D57jhPR5.js";import{d as v,r as y}from"./kasterService-BMY5rO_4.js";import{t as b}from"./LoadingState-C6NB62Ct.js";import{t as x}from"./EmptyState-CCNgsnix.js";import{t as S}from"./SearchInput-BwD50MFz.js";import{n as ne,r as C}from"./kaster-CGWDYFbf.js";import{n as w,t as T}from"./accountService-B2SUOG7y.js";import{d as E,m as D}from"./kampService-Dgr8rFtc.js";import{t as O}from"./Tabs-DZCBJPb0.js";import{t as k}from"./navigationService-CGlIigY9.js";function A(e){return`
    ...
    </div>`}function j(){return`<div class="alert alert-info mb-4">Koblingforespørselen din ventar på godkjenning frå ein administrator.</div>`}function M(e,t){let n=null,i=null,a=e.querySelector(`#thrower-matches`),o=e.querySelector(`#thrower-error`);S({slot:e.querySelector(`#thrower-search-slot`),placeholder:`Søk på navn…`,variant:`form`,onInput:e=>{n!==null&&clearTimeout(n);let t=e.trim().toLowerCase();if(t.length<2){a.innerHTML=``;return}n=setTimeout(async()=>{if(!i){let{data:e}=await y();i=e}let e=i.filter(e=>e.fornavn.toLowerCase().includes(t)||e.etternavn.toLowerCase().includes(t)).slice(0,8);if(!e.length){let e=x(`Ingen treff.`);e.classList.add(`small`),a.replaceChildren(e);return}a.innerHTML=e.map(e=>`<button class="list-group-item list-group-item-action" data-id="${e.id}">
            ${h(C(e))} <span class="text-muted small">· ${h(e.klubb?.navn??``)}</span>
          </button>`).join(``)},300)}}),a.addEventListener(`click`,async e=>{let n=e.target.closest(`[data-id]`);if(!n)return;o.classList.add(`d-none`);let{error:i}=await _(t,Number(n.dataset.id));if(i){o.textContent=`Kunne ikkje sende forespørsel.`,o.classList.remove(`d-none`);return}r(),await m()})}function N(e,t){return t.status===`godkjent`&&t.profil?.kasterid!=null?t.profil.kasterid:t.status===`venter`?(e.innerHTML=j(),null):(e.innerHTML=A(t.status),M(e,t.user.id),null)}function P(e,t,n){let r=N(e,t);return r==null?null:(e.innerHTML=`
    <div class="card mb-4">
      <div class="card-body">
        <h5 class="card-title">${h(n)}</h5>
    ...
    <p class="mb-1"><strong>${h(C(t))}</strong> · ${h(t.klubb?.navn??``)}</p>
    <a href="#/kastere/${ne(t)}" class="btn btn-sm btn-outline-primary mt-1">Vis profil</a>`}function Y(e){let t=e?`Bytt passord`:`Opprett passord`;return`
    ...
    <div class="card mb-4"><div class="card-body">
      ...
      <li class="nav-item">
        <a class="nav-link${e===t?` active`:``}"
           href="#/minside/${t}">${n}</a>
      </li>`).join(``)}</ul>`}async function $(e,n){g(()=>$(e,n));let r=String(n.tab??`kampar`);e.replaceChildren(b(`Laster min side…`));try{let t=await s();if(!t){location.hash=`#/logginn`;return}let{profil:n,user:i}=t,a=n?.kobling_status??`ingen`,o=r===`varslingar`?`innstillingar`:r,c=se.has(o)?o:`kampar`;e.innerHTML=`
      <div class="mypage-container">
        <h2 class="mb-1">Min side</h2>
        <p class="text-muted mb-3">${h(i.email??``)}</p>
        ${le(c)}
