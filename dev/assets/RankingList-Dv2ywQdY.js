import{t as e}from"./escHtml-Z0YwDf7L.js";import{t}from"./expandableRows-DqcsNOuh.js";function n(e,t){let n=1;for(let r=0;r<e.length;r++){let i=e[r];if(i===void 0)continue;let a=e[r-1];a!==void 0&&t(i)<t(a)&&(n=r+1),i.plassering=n}}var r=`<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="12" y1="11" x2="12" y2="17"/><line x1="12" y1="7.5" x2="12" y2="7.5"/></svg>`,i=0;function a({slot:e,label:t=`Vis info`,html:n}){let a=`info-tip-${i++}`,o=document.createElement(`span`);o.className=`info-tip`;let s=document.createElement(`button`);s.type=`button`,s.className=`info-tip__knapp`,s.setAttribute(`aria-label`,t),s.setAttribute(`aria-expanded`,`false`),s.setAttribute(`aria-controls`,a),s.title=t,s.innerHTML=r;let c=document.createElement(`div`);c.className=`info-tip__panel`,c.id=a,c.setAttribute(`role`,`note`),c.hidden=!0,c.innerHTML=n,o.append(s,c);function l(e){c.hidden=!e,s.setAttribute(`aria-expanded`,String(e))}s.addEventListener(`click`,e=>{e.stopPropagation(),l(!!c.hidden)});function u(e){if(!o.isConnected){document.removeEventListener(`click`,u);return}!c.hidden&&!o.contains(e.target)&&l(!1)}function d(e){if(!o.isConnected){document.removeEventListener(`keydown`,d);return}e.key===`Escape`&&!c.hidden&&(l(!1),s.focus())}return document.addEventListener(`click`,u),document.addEventListener(`keydown`,d),e.replaceWith(o),{setHtml:e=>{c.innerHTML=e}}}function o(t,n){return n.length?`
    ...
    <div class="res-tabell-boks">
      <table class="res-table res-table--gruppert rank-table">
        ...
        <tbody>${t.map((e,t)=>c(e,t,r,n)).join(``)}</tbody>
      </table>
    </div>`}function u(t,n,r){let i=r.detail?.(t)??``,a=`${r.idPrefix}-kort-${n}`,o=r.club?.(t),s=r.meta?.(t),c=r.rowClass?.(t);return`
    ...
    <div class="res-mobil-blokk">
      <div class="res-group"><div class="res-group-rows">${e.map((e,n)=>u(e,n,t)).join(``)}</div></div>
    </div>
    <div class="res-desktop-blokk">${l(e,t)}</div>`}function f(e,t){let n=e.querySelector(`.res-detalj-pil, .rank-pil`);n&&(n.textContent=t?`▴`:`▾`)}function p(e){t(e,{trigger:`.res-detalj-btn`,panel:e=>e.closest(`.res-row`)?.querySelector(`.res-detalj`)??null,onToggle:(e,t)=>{let n=e.querySelector(`.res-detalj-tekst`);n&&(n.textContent=t?`Skjul detaljar`:`Vis detaljar`),f(e,t)}}),t(e,{trigger:`.rank-rad--klikk`,panel:e=>{let t=e.nextElementSibling;return t instanceof HTMLElement&&t.classList.contains(`rank-detalj-rad`)?t:null},onToggle:(e,t)=>{e.classList.toggle(`rank-rad--open`,t),f(e,t)}})}export{n as a,a as i,o as n,d as r,p as t};