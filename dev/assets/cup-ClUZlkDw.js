import{B as e,D as t,G as n,L as r,V as i,W as a,b as o,nt as s,t as c}from"./index-IHBsWX3l.js";import{t as l}from"./LoadingState-RVZNML7E.js";import{t as u}from"./ConfirmDialog-DNGrXiEY.js";import{r as d}from"./kamp-BVHPp875.js";import{C as f,S as p,a as m,b as h,f as g,n as _,s as v,x as y,y as b}from"./kampService-BJceDH4N.js";import{t as x}from"./realtime-1KFDfsu_.js";import{a as S,i as C,n as w,o as T,r as E,s as D}from"./kampGenereringCupService-o3Dhi496.js";import{_ as O,b as k,c as A,d as j,f as M,g as N,h as P,l as F,n as I,p as L,r as R,t as z,u as B,v as V,x as H}from"./resultatService-BCsLhSF6.js";import{t as U}from"./ScoreNumberpad-DEYDshL3.js";function W(e,{visSpelarliste:t=!0,initNa:n=null,initFormat:r=null}={}){let i=typeof e==`number`?e:e.length,a=typeof e==`number`?[]:e.map((e,t)=>({...e,cupPlassering:t+1})),o=T(i),s=n===i?i:n!=null&&o.some(e=>e.nA===n)?n:o[0]?.nA??i,c=i-s,l=o.filter(e=>D(e.nA).some(e=>e.c3>0)),u=o.filter(e=>!D(e.nA).some(e=>e.c3>0)),d=D(i).length>0,f=(e,t)=>e.map((e,r)=>{let a=e.nA===s&&n!==i,o=t+r;return`
      ...
      </div>`));let m=p.join(``),h=r?.A??D(s)[0]??null,g=c>=2?r?.B??D(c)[0]??null:null,_=t?`<div id="gruppe-preview">${G(a,s,h?.walkovers??0,g?.walkovers??0)}</div>`:``;return`
  ...
  `}function G(e,t,n=0,r=0){let i=e.slice(0,t),o=e.slice(t);function s(e,t=0){return e.map((e,n)=>{let r=n<t;return`
      <tr>
    ...
    </div>`}function K(e){let t=e.c3>0?3:2;return`${e.walkovers} walkover - ${t} deltakere per bane`}function q(e,t,n,r=null){let i=D(t);return i.length<=1?``:`<div class="d-flex flex-column align-items-start gap-1 mb-2">${i.map((e,t)=>{let i=`${n}-${t}`,a=JSON.stringify(e),o=r?e.walkovers===r.walkovers&&e.c3===r.c3&&e.c2===r.c2:t===0,s=e.c3>0?`btn-outline-success`:`btn-outline-warning`;return`
    ...
    </div>`}function X(e,t,n,r,i,o){let l=n.filter(e=>e.runde_eliminert==null),u=n.length,d=l.length,f=r===1?i?.[t]??null:null,p=f?.walkovers??0,m=(f?f.c3:d%3==0?d/3:0)+(f?f.c2:d%3==0?0:d/2),h=l.slice(p,p+m),g=l.slice(p+m,p+2*m),_=l.slice(p+2*m),v=document.createElement(`div`);v.className=`avsl-dialog-overlay`,document.body.appendChild(v);let y=null,b=!1,x=0,S=0;function T(e){if(!b)return;let t=v.querySelector(`.avsl-dialog-card-wide`);if(!t)return;let n=e.clientX-x,r=e.clientY-S;y={left:n,top:r},t.style.left=`${n}px`,t.style.top=`${r}px`}function E(){b&&(b=!1,document.body.style.userSelect=``)}document.addEventListener(`mousemove`,T),document.addEventListener(`mouseup`,E);function D(){document.removeEventListener(`mousemove`,T),document.removeEventListener(`mouseup`,E)}function O(e){return`<div class="d-flex justify-content-between gap-2 py-1">
    ...
    <div class="avsl-gruppe-kol">
      ...
      ${f}
      ${u}
    </div>`}function ae(e,t,n=!0){let r=e.spelarar.slice().sort((e,n)=>(t[e.kasterid??0]??999)-(t[n.kasterid??0]??999)),i=e=>e?.kaster?`${a(e.kaster.fornavn)} ${a(e.kaster.etternavn)}`:`—`,o=e.er_bekreftet||e.er_walkover,s=r.some(e=>(e.omgangar?.length??0)>0),c=n&&e.er_bekreftet&&!e.er_tre_spelarar&&!s,l=e.er_walkover?`<tr>
        <td>${t[r[0]?.kasterid??0]??``}</td>
        <td colspan="2">${i(r[0])} <span class="badge bg-secondary">Walkover</span></td>
      </tr>`:r.map(n=>{let a=d(n),o=a>0?a:`—`,s=r.length,l=e.er_bekreftet&&n.kamp_plassering!=null&&n.kamp_plassering>=s,u=e.er_bekreftet&&n.kamp_plassering!=null&&n.kamp_plassering<s,f=l?`kamp-eliminert`:u?`kamp-vidare`:``,p=c?` data-endre-score="${e.id}" class="text-center score-redigerbar"`:` class="text-center"`;return`<tr${f?` class="${f}"`:``}>
          <td class="th-36 text-center">${t[n.kasterid??0]??``}</td>
          <td>${i(n)}</td>
          <td${p}>${o}</td>
        </tr>`}).join(``),u,f,p,m;if(e.er_tre_spelarar)u=o?`btn-secondary`:`btn-outline-secondary`,f=o?`Endre plassering`:`Sett plassering`,p=!1,m=``;else{let t=B(e,oe(r),s);u=o?`btn-secondary`:t?`btn-success`:`btn-outline-secondary`,f=o?`Bekreftet`:`Bekreft`,p=o||!t,m=` btn-bekreft`}return`
