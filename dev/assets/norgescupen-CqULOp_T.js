import{t as e}from"./logError-DhxY2JQv.js";import{Q as t,X as n,Z as r,at as i,ct as a,dt as o,lt as s,ot as c,st as l}from"./index-C7fZNAx-.js";import{t as u}from"./LoadingState-xRmJ3K_t.js";import{t as d}from"./EmptyState-BvE_0HiD.js";import{t as f}from"./Table-DSqyRM9K.js";import{t as p}from"./expandableRows-ECRCQeHx.js";var m=2007,h=2024,g={ar:new Date().getFullYear(),cupType:`NC`,klasse:1,visning:`singel`},_={ar:null,regler:null,stevner:[],resultater:[]};async function v(t){if(_.ar===t)return!0;try{let[{data:e,error:n},{stevner:r,resultater:a,error:o}]=await Promise.all([i(t),c(t)]);return n||o?!1:(_.ar=t,_.regler=e,_.stevner=r,_.resultater=a,!0)}catch(t){return e(`hentOgBufferData`,t),!1}}function y(e,t){return t===`SNC`?`Dei ${e.max_snc} beste SNC-stevna er teljande`:t===`DNC`?`Dei ${e.max_dnc} beste DNC-stevna er teljande`:`Dei ${e.maxtotal} beste stevna, herav maks ${e.max_nc_total} NC-stevner og ${e.max_snc_total} SNC-stevner er teljande`}function b(e){return`
    ...
    <div class="content-page">
      ...
      <div id="nc-content"></div>
    </div>`}async function E(e){if(g.ar=new Date().getFullYear(),g.cupType=`NC`,g.klasse=1,g.visning=`singel`,_={ar:null,regler:null,stevner:[],resultater:[]},e.replaceChildren(u(`Laster Norgescupen...`)),!await v(g.ar)){e.replaceChildren(n(`Kunne ikkje laste data for Norgescupen.`));return}e.innerHTML=T(g.ar,g.cupType);function t(){let{ar:n,cupType:r,klasse:i,visning:o}=g,{regler:s}=_,c=e.querySelector(`#nc-content`);if(e.querySelector(`.nc-hovudtittel`).textContent=`Norgescupen ${n}`,e.querySelector(`#nc-cuptype`).classList.toggle(`d-none`,n<h),e.querySelector(`#nc-visning-tabs-container`).innerHTML=r===`NC`?b(o):``,o===`lag`&&r===`NC`){c.innerHTML=`
        <section>
        ...
        </section>`;let e=c.querySelector(`#nc-lag-tabell-container`);if(!s)e.replaceChildren(d(`Ingen data.`));else{let t=l(_.resultater,_.stevner,s);e.replaceChildren(w(t)),p(e,{triggerSel:`.nc-lag-poeng-celle`,idAttr:`lag-idx`,detailSel:`.nc-lag-detalj-rad`,lookupRoot:c})}}else{c.innerHTML=`
        <section id="nc-singel-seksjon">
          <h2 class="nc-seksjon-tittel">${r} Singel ${n} - Klasse ${i}</h2>
          <p class="nc-beskriving">${s?y(s,r):`Ingen telleregel funnet for ${n}`}</p>
          <div id="nc-klasse-tabs-container">${x(i,n)}</div>
          <div id="nc-singel-tabell-container"></div>
        </section>`;let e=c.querySelector(`#nc-singel-tabell-container`);if(!s)e.replaceChildren(d(`Ingen data.`));else{let t=a(_.resultater,_.stevner,s,r,i);e.replaceChildren(C(t)),p(e,{triggerSel:`.nc-poeng-celle`,idAttr:`idx`,detailSel:`.nc-detalj-rad`,lookupRoot:c})}c.querySelector(`#nc-singel-seksjon`).addEventListener(`click`,e=>{let n=e.target.closest(`[data-klasse]`);n&&(g.klasse=Number(n.dataset.klasse),t())})}}t(),e.querySelector(`#nc-ar`).addEventListener(`change`,async r=>{if(g.ar=Number(r.target.value),g.klasse=1,g.ar<h&&(g.cupType=`NC`,g.visning=`singel`,e.querySelector(`#nc-cuptype`).value=`NC`),e.querySelector(`#nc-content`).replaceChildren(u()),!await v(g.ar)){e.querySelector(`#nc-content`).replaceChildren(n(`Feil ved henting av data.`));return}t()}),e.querySelector(`#nc-cuptype`).addEventListener(`change`,e=>{g.cupType=e.target.value,g.klasse=1,g.cupType!==`NC`&&(g.visning=`singel`),t()}),e.querySelector(`#nc-visning-tabs-container`).addEventListener(`click`,e=>{let n=e.target.closest(`[data-visning]`);n&&(g.visning=n.dataset.visning,t())})}export{E as render};