import{n as e,t}from"./logError-CTQ3euge.js";import{t as n}from"./escHtml-CfOHO0aD.js";import{E as r,I as i,Mt as a,P as o,R as s,S as c,T as l,Un as u,Y as d,Zn as f,_r as p,er as m,f as h,fn as g,h as _,hr as v,m as y,mr as b,pr as x,r as S,s as C,t as w,x as T,y as E,zt as D}from"./index-DrqrIQ7w.js";import{d as O,r as k}from"./kasterService-DIBRsqwT.js";import{t as A}from"./SearchSelect-B-n6Fpv7.js";import{n as j,r as M,t as ee}from"./accountService-BOVRYc0a.js";import{f as te,l as N}from"./kampService-CokIQ16C.js";import{n as P,t as F}from"./groupBy-Bg_SEHjk.js";import{t as I}from"./navigationService-Du2bISlr.js";import{n as L}from"./ScoreboardButton-cO_q_Bk1.js";function R(e,t,n){let r=(e?.spelarar??[]).filter(e=>e.kasterid!=null),i={};for(let t of r){let r=e?.stevneid==null?void 0:n[`${e.stevneid}:${t.kasterid}`];r!=null&&(i[t.kasterid]=r)}let a=u(r,i),o=a.findIndex(e=>e.members.some(e=>e.kasterid===t));return{mine:o===-1?[]:a[o]?.members??[],others:a.filter((e,t)=>t!==o).map(e=>e.members)}}function ne(e){return!e?.length||e.every(e=>e.kaster==null)}function z(e,t){return f({rep:e[0],members:e},t)}function re(e,t,n){let r=e?.er_bekreftet??!1,{mine:i,others:a}=R(e,t,n);if(e?.er_walkover)return{kind:`walkover`};if(r&&(e?.er_tre_spelarar||a.length>1)){let e=i.find(e=>e.kasterid===t)?.kamp_plassering;return e==null?{kind:`unknown`}:{kind:`placement`,placement:e}}let o=a[0];return!i.length||!o?.length?{kind:`unknown`}:{kind:`score`,me:z(i,r),them:z(o,r),confirmed:r}}function ie(e){return`
    ...
    </div>`}function B(e,t){let n=e.querySelector(`#pending-name`);!n||t==null||(async()=>{let{data:e}=await O(t);e&&(n.textContent=` for ${T(e)}`)})()}function V(e,t){let n=e.querySelector(`#thrower-error`),i=[],a=null;a=A({slot:e.querySelector(`#thrower-search-slot`),loadItems:async()=>{let{data:e}=await k();return i=e.map(e=>({id:e.id,label:c(e),sublabel:e.klubb?.navn??null})),i},placeholder:`Søk på navn…`,onSelect:e=>{e!=null&&(async()=>{n.classList.add(`d-none`);let o=i.find(t=>t.id===e),s=o?o.label+(o.sublabel?` (${o.sublabel})`:``):``;if(!await _({title:`Er dette deg?`,message:`Send koblingforespørsel for ${s}?`,confirmText:`Send forespørsel`})){a?.setValue(null);return}let{error:c}=await d(t,e);if(c){n.textContent=`Kunne ikkje sende forespørsel.`,n.classList.remove(`d-none`);return}r(),await S()})()}})}function H(e,t){return t.status===`godkjent`&&t.profil?.kasterid!=null?t.profil.kasterid:t.status===`venter`?(e.innerHTML=ae(),B(e,t.profil?.kobling_kasterid??null),null):(e.innerHTML=ie(t.status),V(e,t.user.id),null)}function U(e,t,r){let i=H(e,t);return i==null?null:(e.innerHTML=`
    <div class="card mb-4">
      <div class="card-body">
    ...
    <div class="card mb-4"><div class="card-body">
      ...
      <li class="nav-item">
        <a class="nav-link${e===t?` active`:``}"
           href="#/minside/${t}">${n}</a>
      </li>`).join(``)}</ul>`}async function $(e,r){w(()=>$(e,r));let i=String(r.tab??`kampar`);e.replaceChildren(v(`Laster min side…`));try{let t=await l();if(!t){location.hash=`#/logginn`;return}let{profil:r,user:a}=t,o=r?.kobling_status??`ingen`,s=i===`varslingar`?`innstillingar`:i,c=we.has(s)?s:`kampar`;e.innerHTML=`
      ...
      </div>`,e.querySelector(`[data-slot="logout"]`).appendChild(M());let u=e.querySelector(`#minside-subpage`);await Te[c](u,{user:a,profil:r,status:o})}catch(n){t(`minside.render`,n),e.replaceChildren(b(`Kunne ikkje laste min side.`))}}export{$ as render};