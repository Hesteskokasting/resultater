 ...
 */
export function validRingerRange(poeng: number, shoes: number = SHOES_PER_OMGANG): { min: number; max: number } {
  const min = Math.max(0, Math.ceil((poeng - MAX_SHOE_POENG_WITHOUT_RINGER * shoes) / (RINGER_POENG - MAX_SHOE_POENG_WITHOUT_RINGER)))
...
export function isValidTotalEntry(poeng: number, antallRinger: number, antallOmganger: number): boolean {
  if (!Number.isInteger(poeng) || !Number.isInteger(antallRinger)) return false
