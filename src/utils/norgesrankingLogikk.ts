...
export function calcRingInfo(r: RankingResultRow, eventInfo: EventInfo | undefined): RingInfo[] {
  ...
  if (r.antall_ring_xkast != null) {
    if (finn("minimatch"))
      liste.push({
        ...base,
        prosent: (r.antall_ring_xkast / 60) * 100,
        metodeNamn: "Minimatch",
        antallRing: r.antall_ring_xkast,
      });
    else if (finn("halvmatch"))
      liste.push({
        ...base,
        prosent: r.antall_ring_xkast,
        metodeNamn: "Halvmatch",
        antallRing: r.antall_ring_xkast,
      });
    else if (finn("heilmatch"))
      liste.push({
        ...base,
        prosent: (r.antall_ring_xkast / 200) * 100,
        metodeNamn: "Heilmatch",
        antallRing: r.antall_ring_xkast,
      });
  }
  if (r.antall_ring_kongelag != null)
    liste.push({
      ...base,
      prosent: (r.antall_ring_kongelag / 40) * 100,
      metodeNamn: "Kongelag",
      antallRing: r.antall_ring_kongelag,
    });
...
export function buildRankingList(
  ...
  eventsMap: Map<number, EventInfo>,
): RankingItem[] {
  const kasterMap = new Map<
    number,
    { kaster: RankingResultRow["kaster"]; klubb: RankingResultRow["klubb"]; rader: RingInfo[] }
  >();

  for (const r of resultater) {
    if (r.kasterid == null) continue;
    const ringInfoList = calcRingInfo(
      r,
      r.stevneid != null ? eventsMap.get(r.stevneid) : undefined,
    );
  ...
  for (const [, entry] of kasterMap) {
    ...
    const snittProsent =
      Math.round((top5.reduce((s, r) => s + r.prosent, 0) / top5.length) * 100) / 100;
