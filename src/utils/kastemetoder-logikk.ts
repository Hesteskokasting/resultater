...
export function calcCupStructure(
  n: number,
  { runde1 = null, walkovers1 = null }: Round1Param = {},
): CupRound[] {
  ...
  while (remaining > 2) {
    ...
    if (isRound1) {
      ...
      if (!r1) {
        r1 = validRound1Setups(remaining)[0] ?? {
          walkovers: remaining % 3,
          c3: Math.floor(remaining / 3),
          c2: 0,
        };
      }
...
export function calcCupRoundPairings(
  players: Player[],
  {
    medSeeding = true,
    isRunde1 = false,
