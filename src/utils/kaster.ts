...
export function throwerName(k: { fornavn?: string | null; etternavn?: string | null } | null | undefined): string {
  return [k?.fornavn, k?.etternavn].filter(Boolean).join(' ')
}

/** "Fornavn E." — first name plus last-name initial. */
export function throwerNameShort(k: { fornavn?: string | null; etternavn?: string | null } | null | undefined): string {
