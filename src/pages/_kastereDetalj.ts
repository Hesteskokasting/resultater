...
function detailSkeletonHtml(thrower: ThrowerDetailRow, results: ResultDetailRow[]): string {
  ...
  const years = [
    ...new Set(results.map((r) => getYear(r.stevne?.dato)).filter((a): a is number => a !== null)),
  ].sort((a, b) => b - a);
  const types = [
    ...new Map(
      results
...
function resultsListHtml(
  ...
  if (yearFilter !== "alle")
    filtered = filtered.filter((r) => String(getYear(r.stevne?.dato)) === yearFilter);
  if (typeFilter !== "alle")
    filtered = filtered.filter((r) => String(r.stevne?.stevnetype?.id) === typeFilter);
