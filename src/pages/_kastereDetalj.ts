...
function detailSkeletonHtml(thrower: ThrowerDetailRow, results: ResultDetailRow[]): string {
  ...
  const years = [...new Set(results.map(r => getYear(r.stevne?.dato)).filter((a): a is number => a !== null))].sort((a, b) => b - a)
  const types = [...new Map(
    results.map(r => r.stevne?.stevnetype).filter((t): t is { id: number; navn: string } => t != null).map(t => [t.id, t.navn])
...
function statisticsHtml(results: ResultDetailRow[], thrower: ThrowerDetailRow): string {
  ...
  const statsRows = stats.map(({ label, rekord, snittPoeng, snittProsent }) => `
    <tr>
      <td>${label}</td>
  ...
  try {
    ...
    function updateStatistics(): void {
      container.querySelector<HTMLElement>('#kd-statistics-content')!.innerHTML =
        statisticsHtml(results, thrower)
    }
