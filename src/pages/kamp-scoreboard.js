...
export async function render(container, { id } = {}) {
  ...
  async function _bekreftInnledende() {
    ...
    const spelarUpdates = []
    if (p1ks?.id) spelarUpdates.push(supabase.from('kamp_spelar').update({ score_poeng: t1 + hcp1, kamp_poeng: kp1, antall_ringer: r1 }).eq('id', p1ks.id))
    if (p2ks?.id) spelarUpdates.push(supabase.from('kamp_spelar').update({ score_poeng: t2 + hcp2, kamp_poeng: kp2, antall_ringer: r2 }).eq('id', p2ks.id))
