...
function beregnNcPoeng(
  rader: ResultWithRelations[],
  regler: Regler,
  eventsMap: EventsMap,
): ResultWithRelations[] {
  const nc: ResultWithRelations[] = [],
    snc: ResultWithRelations[] = [],
    dnc: ResultWithRelations[] = [];
...
function groupByThrower(
  rader: ResultWithRelations[],
): Map<number, { kaster: Kaster; rader: ResultWithRelations[] }> {
...
export function buildSingleList(
  ...
  for (const [, entry] of kasterMap) {
    ...
    const klubber = [
      ...new Set(tellendeRader.map((r) => r.klubb?.navn).filter((n): n is string => n != null)),
    ];
...
export function buildTeamList(
  ...
  for (const [, entry] of kasterMap) {
    ...
    for (const r of tellendeRader) {
      const klubb = r.klubb;
      if (klubb && r.klubbid != null && !klubbInfoMap.has(r.klubbid))
        klubbInfoMap.set(r.klubbid, klubb);
      if (r.klubbid != null)
        perKlubb.set(r.klubbid, (perKlubb.get(r.klubbid) ?? 0) + (r.nc_poeng ?? 0));
    }
  ...
  const klubbMap = new Map<
    number,
    { klubb: Klubb; bidragsytere: { kaster: Kaster; klubbId: number; sum: number }[] }
  >();
  ...
  for (const [, entry] of klubbMap) {
    ...
    teamList.push({
      klubb: entry.klubb,
      lagTotal: topp4.reduce((s, b) => s + b.sum, 0),
      bidragsytere: topp4,
      plassering: 0,
    });
  }
