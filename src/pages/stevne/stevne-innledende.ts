...
import { genererNesteSwissRunde } from '../../organizer/kampgenereringDb.js'
...
import { printStartkort } from '../../organizer/startkort-print.js'
import { escHtml } from '../../utils/escHtml.js'
import type { RealtimeChannel } from '@supabase/supabase-js'

interface InnlKlubb { kortnavn: string | null; navn: string | null }
interface InnlKaster { id: number; fornavn: string; etternavn: string; klubb: InnlKlubb | null }
interface InnlKampOmgang { score: number | null; antall_ringer: number | null }
interface InnlKampSpelar extends OrgKampSpelar {
  id: number
  kaster: InnlKaster | null
  omgangar: InnlKampOmgang[] | null
}
interface InnlKamp extends OrgKamp {
  id: number
  stevneid: number
  fase: string
  spelarar: InnlKampSpelar[] | null
}
interface InnlStevne {
  id: number
  navn: string
  erfullfort: boolean
...
interface InnlResultat { kasterid: number; startnummer: number | null; hcp: number | null }
interface RenderOpts { id?: number | string | null; isAdmin?: boolean }
...
export async function render(
  ...
  { id, isAdmin: _isAdmin = false }: RenderOpts = {},
...
async function lastOgVis(container: HTMLElement, stevneid: number): Promise<void> {
  ...
  const metodeNavn = typedStevne.kastemetode?.navn ?? ''
  ...
  if (bannerSlot) {
    bannerSlot.innerHTML = (isAdmin ? renderInnledendeKnappar(typedStevne, erAlleKamperBekreftet, erSwiss) : '') + startkortKnappHtml + toggleKnappHtml
  }
  ...
  if (erSwiss) {
    bannerSlot?.querySelector('#neste-runde-btn')?.addEventListener('click', async () => {
      if (!erAlleKamperBekreftet) { alert('Noen kamper er ikke bekreftet!'); return }
      try {
