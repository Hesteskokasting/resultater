...
const cupVariant: FinalPhaseVariant = {
  ...
  renderMatchesHtml: (ctx) => {
    ...
    const groupColumns = groupNames.map(g => {
  ...
  bindMatchEvents: (container, ctx) => {
    ...
    bindMatchEventsLocal(container, ctx.stevneid, ctx.finalMatches, ctx.isAdmin, ctx.reload, ctx.startNumberMap, ctx.positionMap)
  },
  ...
  bindHeaderEvents: (bannerSlot, ctx) => {
    ...
    if (!hasGroupAssignment) {
      ...
      function updateGroupPreview(nA: number, setupA: RoundSetup | null, setupB: RoundSetup | null): void {
        ...
        prevEl.innerHTML = renderGroupPreview(
          ...
          nA, setupA?.walkovers ?? 0, setupB?.walkovers ?? 0,
        )
      }
...
function renderGroupColumn(
  ...
  const roundsHtml = [...roundMap.entries()].reverse().map(([nr, roundMatches]) => {
...
function sideRowHtml(
  ...
  const score = (tot > 0 || (flags.isConfirmed && !flags.isThreeSides) || flags.hasRounds) ? tot : '—'
...
function playerRowsHtml(kamp: FinalMatchRow, sides: MatchSide<FinalMatchPlayerKnown>[], flags: MatchBlockFlags): string {
  if (kamp.er_walkover) {
    return `<tr>
...
function bindMatchEventsLocal(
  ...
  for (const kamp of finalMatches) {
    ...
    const writeSideScore = async (newS1: number, newS2: number): Promise<{ error: unknown } | null> => {
    ...
    if (isAdminLocal && kamp.er_bekreftet && !kamp.er_tre_spelarar) {
      ...
      container.querySelectorAll<HTMLElement>(`[data-endre-score="${kamp.id}"]`).forEach(cell => cell.addEventListener('click', handler))
    }
  }
}
