...
export function openThreeSideConfirmDialog(
  ...
  function renderDialog(): void {
    const eliminated = selected.length === 2 ? sides.find(s => !selected.includes(s.rep.kasterid)) : null
    modal.innerHTML = `
      <div class="card p-4 final-dialog-card">
        ...
        <div class="d-flex flex-column gap-2 mb-3">
          ${sides.map((side, i) => {
            ...
            ><span>${names[i]}</span>${
              placementLabel ? `<span class="badge bg-success-subtle text-success-emphasis">${placementLabel}</span>` :
              isEliminated ? `<span class="badge bg-danger">Utslått</span>` : ''
            }</button>`
          }).join('')}
        </div>
