...
async function lastOgVis(container: HTMLElement, stevneid: number): Promise<void> {
  ...
  const harFleirRundar = rundeMap.size > 1
  ...
  if (bannerSlot) {
    bannerSlot.innerHTML = (isAdmin ? renderInnledendeKnappar(typedStevne, erAlleKamperBekreftet, true) : '') + toggleKnappHtml
  }
...
function abonnerPaaEndringar(container: HTMLElement, stevneid: number): void {
  ...
  kanal = supabase
    .channel(`stevne-innl-nhm-${stevneid}`)
