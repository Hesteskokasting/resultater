...
function buildScoreboardOptions(
  ctx: MatchViewCtx,
  sides: SideState,
...
export async function render(container: HTMLElement, params: Params): Promise<void> {
  ...
  const throwerIds = (match.spelarar ?? [])
    .map((s) => s.kasterid)
    .filter((id): id is number => id != null);
  const { startNumberMap, positionMap, hcpMap } = await getMatchResultInfo(
    match.stevneid,
    throwerIds,
  );
