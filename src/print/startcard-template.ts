...
function createInfoTable(playerName: string | null, clubName: string): HTMLTableElement {
...
function createRoundsBody(roundInfos: RoundInfo[]): HTMLTableSectionElement {
  ...
  roundInfos.forEach((info, i) => {
    ...
    tr.id = `round-row-${i + 1}`;
    tr.appendChild(
      createTd(info.court != null ? String(info.court) : "", 1, `court-round-${i + 1}`, "small"),
    );
    ...
    tr.appendChild(
      createTd(
        info.opponentId != null ? String(info.opponentId) : "",
        1,
        `opponent-nr-round-${i + 1}`,
        "small",
      ),
    );
