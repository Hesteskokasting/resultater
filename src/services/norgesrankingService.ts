...
const _rankingStevneQuery = supabase
  .from("stevne")
  .select(
    "id, navn, dato, stevnetype:stevnetypeid(id, navn), innledendekastemetode:kastemetode!innledendekastemetodeid(navn), avsluttendekastemetode:kastemetode!avsluttendekastemetodeid(navn)",
  );

export type RankingTournamentRow = QueryData<typeof _rankingStevneQuery>[number];

const _rankingResultatQuery = supabase.from("resultat").select(`
...
export async function getTournamentsAndResults(ar: number): Promise<{
  ...
  const resultater = (rader ?? []).filter(
    (r) => r.antall_ring_xkast != null || r.antall_ring_kongelag != null,
  );
