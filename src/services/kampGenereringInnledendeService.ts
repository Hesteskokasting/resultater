...
async function _insertRounds(
  ...
  const { data: insertedMatches, error: kampErr } = await supabase
    ...
    .select("id, match_id");
  if (kampErr)
    throw new Error(`Feil ved innsetting av kampar (${errorContext}): ` + kampErr.message);
...
async function _insertSwissRound1(
  ...
  return _insertRounds(
    stevneid,
    [{ roundNumber: 1, matchups: buildSwissRound1Matchups(N) }],
    posToKasterids,
    "Swiss runde 1",
  );
}
...
export async function generateNextSwissRound(
  ...
  const { data: rawMatches, error } = await supabase
    .from("kamp")
    .select(
      "id, runde_nummer, er_bekreftet, er_walkover, spelarar:kamp_spelar(kasterid, kamp_poeng, score_poeng)",
    )
    .eq("stevneid", stevneid)
    .eq("fase", "innledende")
    .order("runde_nummer");
  ...
  type MatchRow = {
    ...
    er_walkover: boolean;
    spelarar:
      | { kasterid: number | null; kamp_poeng: number | null; score_poeng: number | null }[]
      | null;
  };
  ...
  const standing = allStartNrs.map((snr) => {
    ...
    for (const kamp of matchRows) {
      const sideRows = (kamp.spelarar ?? []).filter(
        (s): s is typeof s & { kasterid: number } =>
          s.kasterid != null && members.includes(s.kasterid),
      );
  ...
  if (!matchups)
    throw new Error("Paring er ikkje mogleg. Alle moglege motstandarar er allereie spela.");
