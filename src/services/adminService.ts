...
export async function updateUserRole(userId: string, role: string): Promise<{ error: unknown }> {
  const { error } = await supabase.from("bruker_profil").update({ rolle: role }).eq("id", userId);
...
export async function getClubAdminAssignments(): Promise<{
  data: ClubAdminAccess[];
  error: unknown;
}> {
  const { data, error } = await supabase.from("klubbadmin_klubber").select("bruker_id, klubbid");
