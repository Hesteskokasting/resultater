-- Fix: NOT IN with NULL runde_navn evaluates to NULL (false) in Postgres, causing
-- the Finale/Bronsefinale branch to execute for regular rounds. Restore IF branch
-- to check IN ('Finale', 'Bronsefinale') so NULL runde_navn falls correctly to
-- the ELSE (set runde_eliminert) branch.
...
BEGIN
  ...
  IF p_eliminert_kasterid IS NOT NULL THEN
    IF v_runde_navn IN ('Finale', 'Bronsefinale') THEN
      -- Finale/Bronsefinale: finalists get plassering, not runde_eliminert.
      -- Reset runde_eliminert for both in case of re-confirmation.
      ...
      END IF;
    ELSE
      -- Regular rounds (including NULL runde_navn): mark the loser as eliminated.
      UPDATE public.resultat
