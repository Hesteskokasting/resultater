-- Fix: bekreft_avsluttende_kamp_deltakar was setting runde_eliminert for Finale/Bronsefinale
-- losers (runner-up, 4th place). The TypeScript path (bekreftCupKamp) already skips this
-- for finals, so the RPC must match. Finalists get plassering instead of runde_eliminert.

-- Clean up existing bad data first.
UPDATE public.resultat
SET runde_eliminert = NULL
WHERE plassering IS NOT NULL
  AND runde_eliminert IS NOT NULL;
